const RELEASE='1.43.1-campaign-history';
...
self.addEventListener('install',event=>event.waitUntil((async()=>{
  ...
  await Promise.all(ASSETS.map(async path=>{
    ...
    if(path.startsWith('./app.js?')&&!(await response.clone().text()).includes("appVersion:'1.43.1'"))throw new Error('Release script mismatch');
    if(path.startsWith('./runtime.js?')&&!(await response.clone().text()).includes("version='1.43.1'"))throw new Error('Runtime script mismatch');
