...
const PROSPECT_MAP_SOURCE='agnt-prospector-map-source',PROSPECT_MAP_TOKEN_KEY='agnt-prospector-mapbox-token-v1',PROSPECT_MAP_BATCH_LIMIT=100;
...
function loadProspectorMapAssets(){if(window.mapboxgl)return Promise.resolve(window.mapboxgl);if(prospectMapAssetsPromise)return prospectMapAssetsPromise;prospectMapAssetsPromise=new Promise((resolve,reject)=>{if(!document.querySelector('#agntMapboxCss')){const link=document.createElement('link');link.id='agntMapboxCss';link.rel='stylesheet';link.href='https://api.mapbox.com/mapbox-gl-js/v3.14.0/mapbox-gl.css';document.head.append(link)}const existing=document.querySelector('#agntMapboxJs');if(existing){existing.addEventListener('load',()=>resolve(window.mapboxgl),{once:true});existing.addEventListener('error',()=>reject(new Error('Mapbox could not load')),{once:true});return}const script=document.createElement('script');script.id='agntMapboxJs';script.src='https://api.mapbox.com/mapbox-gl-js/v3.14.0/mapbox-gl.js';script.onload=()=>resolve(window.mapboxgl);script.onerror=()=>reject(new Error('Mapbox could not load'));document.head.append(script)});return prospectMapAssetsPromise}
...
function startProspectorMapCorrection(id){const p=prospectById(id);if(!p||!prospectMap)return;closeProspectorMapSheet();prospectMapManualProspectId=p.id;const current=prospectMapCurrentLocation(p),centre=current?[current.lng,current.lat]:prospectMap.getCenter();prospectMapManualCoordinate={lng:Number(centre.lng??centre[0]),lat:Number(centre.lat??centre[1])};if(prospectMapManualMarker)prospectMapManualMarker.remove();prospectMapManualMarker=new window.mapboxgl.Marker({color:'#111318',draggable:true}).setLngLat(prospectMapManualCoordinate).addTo(prospectMap);prospectMapManualMarker.on('dragend',()=>{const point=prospectMapManualMarker.getLngLat();prospectMapManualCoordinate={lng:point.lng,lat:point.lat}});$('#prospectorMapCorrection')?.classList.remove('hidden');prospectMap.easeTo({center:prospectMapManualCoordinate,zoom:Math.max(16,prospectMap.getZoom()),duration:400})}
...
async function geocodeProspectMapRecord(p,token){const address=prospectMapAddress(p),addressKey=prospectMapAddressKey(p),url=`https://api.mapbox.com/search/geocode/v6/forward?q=${encodeURIComponent(address)}&country=au&types=address&limit=1&permanent=true&access_token=${encodeURIComponent(token)}`,response=await fetch(url);if(response.status===401||response.status===403){const err=new Error('Mapbox token rejected');err.code='map-token';throw err}if(!response.ok)throw new Error(`Mapbox geocoding failed (${response.status})`);const data=await response.json(),feature=data?.features?.[0],coordinates=feature?.geometry?.coordinates;if(!feature||!Array.isArray(coordinates)||coordinates.length<2){persistProspectMapLocation({prospectId:p.id,addressKey,addressLabel:address,lat:null,lng:null,accuracy:'not-found',status:'failed',source:'mapbox',updatedAt:Date.now()});return false}const accuracy=cleanText(feature?.properties?.coordinates?.accuracy||feature?.properties?.accuracy||'unknown',40),status=['rooftop','parcel','point'].includes(accuracy)?'mapped':'review';persistProspectMapLocation({prospectId:p.id,addressKey,addressLabel:address,lng:Number(coordinates[0]),lat:Number(coordinates[1]),accuracy,status,source:'mapbox',providerFeatureId:feature.id,updatedAt:Date.now()});return true}
async function mapProspectorAddresses(){const token=prospectMapToken();if(!token){setProspectorMapSetup(true);return}if(prospectMapIsGeocoding)return;const pending=prospectMapAllRecords().filter(p=>!prospectMapCurrentLocation(p)).slice(0,PROSPECT_MAP_BATCH_LIMIT);if(!pending.length)return toast('All available addresses are mapped');prospectMapIsGeocoding=true;const button=$('#mapProspectorAddresses');if(button)button.disabled=true;let mapped=0,failed=0;showProspectorMapMessage('Mapping addresses',`0 of ${pending.length} complete`);try{for(let index=0;index<pending.length;index++){try{if(await geocodeProspectMapRecord(pending[index],token))mapped++;else failed++}catch(err){if(err?.code==='map-token')throw err;failed++;console.warn('Address could not be mapped',pending[index]?.id,err)}if(button)button.textContent=`Mapping ${index+1} of ${pending.length}`;showProspectorMapMessage('Mapping addresses',`${index+1} of ${pending.length} complete`);updateProspectorMap()}prospectMapFitPending=true;updateProspectorMap();showProspectorMapMessage('', '');toast(`${mapped} address${mapped===1?'':'es'} mapped${failed?` · ${failed} need review`:''}`)}catch(err){console.error('Prospector mapping stopped',err);showProspectorMapMessage('Map connection needs attention','Check the public token, billing and staging URL restriction.');if(err?.code==='map-token')setProspectorMapSetup(true)}finally{prospectMapIsGeocoding=false;updateProspectorMapStatus()}}
...
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='prospectorMapTokenForm'){e.preventDefault();const token=cleanText(new FormData(e.target).get('token')||'',700);if(!/^pk\./.test(token))return toast('Add a valid public Mapbox token');try{localStorage.setItem(PROSPECT_MAP_TOKEN_KEY,token)}catch{return toast('Map token could not be saved on this device')}resetProspectorMapInstance();setProspectorMapSetup(false);showProspectorMapMessage('', '');await ensureProspectorMap();updateProspectorMapStatus();toast('Mapbox connected');return}
