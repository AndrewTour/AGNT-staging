...
let buyerQuickFilter='All',buyerFilterState={budgetMin:0,budgetMax:BUYER_BUDGET_MAX,suburb:'',bedrooms:0,bathrooms:0,cars:0,propertyType:'',stage:'',temperature:'',features:new Set()},pendingBuyerEditorContext=null;
...
function normaliseLeaderboardAppointmentDetails(value){
  ...
  return value.map(item=>({type:normaliseAppointmentType(item?.type),bookedDate:validDateKey(item?.bookedDate)?item.bookedDate:'',scheduledDate:validDateKey(item?.scheduledDate)?item.scheduledDate:'',time:String(item?.time||''),contactName:String(item?.contactName||''),address:String(item?.address||''),assignedToUid:String(item?.assignedToUid||''),assignedToName:String(item?.assignedToName||'')})).filter(item=>['MAP','LAP','BAP','DATA'].includes(item.type)).map(item=>({...item,type:item.type==='DATA'?'Data':item.type}));
}
...
function showLeaderboardAgentSummary(agentUid){
  ...
  const cards=details?.length?details.map(item=>`<article class="leaderboard-agent-appointment"><span>${escapeHtml(item.type)}</span><div><strong>${escapeHtml(item.contactName||'Client not recorded')}</strong><small>${escapeHtml(item.address||'Address not recorded')}</small>${item.assignedToName?`<em class="leaderboard-booked-for">Booked for ${escapeHtml(teamAppointmentSetterFirstName(item.assignedToName))}</em>`:''}${item.type==='Data'?'':`<time>${escapeHtml(leaderboardAppointmentDateTime(item))}</time>`}</div></article>`).join(''):details===null&&knownTotal?`<div class="leaderboard-agent-summary-empty"><strong>${knownTotal} appointment${knownTotal===1?'':'s'} booked</strong><p>Details will appear after this agent receives the latest app update.</p></div>`:`<div class="leaderboard-agent-summary-empty"><strong>No appointments or data recorded</strong><p>There are no MAP, LAP, BAP or Data entries recorded for this period.</p></div>`;
...
function defaultBuyerFilters(){return{budgetMin:0,budgetMax:BUYER_BUDGET_MAX,suburb:'',bedrooms:0,bathrooms:0,cars:0,propertyType:'',stage:'',temperature:'',features:new Set()}}
function buyerBudgetText(min=0,max=0){min=Math.max(0,Number(min)||0);max=Math.max(0,Number(max)||0);if(!min&&!max)return'Budget not set';if(min&&!max)return`${formatBuyerMoney(min)}+`;if(!min&&max)return`Up to ${formatBuyerMoney(max)}`;return`${formatBuyerMoney(min)}–${formatBuyerMoney(max)}`}
function formatBuyerMoney(value=0){const n=Math.max(0,Number(value)||0);if(!n)return'Any';if(n>=1000000){const m=n/1000000;return`$${Number.isInteger(m)?m:m.toFixed(m<2?2:1).replace(/0+$/,'').replace(/\.$/,'')}m`}return`$${Math.round(n/1000)}k`}
function buyerRangeLabel(value,{max=false}={}){const n=Math.max(0,Number(value)||0);if(!n)return'Any';if(max&&n>=BUYER_BUDGET_MAX)return'No max';return formatBuyerMoney(n)}
...
function buyerFilterCount(){const f=buyerFilterState;return Number(f.budgetMin>0)+Number(f.budgetMax<BUYER_BUDGET_MAX)+Number(Boolean(f.suburb))+Number(f.bedrooms>0)+Number(f.bathrooms>0)+Number(f.cars>0)+Number(Boolean(f.propertyType))+Number(Boolean(f.stage))+Number(Boolean(f.temperature))+(f.features?.size||0)}
function buyerMatchesFilters(p){const f=buyerFilterState,filterBudgetActive=f.budgetMin>0||f.budgetMax<BUYER_BUDGET_MAX;if(filterBudgetActive){const hasBudget=Boolean(p.buyerBudgetMin||p.buyerBudgetMax);if(!hasBudget)return false;const buyerMin=Number(p.buyerBudgetMin)||0,buyerMax=Number(p.buyerBudgetMax)||Infinity,filterMax=f.budgetMax>=BUYER_BUDGET_MAX?Infinity:f.budgetMax;if(buyerMax<f.budgetMin||buyerMin>filterMax)return false}const suburb=cleanText(f.suburb,120).toLowerCase();if(suburb){const terms=suburb.split(/[,;|]/).map(x=>x.trim()).filter(Boolean);if(!terms.some(term=>(p.buyerSuburbs||[]).some(value=>value.toLowerCase().includes(term))))return false}if(f.bedrooms&&Number(p.buyerBedrooms||0)<f.bedrooms)return false;if(f.bathrooms&&Number(p.buyerBathrooms||0)<f.bathrooms)return false;if(f.cars&&Number(p.buyerCars||0)<f.cars)return false;if(f.propertyType&&p.buyerPropertyType!==f.propertyType)return false;if(f.stage&&p.buyerStage!==f.stage)return false;if(f.temperature&&p.temperature!==f.temperature)return false;if(f.features?.size&&![...f.features].every(feature=>(p.buyerFeatures||[]).includes(feature)))return false;if(buyerQuickFilter==='Hot'&&p.temperature!=='Hot')return false;if(buyerQuickFilter==='Warm'&&p.temperature!=='Warm')return false;if(['Looking','Inspecting','Negotiating'].includes(buyerQuickFilter)&&p.buyerStage!==buyerQuickFilter)return false;return true}
function filteredBuyers(){const q=cleanText($('#prospectSearch')?.value||'',120).toLowerCase();return activeBuyerProspects().filter(p=>buyerMatchesFilters(p)).filter(p=>!q||[p.name,p.phone,p.email,p.address,p.buyerStage,p.temperature,p.buyerPropertyType,...(p.buyerSuburbs||[]),...(p.buyerFeatures||[])].join(' ').toLowerCase().includes(q)).sort((a,b)=>(Number(b.updatedAt)||0)-(Number(a.updatedAt)||0)||a.name.localeCompare(b.name,'en-AU',{sensitivity:'base'}))}
function buyerCard(p){const phone=primaryProspectPhone(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#',tags=buyerTagsForCard(p),last=p.lastContact?`Last contact ${fmtDate(p.lastContact)}`:'No contact logged';return`<article class="buyer-card"><div class="buyer-card-head"><button type="button" data-open-buyer="${p.id}" class="buyer-card-name"><span class="prospect-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'B')}</span><span><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax))}</small></span></button><div class="buyer-card-actions"><button type="button" data-call-buyer="${p.id}" ${phone?'':'disabled'} aria-label="Call ${escapeHtml(p.name)}">Call</button><a href="${sms}" class="${phone?'':'disabled'}" aria-label="SMS ${escapeHtml(p.name)}">SMS</a></div></div><button type="button" data-open-buyer="${p.id}" class="buyer-card-brief"><strong>${escapeHtml(buyerConfigText(p))}</strong><small>${escapeHtml(buyerSuburbText(p))}</small><span class="buyer-card-tags">${tags.length?tags.slice(0,3).map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')+(tags.length>3?`<span>+${tags.length-3}</span>`:''):'<span>Requirements not set</span>'}</span><span class="buyer-card-footer"><span class="buyer-stage-pill">${escapeHtml(p.buyerStage||'Looking')}</span><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${escapeHtml(p.temperature)}</span></span><small class="buyer-card-last">${escapeHtml(last)}</small></button></article>`}
function renderBuyerProfiles(){const host=$('#buyerProfileList');if(!host)return;const list=filteredBuyers(),total=activeBuyerProspects().length,count=buyerFilterCount(),hasAnyFilter=count>0||buyerQuickFilter!=='All'||Boolean(cleanText($('#prospectSearch')?.value||'',120));$('#buyerProfileCount').textContent=`${total} active buyer${total===1?'':'s'}`;$('#buyerListMeta').textContent=hasAnyFilter?`${list.length} matching buyer${list.length===1?'':'s'}`:'Search or filter to narrow the database.';$('#buyerFilterCount').textContent=count;$('#buyerFilterCount').classList.toggle('active',count>0);$('#clearBuyerFilters').classList.toggle('hidden',!hasAnyFilter);$$('[data-buyer-quick-filter]').forEach(button=>button.classList.toggle('active',button.dataset.buyerQuickFilter===buyerQuickFilter));host.innerHTML=list.length?list.map(buyerCard).join(''):'<div class="prospect-empty"><strong>No matching buyers</strong><small>Adjust the search or filters, or add a new buyer.</small></div>'}
function syncBuyerFilterControls(){const f=buyerFilterState;if($('#buyerFilterSuburb'))$('#buyerFilterSuburb').value=f.suburb;if($('#buyerFilterBudgetMin'))$('#buyerFilterBudgetMin').value=String(f.budgetMin);if($('#buyerFilterBudgetMax'))$('#buyerFilterBudgetMax').value=String(f.budgetMax);if($('#buyerFilterBudgetMinLabel'))$('#buyerFilterBudgetMinLabel').textContent=buyerRangeLabel(f.budgetMin);if($('#buyerFilterBudgetMaxLabel'))$('#buyerFilterBudgetMaxLabel').textContent=buyerRangeLabel(f.budgetMax,{max:true});[['buyerFilterBedrooms','bedrooms'],['buyerFilterBathrooms','bathrooms'],['buyerFilterCars','cars'],['buyerFilterPropertyType','propertyType'],['buyerFilterStage','stage'],['buyerFilterTemperature','temperature']].forEach(([id,key])=>{if($('#'+id))$('#'+id).value=String(f[key]??'')});$$('[data-buyer-filter-feature]').forEach(button=>{const active=f.features.has(button.dataset.buyerFilterFeature);button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))})}
...
function setBuyerFilterFromControls(){const rawMin=Math.max(0,Number($('#buyerFilterBudgetMin')?.value)||0),rawMax=Math.max(0,Number($('#buyerFilterBudgetMax')?.value)||0),effectiveMax=rawMax||BUYER_BUDGET_MAX,min=Math.min(rawMin,effectiveMax),max=Math.max(rawMin,effectiveMax);buyerFilterState={...buyerFilterState,budgetMin:min,budgetMax:max,suburb:cleanText($('#buyerFilterSuburb')?.value||'',120),bedrooms:Number($('#buyerFilterBedrooms')?.value)||0,bathrooms:Number($('#buyerFilterBathrooms')?.value)||0,cars:Number($('#buyerFilterCars')?.value)||0,propertyType:cleanText($('#buyerFilterPropertyType')?.value||'',30),stage:cleanText($('#buyerFilterStage')?.value||'',30),temperature:cleanText($('#buyerFilterTemperature')?.value||'',20)};syncBuyerFilterControls();renderBuyerProfiles();updateTopbar()}
...
function renderBuyerSuburbChips(form){const host=form?.querySelector('[data-buyer-suburb-chips]');if(!host)return;const suburbs=Array.isArray(form._buyerSuburbs)?form._buyerSuburbs:[];host.innerHTML=suburbs.length?suburbs.map((suburb,index)=>`<span>${escapeHtml(suburb)}<button type="button" data-remove-buyer-suburb="${index}" aria-label="Remove ${escapeHtml(suburb)}">×</button></span>`).join(''):'<small>Add one or more target suburbs.</small>'}
function addBuyerSuburb(form){const input=form?.querySelector('[data-buyer-suburb-input]'),value=cleanText(input?.value||'',100).replace(/^,|,$/g,'').trim();if(!value)return;form._buyerSuburbs=Array.from(new Set([...(form._buyerSuburbs||[]),value])).slice(0,12);input.value='';renderBuyerSuburbChips(form);input.focus({preventScroll:true})}
function buyerEditorForm(p={},prefill={}){const current={...p,...prefill},min=Math.max(0,Number(current.buyerBudgetMin)||0),max=Number(current.buyerBudgetMax)||BUYER_BUDGET_MAX,stage=BUYER_STAGES.includes(current.buyerStage)&&current.buyerStage!=='Purchased'?current.buyerStage:'Looking',temperature=['Cold','Warm','Hot'].includes(current.temperature)?current.temperature:'Warm';return`<form id="buyerEditor" class="buyer-editor glass"><div class="prospect-detail-nav"><button type="button" data-close-buyer>‹ Back</button><strong>${p.id?'Edit Buyer':'New Buyer'}</strong><span></span></div><div class="buyer-editor-intro"><span>BUYER REQUIREMENTS</span><h2>${p.id?'Keep the brief current':'Capture the brief quickly'}</h2><p>Name and mobile first, then only what matters for the search.</p></div><div class="prospect-form-grid"><label>Name<input name="name" value="${escapeHtml(current.name||'')}" autocomplete="name" required></label><label>Mobile<input name="phone" type="tel" inputmode="tel" value="${escapeHtml(current.phone||'')}" autocomplete="tel"></label></div><label>Current address <small>Optional</small><input name="address" value="${escapeHtml(current.address||'')}" autocomplete="street-address"></label><section class="buyer-editor-section"><div class="buyer-editor-section-head"><span>BUDGET</span><strong><b id="buyerBudgetMinLabel">${buyerRangeLabel(min)}</b> – <b id="buyerBudgetMaxLabel">${buyerRangeLabel(max,{max:true})}</b></strong></div><div class="buyer-budget-controls"><label>From<input id="buyerBudgetMin" name="buyerBudgetMin" type="range" min="0" max="5000000" step="50000" value="${min}"></label><label>To<input id="buyerBudgetMax" name="buyerBudgetMax" type="range" min="0" max="5000000" step="50000" value="${max}"></label></div></section><section class="buyer-editor-section"><span class="buyer-field-label">CONFIGURATION</span><div class="buyer-config-grid"><div><small>Bedrooms</small>${buyerChoiceMarkup('buyerBedrooms',current.buyerBedrooms||0,[0,1,2,3,4,5],{0:'Any',5:'5+'})}</div><div><small>Bathrooms</small>${buyerChoiceMarkup('buyerBathrooms',current.buyerBathrooms||0,[0,1,2,3,4],{0:'Any',4:'4+'})}</div><div><small>Cars</small>${buyerChoiceMarkup('buyerCars',current.buyerCars||0,[0,1,2,3],{0:'Any',3:'3+'})}</div></div></section><section class="buyer-editor-section"><span class="buyer-field-label">SUBURBS</span><div class="buyer-suburb-entry"><input type="text" data-buyer-suburb-input placeholder="Add suburb" autocomplete="off"><button type="button" data-add-buyer-suburb>Add</button></div><div class="buyer-suburb-chips" data-buyer-suburb-chips></div></section><section class="buyer-editor-section"><span class="buyer-field-label">PROPERTY TYPE</span><div class="buyer-option-tags buyer-property-types">${buyerPropertyTypeMarkup(current.buyerPropertyType||'')}</div></section><section class="buyer-editor-section"><span class="buyer-field-label">MUST-HAVES</span><div class="buyer-option-tags buyer-feature-tags">${buyerFeatureMarkup(current.buyerFeatures||[])}</div></section><section class="buyer-editor-section"><span class="buyer-field-label">JOURNEY</span><div class="prospect-form-grid"><label>Stage<select name="buyerStage">${['Looking','Inspecting','Negotiating'].map(value=>`<option ${stage===value?'selected':''}>${value}</option>`).join('')}</select></label><label>Temperature<select name="temperature">${['Cold','Warm','Hot'].map(value=>`<option ${temperature===value?'selected':''}>${value}</option>`).join('')}</select></label></div></section><label>Buyer notes<textarea name="notes" rows="4" placeholder="Anything else that matters to the search">${escapeHtml(current.notes||'')}</textarea></label><button class="primary buyer-save-button" type="submit">${p.id?'Save Buyer':'Add Buyer'}</button></form>`}
function openBuyerEditor(id='',prefill={},context=null){const p=id?prospectById(id):{};activeProspectId=id||null;pendingBuyerEditorContext=context;setProspectorSection('buyers');$('#prospectingDashboard').classList.add('hidden');$('#prospectingSession').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');$('#prospectDetail').innerHTML=buyerEditorForm(p,prefill);const form=$('#buyerEditor');form._buyerSuburbs=[...(p?.buyerSuburbs||prefill.buyerSuburbs||[])];renderBuyerSuburbChips(form);requestAnimationFrame(()=>form.querySelector('input[name="name"]')?.focus({preventScroll:true}))}
async function upsertBuyer(data,id='',context=null){const existing=id?prospectById(id):null,createdAt=existing?.createdAt||Date.now(),dataCreditedAt=existing?.dataCreditedAt||(!existing?createdAt:0),record=normaliseProspect({...existing,...data,id:id||prospectId(),recordType:'buyer',stage:'Nurture',sellingTimeframe:'',temperatureManual:true,dataCreditedAt,createdAt,updatedAt:Date.now(),lastContact:context?.outcome&&context.outcome!=='Cancelled'?todayKey():(existing?.lastContact||'')});if(existing)prospects=prospects.map(item=>item.id===id?record:item);else prospects.unshift(record);if(context?.outcome&&context.outcome!=='Cancelled'&&context.interactionId&&!prospectInteractions.some(item=>item.id===context.interactionId))prospectInteractions.push({id:context.interactionId,prospectId:record.id,date:todayKey(),at:Number(context.at)||Date.now(),type:'Call',outcome:cleanText(context.outcome,80),note:'Added to buyer database from call outcome.',nextFollowUp:''});activeProspectId=record.id;await saveProspecting({render:false});if(!existing)await creditNewProspectData(record);const returnToBuyerSession=Boolean(context?.returnToBuyerSession&&buyerSession.active);pendingBuyerEditorContext=null;if(returnToBuyerSession){activeProspectId=null;setProspectorSection('today');renderProspecting();showBuyerSession()}else{setProspectorSection('buyers');renderProspecting();$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');renderBuyerDetail(record.id)}return record}
function buyerRequirementRows(p){const tags=buyerTagsForCard(p);return`<div class="buyer-detail-requirements"><div><span>BUDGET</span><strong>${escapeHtml(buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax))}</strong></div><div><span>CONFIGURATION</span><strong>${escapeHtml(buyerConfigText(p))}</strong></div><div class="wide"><span>SUBURBS</span><strong>${escapeHtml((p.buyerSuburbs||[]).join(' · ')||'Not set')}</strong></div><div class="wide"><span>PROPERTY</span><strong>${escapeHtml(tags.join(' · ')||'Requirements not set')}</strong></div></div>`}
function renderBuyerDetail(id){const p=prospectById(id);if(!p||p.recordType!=='buyer')return closeProspectDetail();activeProspectId=id;const history=interactionsFor(id),phone=primaryProspectPhone(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#';$('#prospectDetail').innerHTML=`<div class="prospect-detail-nav"><button type="button" data-close-buyer>‹ Back</button><button type="button" data-edit-buyer="${p.id}">Edit</button></div><section class="buyer-detail-card glass"><div class="buyer-detail-head"><span class="prospect-avatar large">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><div><span>${escapeHtml(p.buyerStage||'Looking')} · ${escapeHtml(p.temperature)}</span><h2>${escapeHtml(p.name)}</h2><small>${escapeHtml(displayDialNumber(phone)||p.address||'No mobile added')}</small></div></div><div class="prospect-quick-actions buyer-detail-actions"><button type="button" data-call-buyer="${p.id}" class="${phone?'':'disabled'}">Call</button><a href="${sms}" class="${phone?'':'disabled'}">SMS</a><button type="button" data-edit-buyer="${p.id}">Update</button></div>${buyerRequirementRows(p)}${p.notes?`<p class="prospect-background">${escapeHtml(p.notes)}</p>`:''}<button class="primary buyer-purchased-button" type="button" data-mark-buyer-purchased="${p.id}">Mark as Purchased</button></section><section class="prospecting-section glass"><div class="prospecting-section-head"><div><span>BUYER HISTORY</span><h3>Every conversation</h3></div></div><div class="prospect-history">${history.length?history.map(x=>`<article><i></i><div><strong>${escapeHtml(x.outcome||x.type)}</strong><small>${fmtDate(x.date)} · ${new Date(x.at).toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'})}</small>${x.note?`<p>${escapeHtml(x.note)}</p>`:''}</div></article>`).join(''):'<div class="prospect-empty"><strong>No buyer history yet</strong><small>Calls from the buyer list will build the timeline.</small></div>'}</div></section><button class="prospect-delete" type="button" data-delete-buyer="${p.id}">Delete Buyer</button>`}
...
function setProspectorSection(section='today'){
  ...
  const input=$('#prospectSearch');if(input)input.placeholder=prospectSection==='contacts'?'Search name, street, suburb, phone etc.':prospectSection==='buyers'?'Search buyers or suburbs':prospectSection==='pipeline'?'Search seller pipeline':'Search name, address or phone';const addButton=$('#addProspectButton');if(addButton)addButton.setAttribute('aria-label',prospectSection==='buyers'?'Add buyer':'Add prospect');
...
$('#prospectingView').onclick=async e=>{
  ...
  const deleteBuyer=e.target.closest('[data-delete-buyer]');if(deleteBuyer&&confirm('Delete this buyer and their interaction history?')){prospects=prospects.filter(p=>p.id!==deleteBuyer.dataset.deleteBuyer);prospectInteractions=prospectInteractions.filter(x=>x.prospectId!==deleteBuyer.dataset.deleteBuyer);await saveProspecting();closeProspectDetail();toast('Buyer deleted');return}
  ...
  const removeBuyerSuburb=e.target.closest('[data-remove-buyer-suburb]');if(removeBuyerSuburb){const form=removeBuyerSuburb.closest('#buyerEditor'),index=Number(removeBuyerSuburb.dataset.removeBuyerSuburb);form._buyerSuburbs=(form._buyerSuburbs||[]).filter((_,i)=>i!==index);renderBuyerSuburbChips(form);return}
...
$('#prospectingView').addEventListener('input',e=>{if(e.target.closest('#prospectorBroadcastPanel')){renderCampaignBroadcast();return}if(e.target.matches('#buyerFilterSuburb,#buyerFilterBudgetMin,#buyerFilterBudgetMax')){setBuyerFilterFromControls();return}if(e.target.matches('#buyerBudgetMin,#buyerBudgetMax')){const form=e.target.closest('#buyerEditor'),minInput=form?.querySelector('#buyerBudgetMin'),maxInput=form?.querySelector('#buyerBudgetMax');let min=Number(minInput?.value)||0,max=Number(maxInput?.value)||BUYER_BUDGET_MAX;if(min>max){if(e.target.id==='buyerBudgetMin')max=min;else min=max;if(minInput)minInput.value=String(min);if(maxInput)maxInput.value=String(max)}if($('#buyerBudgetMinLabel'))$('#buyerBudgetMinLabel').textContent=buyerRangeLabel(min);if($('#buyerBudgetMaxLabel'))$('#buyerBudgetMaxLabel').textContent=buyerRangeLabel(max,{max:true})}});
$('#prospectingView').addEventListener('change',e=>{
  if(e.target.matches('#buyerFilterBedrooms,#buyerFilterBathrooms,#buyerFilterCars,#buyerFilterPropertyType,#buyerFilterStage,#buyerFilterTemperature')){setBuyerFilterFromControls();return}
...
$('#prospectingView').addEventListener('keydown',e=>{if(e.key==='Enter'&&e.target.matches('[data-buyer-suburb-input]')){e.preventDefault();addBuyerSuburb(e.target.closest('#buyerEditor'))}});
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='buyerEditor'){e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),editingBuyerId=activeProspectId||'',wasExisting=Boolean(editingBuyerId);if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}const f=new FormData(form),maxValue=Number(f.get('buyerBudgetMax'))||BUYER_BUDGET_MAX,propertyType=form.querySelector('[data-buyer-property-type].active')?.dataset.buyerPropertyType||'',features=[...form.querySelectorAll('[data-buyer-feature].active')].map(button=>button.dataset.buyerFeature);try{await upsertBuyer({name:f.get('name'),phone:f.get('phone'),address:f.get('address'),buyerBudgetMin:Number(f.get('buyerBudgetMin'))||0,buyerBudgetMax:maxValue>=BUYER_BUDGET_MAX?0:maxValue,buyerBedrooms:Number(f.get('buyerBedrooms'))||0,buyerBathrooms:Number(f.get('buyerBathrooms'))||0,buyerCars:Number(f.get('buyerCars'))||0,buyerSuburbs:form._buyerSuburbs||[],buyerPropertyType:propertyType,buyerFeatures:features,buyerStage:f.get('buyerStage'),temperature:f.get('temperature'),notes:f.get('notes'),source:wasExisting?prospectById(editingBuyerId)?.source||'Buyer':'Buyer'},editingBuyerId,pendingBuyerEditorContext);toast(wasExisting?'Buyer saved':'Buyer added')}catch(err){console.error('Buyer save failed',err);toast('Buyer saved locally. Please check sync.');if(submit){submit.disabled=false;submit.textContent=wasExisting?'Save Buyer':'Add Buyer'}}return}
