...
function buyerCardMetaText(p){const parts=[],config=buyerConfigText(p),suburb=buyerPrioritySuburbText(p);if(config!=='Configuration not set')parts.push(config);if(suburb)parts.push(suburb);return parts.join(' · ')||'Buyer criteria not set'}
function buyerTagsForCard(p){return[p.buyerPropertyType,...(p.buyerFeatures||[])].filter(Boolean)}
function buyerFilterCount(){const f=buyerFilterState;return Number(f.budgetMin>0)+Number(Boolean(f.suburb))+Number(f.bedrooms>0)+Number(f.bathrooms>0)+Number(f.cars>0)+Number(Boolean(f.propertyType))+Number(Boolean(f.stage))+Number(Boolean(f.temperature))+(f.features?.size||0)}
...
function filteredBuyers(){const q=cleanText($('#prospectSearch')?.value||'',120).toLowerCase();return activeBuyerProspects().filter(p=>buyerMatchesFilters(p)).filter(p=>!q||[p.name,p.phone,p.email,p.address,p.buyerStage,p.temperature,p.buyerPropertyType,p.buyerSeller?'Buyer Seller':'',buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax),...(p.buyerSuburbs||[]),...(p.buyerFeatures||[])].join(' ').toLowerCase().includes(q)).sort((a,b)=>a.name.localeCompare(b.name,'en-AU',{sensitivity:'base'})||(Number(b.updatedAt)||0)-(Number(a.updatedAt)||0))}
function buyerCard(p){const phone=primaryProspectPhone(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#',meta=buyerCardMetaText(p);return`<article class="buyer-card"><button type="button" data-open-buyer="${p.id}" class="buyer-row-profile"><span class="buyer-row-head"><strong>${escapeHtml(p.name)}</strong><b>${escapeHtml(buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax))}</b></span><span class="buyer-row-meta"><span>${escapeHtml(meta)}</span>${p.buyerSeller?'<em class="buyer-seller-tag">Buyer Seller</em>':''}</span></button><div class="buyer-card-actions"><button type="button" data-call-buyer="${p.id}" ${phone?'':'disabled'} aria-label="Call ${escapeHtml(p.name)}">Call</button><a href="${sms}" class="${phone?'':'disabled'}" aria-label="SMS ${escapeHtml(p.name)}">SMS</a><button type="button" data-buyer-followup="${p.id}" aria-label="Add follow-up for ${escapeHtml(p.name)}">Follow Up</button></div></article>`}
function renderBuyerProfiles(){const host=$('#buyerProfileList');if(!host)return;const list=filteredBuyers(),total=activeBuyerProspects().length,count=buyerFilterCount(),hasAnyFilter=count>0||buyerQuickFilter!=='All'||Boolean(cleanText($('#prospectSearch')?.value||'',120));const profileCount=$('#buyerProfileCount');if(profileCount)profileCount.textContent=`${total} buyer${total===1?'':'s'}`;$('#buyerListMeta').textContent=hasAnyFilter?`${list.length} of ${total} buyer${total===1?'':'s'}`:`${total} buyer${total===1?'':'s'} · A–Z`;$('#buyerFilterCount').textContent=count;$('#buyerFilterCount').classList.toggle('active',count>0);$('#toggleBuyerFilters')?.classList.toggle('active',count>0);$('#clearBuyerFilters').classList.toggle('hidden',!hasAnyFilter);$$('[data-buyer-quick-filter]').forEach(button=>button.classList.toggle('active',button.dataset.buyerQuickFilter===buyerQuickFilter));host.innerHTML=list.length?list.map(buyerCard).join(''):'<div class="prospect-empty"><strong>No matching buyers</strong><small>Adjust the search or filters, or add a new buyer.</small></div>'}
...
function buyerEditorForm(p={},prefill={}){const current={...p,...prefill},max=buyerMaximumBudget(current),stage=BUYER_STAGES.includes(current.buyerStage)&&current.buyerStage!=='Purchased'?current.buyerStage:'Looking',temperature=['Cold','Warm','Hot'].includes(current.temperature)?current.temperature:'Warm';return`<form id="buyerEditor" class="buyer-editor glass"><div class="prospect-detail-nav"><button type="button" data-close-buyer>‹ Back</button><strong>${p.id?'Edit Buyer':'New Buyer'}</strong><span></span></div><div class="buyer-editor-intro"><span>BUYER REQUIREMENTS</span><h2>${p.id?'Keep the brief current':'Capture the brief quickly'}</h2><p>Keep the essentials simple. Add more detail only where it helps the search.</p></div><div class="prospect-form-grid"><label>Name<input name="name" value="${escapeHtml(current.name||'')}" autocomplete="name" required></label><label>Mobile<input name="phone" type="tel" inputmode="tel" value="${escapeHtml(current.phone||'')}" autocomplete="tel"></label></div><label>Current address <small>Optional</small><input name="address" value="${escapeHtml(current.address||'')}" autocomplete="street-address"></label><section class="buyer-editor-section buyer-budget-section"><div class="buyer-editor-section-head"><span>MAXIMUM BUDGET</span><strong id="buyerBudgetMaxLabel">${buyerBudgetEditorLabel(max)}</strong></div><input id="buyerBudgetMax" name="buyerBudgetMax" class="buyer-budget-slider" type="range" min="0" max="5000000" step="50000" value="${max}" aria-label="Maximum buyer budget"></section><section class="buyer-editor-section"><span class="buyer-field-label">SUBURBS</span><div class="buyer-suburb-picker"><div class="buyer-suburb-entry"><input type="search" data-buyer-suburb-input list="buyerSydneySuburbs" placeholder="Add suburb" autocomplete="off"><button type="button" data-add-buyer-suburb>Add</button></div><div class="buyer-suburb-suggestions hidden" data-buyer-suburb-suggestions></div></div><div class="buyer-suburb-chips" data-buyer-suburb-chips></div></section><section class="buyer-editor-section"><span class="buyer-field-label">CONFIGURATION</span><div class="buyer-config-grid"><div><small>Bedrooms</small>${buyerChoiceMarkup('buyerBedrooms',current.buyerBedrooms||0,[0,1,2,3,4,5],{0:'Any'})}</div><div><small>Bathrooms</small>${buyerChoiceMarkup('buyerBathrooms',current.buyerBathrooms||0,[0,1,2,3,4],{0:'Any'})}</div><div><small>Cars</small>${buyerChoiceMarkup('buyerCars',current.buyerCars||0,[0,1,2,3],{0:'Any'})}</div></div></section><section class="buyer-editor-section buyer-position-section"><span class="buyer-field-label">POSITION</span><div class="buyer-option-tags buyer-position-tags"><button type="button" class="${current.buyerSeller?'active':''}" data-buyer-seller aria-pressed="${Boolean(current.buyerSeller)}">Buyer Seller</button></div><small class="buyer-position-help">Has a property to sell if they buy.</small></section><section class="buyer-editor-section"><span class="buyer-field-label">PROPERTY TYPE</span><div class="buyer-option-tags buyer-property-types">${buyerPropertyTypeMarkup(current.buyerPropertyType||'')}</div></section><section class="buyer-editor-section"><span class="buyer-field-label">MUST-HAVES</span><div class="buyer-option-tags buyer-feature-tags">${buyerFeatureMarkup(current.buyerFeatures||[])}</div></section><section class="buyer-editor-section"><span class="buyer-field-label">JOURNEY</span><div class="prospect-form-grid"><label>Stage<select name="buyerStage">${['Looking','Inspecting','Negotiating'].map(value=>`<option ${stage===value?'selected':''}>${value}</option>`).join('')}</select></label><label>Temperature<select name="temperature">${['Cold','Warm','Hot'].map(value=>`<option ${temperature===value?'selected':''}>${value}</option>`).join('')}</select></label></div></section><label>Buyer notes<textarea name="notes" rows="4" placeholder="Anything else that matters to the search">${escapeHtml(current.notes||'')}</textarea></label><button class="primary buyer-save-button" type="submit">${p.id?'Save Buyer':'Add Buyer'}</button></form>`}
function openBuyerEditor(id='',prefill={},context=null){const p=id?prospectById(id):{};activeProspectId=id||null;pendingBuyerEditorContext=context;setProspectorSection('buyers');$('#prospectingDashboard').classList.add('hidden');$('#prospectingSession').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');$('#prospectDetail').innerHTML=buyerEditorForm(p,prefill);ensureBuyerSuburbDatalist();const form=$('#buyerEditor');form._buyerSuburbs=[...(p?.buyerSuburbs||prefill.buyerSuburbs||[])];renderBuyerSuburbChips(form);requestAnimationFrame(()=>form.querySelector('input[name="name"]')?.focus({preventScroll:true}))}
...
function openBuyerFollowUp(id){const p=prospectById(id);if(!p||p.recordType!=='buyer')return;const returnFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;closeBuyerFollowUp({restoreFocus:false});buyerFollowUpReturnFocus=returnFocus;const overlay=document.createElement('div');overlay.className='buyer-followup-overlay';overlay.innerHTML=`<form id="buyerFollowUpForm" class="buyer-followup-sheet glass" data-buyer-id="${escapeHtml(p.id)}" role="dialog" aria-modal="true" aria-labelledby="buyerFollowUpTitle"><div class="buyer-followup-head"><div><span>FOLLOW UP</span><h2 id="buyerFollowUpTitle">${escapeHtml(p.name)}</h2></div><button type="button" data-close-buyer-followup aria-label="Close">×</button></div><label>Follow-up date<input name="followUpDate" type="date" value="${buyerFollowUpDefaultDate(p)}" required></label><label>Notes<textarea name="followUpNote" rows="4" placeholder="What needs to happen next?"></textarea></label><div class="buyer-followup-actions"><button class="secondary" type="button" data-close-buyer-followup>Cancel</button><button class="primary" type="submit">Add Follow Up</button></div></form>`;document.body.append(overlay);document.body.classList.add('buyer-followup-open');const form=overlay.querySelector('#buyerFollowUpForm');overlay.addEventListener('click',event=>{if(event.target===overlay||event.target.closest('[data-close-buyer-followup]')){event.preventDefault();event.stopPropagation();closeBuyerFollowUp()}});overlay.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();closeBuyerFollowUp()}});form?.addEventListener('submit',async event=>{event.preventDefault();event.stopPropagation();const submit=form.querySelector('button[type=submit]'),f=new FormData(form),buyerId=form.dataset.buyerId;if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}try{await scheduleBuyerFollowUp(buyerId,f.get('followUpDate'),f.get('followUpNote'))}catch(err){console.error('Buyer follow-up save failed',err);toast('Follow-up saved locally. Please check sync.');if(submit){submit.disabled=false;submit.textContent='Add Follow Up'}}});requestAnimationFrame(()=>overlay.querySelector('textarea')?.focus({preventScroll:true}))}
async function scheduleBuyerFollowUp(id,date,note=''){const p=prospectById(id),followUpDate=validDateKey(date)?date:'';if(!p||p.recordType!=='buyer'||!followUpDate)return false;const at=Date.now();prospectInteractions.push({id:prospectId(),prospectId:p.id,date:todayKey(),at,type:'Follow-up',outcome:'Follow-up scheduled',note:cleanText(note,2000),nextFollowUp:followUpDate});prospects=prospects.map(item=>item.id===p.id?normaliseProspect({...item,nextFollowUp:followUpDate,updatedAt:at}):item);await saveProspecting({render:false});closeBuyerFollowUp();renderBuyerProfiles();renderTimeline();renderNowCard();if(activeProspectId===p.id&&!$('#prospectDetail')?.classList.contains('hidden'))renderBuyerDetail(p.id);updateTopbar();toast('Buyer follow-up added');return true}
...
$('#prospectingView').onclick=async e=>{
  ...
  const buyerSeller=e.target.closest('[data-buyer-seller]');if(buyerSeller){buyerSeller.classList.toggle('active');buyerSeller.setAttribute('aria-pressed',String(buyerSeller.classList.contains('active')));return}
...
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='buyerEditor'){e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),editingBuyerId=activeProspectId||'',wasExisting=Boolean(editingBuyerId);if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}const f=new FormData(form),maxValue=Math.max(0,Number(f.get('buyerBudgetMax'))||0),propertyType=form.querySelector('[data-buyer-property-type].active')?.dataset.buyerPropertyType||'',features=[...form.querySelectorAll('[data-buyer-feature].active')].map(button=>button.dataset.buyerFeature);try{await upsertBuyer({name:f.get('name'),phone:f.get('phone'),address:f.get('address'),buyerBudgetMin:0,buyerBudgetMax:maxValue,buyerBedrooms:Number(f.get('buyerBedrooms'))||0,buyerBathrooms:Number(f.get('buyerBathrooms'))||0,buyerCars:Number(f.get('buyerCars'))||0,buyerSuburbs:form._buyerSuburbs||[],buyerPropertyType:propertyType,buyerFeatures:features,buyerSeller:Boolean(form.querySelector('[data-buyer-seller].active')),buyerStage:f.get('buyerStage'),temperature:f.get('temperature'),notes:f.get('notes'),source:wasExisting?prospectById(editingBuyerId)?.source||'Buyer':'Buyer'},editingBuyerId,pendingBuyerEditorContext);toast(wasExisting?'Buyer saved':'Buyer added')}catch(err){console.error('Buyer save failed',err);toast('Buyer saved locally. Please check sync.');if(submit){submit.disabled=false;submit.textContent=wasExisting?'Save Buyer':'Add Buyer'}}return}
