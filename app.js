...
function renderAppointments(){
  ...
  if(appointmentHistoryMode&&$('#appointmentHistoryList')){
    ...
    $('#appointmentHistoryList').innerHTML=history.length?appointmentHistoryGroupedMarkup(appointmentHistoryMode,history):emptyStateMarkup(getEmptyState('appointments-history',{mode:appointmentHistoryMode}));
  }
...
let myMarketFilter='current',myMarketSuburb='',myMarketCategory='',myMarketLimit=30;
...
function myMarketContactMarkup(contact){
...
function myMarketAnalyticsMarkup(rows){
  const stats=rows.map(myMarketCampaignStats),soldView=myMarketFilter==='sold',metric=(label,values,format)=>{values=values.filter(value=>value!=null).sort((a,b)=>a-b);const n=values.length,median=n?(values[Math.floor((n-1)/2)]+values[Math.floor(n/2)])/2:null;return[label,n?`${format(median)} · ${n} recorded`:'Not enough recorded data']};
  return myMarketFactsMarkup([metric(soldView?'Median sold price':'Median recorded guide',stats.map(s=>soldView?s.sold:s.final),myMarketMoney),metric('Median reported days on market',stats.map(s=>s.days),v=>`${Math.round(v)} days`),soldView?metric('Median sale vs first recorded guide',stats.map(s=>s.delta),myMarketPercent):['Campaigns with a recorded price change',`${stats.filter(s=>s.changes>0).length} of ${stats.length}`]])+'<p class="my-market-muted">Based on this selection only. Price ranges and undisclosed prices are excluded from price calculations. Days on market use the source figure, not email dates.</p>';
}
function myMarketPropertyMarkup(row,recorded){
  ...
  return`<details class="my-market-property" data-property-key="${escapeHtml(row.key)}"><summary><span><small>${label} · ${escapeHtml(marketPulseEventPropertyCategory(e)||'Category not recorded')}</small><strong>${escapeHtml(e.address)}</strong><small>${escapeHtml(e.suburb)} · ${escapeHtml(e.propertyDetails||'Configuration not recorded')}</small><b>${escapeHtml(price)}</b><small>${sold?'Sale reported':'Latest report'} ${escapeHtml(fmtDate(e.receivedDate))} · ${escapeHtml(agency)}</small></span></summary><div class="my-market-property-body">${e.agents?.length?`<p>${escapeHtml(e.agents.join(', '))}</p>`:''}${myMarketFactsMarkup(facts)}<h4>Campaign history</h4><p class="my-market-muted">Recorded reports, oldest first. Dates are report dates. Earlier or unreported steps may be missing.</p><ol class="my-market-history">${history}</ol><details class="my-market-outreach"><summary>Recorded outreach · ${recorded.length} contact${recorded.length===1?'':'s'}</summary><p class="my-market-muted">Includes attempts and requested follow-ups; these do not confirm successful contact.</p>${recorded.length?recorded.map(myMarketContactMarkup).join(''):'<p class="my-market-muted">No property-linked outreach recorded.</p>'}</details></div></details>`;
}
function renderMyMarketHub(){
  ...
  $('#myMarketCounts').innerHTML=counts.map(([label,count])=>`<span><strong>${count}</strong>${label}</span>`).join('');
  $$('[data-my-market-filter]').forEach(button=>{const active=button.dataset.myMarketFilter===myMarketFilter;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))});
  ...
  const contacts=myMarketContactIndex(rows),list=$('#myMarketProperties'),open=new Set([...list.querySelectorAll('details[open]')].map(el=>el.dataset.propertyKey));
  ...
  if(list.dataset.markup!==markup){list.innerHTML=markup;list.dataset.markup=markup;for(const detail of list.querySelectorAll('details'))detail.open=open.has(detail.dataset.propertyKey)}
...
function openMyMarketHub(){marketPageMode='hub';setProspectorSection('market');renderProspecting()}
...
function setMarketPageMode(mode='hub',{refreshHeader=true}={}){
  ...
  $('#myMarketHub')?.classList.toggle('hidden',marketPageMode!=='hub');
...
async function flushProspectingSave(){
  ...
  try{await setDoc(doc(db,'users',uid,'prospecting','state'),{...payload,clientUpdatedAt:revision||Date.now(),updatedAt:serverTimestamp()},{merge:false});lastProspectingSignature=signature;clearProspectingDirty(revision);clearTimeout(prospectingRetryTimer);prospectingRetryTimer=null;prospectingRetryDelay=2500;clearSyncError();endSyncOperation();waiters.forEach(({resolve})=>resolve())}
  catch(err){console.error(err);endSyncOperation({error:true});waiters.forEach(({reject})=>reject(err));toast('Prospecting changes saved on this device. Cloud sync will retry.');scheduleProspectingRetry()}
...
$('#prospectingView').onclick=async e=>{
  const marketFilter=e.target.closest('[data-my-market-filter]');if(marketFilter){myMarketFilter=marketFilter.dataset.myMarketFilter;myMarketLimit=30;renderMyMarketHub();return}
  ...
  const section=e.target.closest('[data-prospector-section]');if(section){e.preventDefault();e.stopPropagation();homeQuickProspectorReturn=false;const target=section.dataset.prospectorSection;if(target==='market')marketPageMode='hub';if(target==='today')prospectTodayMode='dashboard';setProspectorSection(target,{todayMode:target==='today'?'dashboard':null});renderProspecting();return}
