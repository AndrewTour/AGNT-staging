...
function buyerCard(p){const phone=primaryProspectPhone(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#',config=buyerConfigText(p),configLabel=config==='Configuration not set'?'—':config;return`<article class="buyer-card"><button type="button" data-open-buyer="${p.id}" class="buyer-row-profile"><span class="buyer-row-head"><strong>${escapeHtml(p.name)}</strong><b>${escapeHtml(buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax))}</b></span><span class="buyer-row-meta"><span>${escapeHtml(configLabel)}</span>${p.buyerSeller?'<em class="buyer-seller-tag">Buyer Seller</em>':''}</span></button><div class="buyer-card-actions"><button type="button" data-call-buyer="${p.id}" ${phone?'':'disabled'} aria-label="Call ${escapeHtml(p.name)}">Call</button><a href="${sms}" class="${phone?'':'disabled'}" aria-label="SMS ${escapeHtml(p.name)}">SMS</a><button type="button" data-buyer-followup="${p.id}" aria-label="Add follow-up for ${escapeHtml(p.name)}">Follow Up</button></div></article>`}
...
function closeBuyerFollowUp(){document.querySelectorAll('.buyer-followup-overlay').forEach(node=>node.remove())}
