...
import { initializeFirestore, memoryLocalCache, collection, doc, setDoc, getDoc, getDocs, query, where, writeBatch, onSnapshot, serverTimestamp } from 'https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js';
...
const SELLER_PROPERTY_TYPES=['House','Duplex','Townhouse','Villa','Unit','Land'];
const SELLER_TITLE_TYPES=['Torrens title','Strata title','Community title'];
const SELLER_LAND_SIZE_BANDS=['under-400','400-599','600-999','1000-plus'];
const SELLER_LAND_SIZE_LABELS={'under-400':'Under 400m²','400-599':'400–599m²','600-999':'600–999m²','1000-plus':'1,000m²+'};
...
const SELLER_PRIORITY_DEFERRAL_VERSION=1;
const SELLER_PRIORITY_MAX_CANDIDATES=2500;
const SELLER_PRIORITY_CHUNK_SIZE=100;
...
let appointmentAssignees=[],assignedTeamAppointments=[],assignedTeamTasks=[],pendingTeamAppointmentNotice=null,teamAppointmentNoticeOpen=false,teamAppointmentNoticeReturnState=null,dismissedTeamAppointmentNotices=new Set(),subscribedAppointmentTeamId='',pendingAppointmentAssignment=null;
...
let sellerPriorityRefreshTimer=null,sellerPriorityBuildToken=0,sellerPriorityBuilding=false,sellerPriorityRefreshRequested=false;
...
function blankDay(){return{calls:0,connects:0,data:0,knockSeconds:0,timerStartedAt:null,appointments:[],tasks:[],events:[],review:{},clientUpdatedAt:0}}
...
function taskTimestampMillis(value){if(typeof value?.toMillis==='function')return value.toMillis();if(value instanceof Date)return value.getTime();if(Number(value?.seconds))return Number(value.seconds)*1000;const numeric=Number(value);return Number.isFinite(numeric)&&numeric>0?numeric:0}
function normaliseTaskRecord(raw={},sourceDate=''){
  const task=raw&&typeof raw==='object'?{...raw}:{},scheduledDate=validDateKey(task.scheduledDate)?task.scheduledDate:validDateKey(task.date)?task.date:sourceDate,time=/^([01]\d|2[0-3]):[0-5]\d$/.test(String(task.time||''))?String(task.time):'09:00',scheduledAt=Number(task.scheduledAt)||new Date(`${scheduledDate}T${time}`).getTime();
  return{...task,id:String(task.id||task.taskId||uuid()),taskId:String(task.taskId||task.id||''),title:cleanText(task.title,160)||'Task',note:cleanText(task.note,1000),date:scheduledDate,scheduledDate,time,scheduledAt:Number.isFinite(scheduledAt)?scheduledAt:0,createdDate:validDateKey(task.createdDate)?task.createdDate:sourceDate,assignedToUid:String(task.assignedToUid||uid||''),assignedToName:cleanText(task.assignedToName,120)||displayAgentName(),setterUid:String(task.setterUid||uid||''),setterName:cleanText(task.setterName,120)||displayAgentName(),at:taskTimestampMillis(task.at)||Date.now(),completedAt:taskTimestampMillis(task.completedAt),isTeamAssigned:Boolean(task.isTeamAssigned)};
}
function normaliseTasks(list,sourceDate=''){const seen=new Set();return(Array.isArray(list)?list:[]).map(task=>normaliseTaskRecord(task,sourceDate)).filter(task=>{if(seen.has(task.id))return false;seen.add(task.id);return true})}
function normaliseDayRecord(raw={},sourceDate=''){
  const value=raw&&typeof raw==='object'?raw:{};
  return{...blankDay(),...value,calls:Math.max(0,Number(value.calls)||0),connects:Math.max(0,Number(value.connects)||0),data:Math.max(0,Number(value.data)||0),knockSeconds:Math.max(0,Number(value.knockSeconds)||0),timerStartedAt:Number.isFinite(Number(value.timerStartedAt))?Number(value.timerStartedAt):null,appointments:normaliseAppointments(value.appointments,sourceDate),tasks:normaliseTasks(value.tasks,sourceDate),events:Array.isArray(value.events)?value.events.filter(Boolean).slice(-500):[],review:value.review&&typeof value.review==='object'?value.review:{},clientUpdatedAt:Number(value.clientUpdatedAt)||0};
}
...
function navigateDailyPlanAction(action='open',eventId=''){
  ...
  if(action==='open-buyer'&&eventId){const buyer=prospectById(eventId);if(!prospectHasActiveBuyerRole(buyer))return toast('Buyer could not be found');const returnView=activeViewId();sellerPriorityReturnView=['todayView','scheduleView'].includes(returnView)?returnView:'';switchView('prospectingView');setProspectorSection('buyers');renderProspecting();requestAnimationFrame(()=>renderBuyerDetail(buyer.id));return}
  if(action==='open-seller'&&eventId){const seller=prospectById(eventId);if(!seller)return toast('Seller could not be found');const returnView=activeViewId();switchView('prospectingView');setProspectorSection('contacts');renderProspecting();requestAnimationFrame(()=>renderProspectDetail(seller.id,{returnView}));return}
...
function pageHeaderState(id=activeViewId()){
  ...
  if(id==='appointmentsView'){
    ...
    if(appointmentHistoryMode==='week')return{title:'Appointments',subtitle:'Upcoming appointments for this calendar week.'};
...
function dayStatsLines(k=selectedDate){
  ...
  const add=(value,label)=>{const count=Math.max(0,Math.round(Number(value)||0));if(count)lines.push(`${count} ${label}`)};
  add(d.calls,'Calls');add(knocking.knocks,'Knocks');add(d.connects,'Connects');add(d.data,'Data');add(appointments.LAP,'LAP');add(appointments.MAP,'MAP');add(appointments.BAP,'BAP');
  return lines;
}
function sendDayStatsToWhatsApp(k=selectedDate){
  const lines=dayStatsLines(k);if(!lines.length)return toast('No activity has been logged for this day yet');
  window.location.assign(`https://wa.me/?text=${encodeURIComponent(lines.join('\n'))}`);
}
function renderDayLog(){
  ...
  $('#dayLogDateLabel').textContent=k===todayKey()?'Today Log':`${fmtDate(k)} Log`;
...
function renderToday(){
  ...
  const past=isPastDate(selectedDate),scheduled=isWorkDayKey(selectedDate),locked=past||!scheduled,offDayOpportunityMode=!past&&!scheduled&&selectedDate===todayKey();
  updateTopbar();
  updateBackTodayVisibility();
  $('#lockBadge').classList.toggle('hidden',!past);$('#lockBadge').textContent='LOCKED';
  $('#dayScheduleStatus')?.classList.toggle('hidden',scheduled||past);
  $('#todayView').classList.toggle('date-locked',locked);
  $('#todayView').classList.toggle('off-day-opportunity-mode',offDayOpportunityMode);
  $('.dashboard .score-week')?.classList.toggle('hidden',offDayOpportunityMode);
  $('.dashboard .metrics')?.classList.toggle('hidden',offDayOpportunityMode);
  $('#offDayConversations')?.classList.toggle('hidden',!offDayOpportunityMode);
  if(offDayOpportunityMode)renderOffDayHome();
  if($('#welcomeMessage')){
  ...
  $('#knockRemaining').textContent=past?'Day locked':(!scheduled?'Not scheduled':knockPaceText(Math.floor(secs/60),kt));
  const knockMinutes=Math.floor(secs/60),knockActual=pct(knockMinutes,kt);
  $('#knockingMetricCard')?.classList.toggle('complete',knockActual>=100);
  const knockPaceNow=new Date(),knockPaceEnd=new Date(knockPaceNow);knockPaceEnd.setHours(17,0,0,0);const knockExpected=(selectedDate===todayKey()&&scheduled)?(knockPaceNow>=knockPaceEnd?100:Math.min(100,Math.round(expectedKnockAt(kt,knockPaceNow)/Math.max(1,kt)*100))):0;
  const knockRing=$('#knockPercent');
  if(knockRing){
    knockRing.textContent=`${knockActual}%`;
...
function legacyTimelineItemsForDate(viewDate){
  ...
  items.push(...timelineTaskItemsForDate(viewDate));
  ...
  const order={task:0,followup:1,appointment:2,ofi:2,focus:3,knock:4,check:5,wrap:6};
...
function timelineItemsForDate(viewDate){
  ...
  const appointmentModel=dailyPlanAppointmentModel(viewDate),taskItems=timelineTaskItemsForDate(viewDate),fixedBusy=dailyPlanMergeIntervals([...appointmentModel.busy,...taskItems.map(item=>({start:item.minutes,end:item.minutes+item.duration}))]),knockTarget=rollingKnockTarget(viewDate),knockDone=Math.floor(liveKnockSeconds(dayData(viewDate))/60),knockRemaining=Math.max(0,knockTarget-knockDone),streets=knockingHotSpottingRecommendations(),knockDuration=knockRemaining||Math.max(15,knockTarget),knockSlots=dailyPlanFreeSlots(14*60,18*60,fixedBusy),knockWorkload={id:'knocking',workloadId:'knocking',kind:'knock',title:'Priority street',meta:`${knockRemaining} min remaining · ${streets.length} qualifying street${streets.length===1?'':'s'}`,kicker:`DOORKNOCKING · ${knockTarget} MIN TARGET`,duration:knockDuration,completed:false,actionable:true,action:'start-knocking',label:knockingSessionActive?'Resume knocking':'Start knocking'};
  let knockAllocation;
  if(!knockRemaining){
...
function sellerPriorityDeferralKey(){return`${storagePrefix(uid)}seller-priority-deferrals:v${SELLER_PRIORITY_DEFERRAL_VERSION}`}
function readSellerPriorityDeferrals(now=Date.now()){
  try{const rows=safeJsonParse(localStorage.getItem(sellerPriorityDeferralKey())||'[]',[]),cutoff=now-30*24*60*60*1000;return(Array.isArray(rows)?rows:[]).filter(row=>row&&cleanText(row.prospectId,80)&&Number(row.at)>=cutoff)}catch{return[]}
}
function writeSellerPriorityDeferrals(rows=[]){try{localStorage.setItem(sellerPriorityDeferralKey(),JSON.stringify(rows.slice(-120)))}catch(err){console.warn('Seller priority deferral could not be saved',err)}}
...
async function sellerPriorityBuildContext(now=new Date(),token=sellerPriorityBuildToken){
  const today=todayKey(),interactions=prospectInteractions.slice(),marketEvents=[...marketPulseHistory,...marketPulseEvents],interactionState=new Map(),deferralState=new Map(),marketByStreet=new Map(),eventsById=new Map();
  for(let index=0;index<interactions.length;index++){if(token!==sellerPriorityBuildToken)return null;const item=interactions[index],id=cleanText(item.prospectId,80);if(id){const state=interactionState.get(id)||{workedToday:false,doNotContact:false};if(item.date===today&&['Call','SMS','Appointment','Follow-up'].includes(item.type))state.workedToday=true;if(item.outcome==='Do not contact')state.doNotContact=true;interactionState.set(id,state)}if(index&&index%250===0)await sellerPriorityYield()}
  ...
  for(const event of normaliseMarketPulseEvents(marketEvents))eventsById.set(event.id,event);
...
function sellerPriorityCandidate(prospect={},now=new Date(),context={}){
  ...
  const sellerIntent=Boolean(timeframe||['Appraisal Opportunity','Appointment Booked','Pipeline','Past Client'].includes(stage)||prospect.temperature==='Hot'||overdueDays||dueToday||market);if(!sellerIntent)return null;
  const timeframeWeight={'Now':32,'1–3 months':24,'6–12 months':10,'12 months+':4}[timeframe]||0,temperatureWeight={Hot:24,Warm:12,Cold:2}[prospect.temperature]||0,stageWeight={'Appraisal Opportunity':28,'Appointment Booked':22,'Pipeline':12,'Past Client':7}[stage]||0,followUpWeight=overdueDays?34+Math.min(18,overdueDays*2):dueToday?30:prospect.nextFollowUp&&sellerPriorityDateDistance(prospect.nextFollowUp)<0&&sellerPriorityDateDistance(prospect.nextFollowUp)>=-3?8:0,marketWeight=market?Math.min(22,12+Math.round(market.priorityScore/8)):0,freshnessWeight=lastContactDays===null?10:lastContactDays>=14?12:lastContactDays>=7?7:lastContactDays<=1&&!dueToday?-12:0,score=Number(prospect.motivation||1)*4+timeframeWeight+temperatureWeight+stageWeight+followUpWeight+marketWeight+freshnessWeight+4-Math.min(18,(Number(deferral.count)||0)*6);
  const reasons=[];if(overdueDays)reasons.push(`${overdueDays} day${overdueDays===1?'':'s'} overdue`);else if(dueToday)reasons.push('Follow-up due today');if(prospect.temperature==='Hot')reasons.push('Hot seller');if(timeframe)reasons.push(`${timeframe} timeframe`);if(stage==='Appraisal Opportunity')reasons.push('Appraisal opportunity');if(market)reasons.push(`${market.event.eventType} activity nearby`);if(lastContactDays!==null&&lastContactDays>=7)reasons.push(`Last contact ${lastContactDays} days ago`);if(!reasons.length)reasons.push(`${prospect.motivation||1}/5 motivation`);
  const purpose=stage==='Appraisal Opportunity'?'appraisal follow-up':overdueDays||dueToday?'seller follow-up':market?'market conversation':'seller conversation';return{prospect,score,reasons:reasons.slice(0,3),purpose}
}
...
async function rebuildSellerPriorityCache(token){
  sellerPriorityBuilding=true;try{await sellerPriorityYield();if(token!==sellerPriorityBuildToken)return;const now=new Date(),context=await sellerPriorityBuildContext(now,token);if(!context||token!==sellerPriorityBuildToken)return;const candidates=activeProspects().filter(prospect=>prospect.sellingTimeframe||['Appraisal Opportunity','Appointment Booked','Pipeline','Past Client'].includes(prospect.stage)||prospect.temperature==='Hot'||prospect.nextFollowUp||context.marketByStreet.has(prospectMarketKey(prospect))).sort((a,b)=>sellerPriorityCandidatePreRank(b)-sellerPriorityCandidatePreRank(a)||String(a.id).localeCompare(String(b.id))).slice(0,SELLER_PRIORITY_MAX_CANDIDATES);let winner=null;
    for(let index=0;index<candidates.length;index++){if(token!==sellerPriorityBuildToken)return;const candidate=sellerPriorityCandidate(candidates[index],now,context);if(candidate&&candidate.score>=44&&(!winner||candidate.score>winner.score||candidate.score===winner.score&&sellerPriorityTieBreak(candidate.prospect,winner.prospect)<0))winner=candidate;if(index&&index%SELLER_PRIORITY_CHUNK_SIZE===0)await sellerPriorityYield()}
    if(token!==sellerPriorityBuildToken)return;sellerPriorityCache={ready:true,uid,day:todayKey(),value:winner?{title:`Call ${winner.prospect.name} — ${winner.purpose}`,meta:winner.reasons.join(' · '),kicker:'SELLER PRIORITY',timeLabel:'10 min',focusItemId:'',action:'open-seller',label:'Open seller',eventId:winner.prospect.id,sellerPriority:true,priorityRank:Math.max(-88,-40-Math.round(winner.score/2))}:null,expiresAt:Date.now()+5*60*1000};requestAnimationFrame(()=>{if(token!==sellerPriorityBuildToken||$('#app')?.classList.contains('hidden'))return;renderNowCard();if($('#scheduleView')?.classList.contains('active'))renderTimeline()})
  }catch(err){console.error('Seller priority background refresh failed',err);if(token===sellerPriorityBuildToken)sellerPriorityCache={...sellerPriorityCache,ready:true,value:validCachedSellerPriority(),expiresAt:Date.now()+30000}}
  finally{sellerPriorityBuilding=false;if(sellerPriorityRefreshRequested&&!sellerPriorityCache.ready)scheduleSellerPriorityRefresh(100)}
}
function sellerPriorityTieBreak(a={},b={}){return(a.nextFollowUp||'9999-12-31').localeCompare(b.nextFollowUp||'9999-12-31')||String(a.id).localeCompare(String(b.id))}
function validCachedSellerPriority(now=new Date()){
  const cached=sellerPriorityCache.value;
  if(!cached||sellerPriorityCache.uid!==uid||sellerPriorityCache.day!==todayKey())return null;
  const prospect=prospectById(cached.eventId);
  if(!prospect||prospect.archived||prospect.stage==='Listed'||!primaryProspectPhone(prospect))return null;
  if((Number(sellerPriorityImmediateDeferrals.get(prospect.id))||0)>now.getTime())return null;
  if(prospectInteractions.some(item=>item.prospectId===prospect.id&&(item.outcome==='Do not contact'||item.date===todayKey()&&['Call','SMS','Appointment','Follow-up'].includes(item.type))))return null;
  return cached;
}
function scheduleSellerPriorityRefresh(delay=180){
  if(sellerPriorityCache.ready)return;sellerPriorityRefreshRequested=true;
  if(sellerPriorityRefreshTimer||sellerPriorityBuilding)return;
  sellerPriorityRefreshTimer=setTimeout(()=>{sellerPriorityRefreshTimer=null;sellerPriorityRefreshRequested=false;const token=++sellerPriorityBuildToken;rebuildSellerPriorityCache(token)},Math.max(0,delay));
}
function invalidateSellerPriorityCache({schedule=true,delay=180}={}){
  const value=schedule?validCachedSellerPriority():null;
  sellerPriorityBuildToken++;sellerPriorityCache={ready:false,value,uid,day:todayKey(),expiresAt:0};sellerPriorityRefreshRequested=schedule;
  if(sellerPriorityRefreshTimer){clearTimeout(sellerPriorityRefreshTimer);sellerPriorityRefreshTimer=null}
  if(schedule)scheduleSellerPriorityRefresh(delay);
}
function sellerNextBestAction(now=new Date()){
  if(selectedDate!==todayKey())return null;
  if(sellerPriorityCache.ready&&Number(sellerPriorityCache.expiresAt)<=now.getTime())invalidateSellerPriorityCache({delay:0});
  if(!sellerPriorityCache.ready)scheduleSellerPriorityRefresh();
  return validCachedSellerPriority(now);
}
...
function offDayConversationLastContact(state={},prospect={}){
  const latest=state.latest?.get(prospect.id),date=cleanText(latest?.date||prospect.lastContact,20),result=cleanText(latest?.outcome||latest?.type,80);if(!date)return'No contact recorded';return`${fmtDate(date)}${result?` · ${result}`:''}`
}
function nextScheduledWorkdayKey(from=todayKey()){
  const date=parseKey(from);for(let step=1;step<=31;step++){date.setDate(date.getDate()+1);const key=dateKey(date);if(isWorkDayKey(key))return key}return''
}
function offDayNextWorkdayModel(){
  const key=nextScheduledWorkdayKey(),appointments=key?appointmentEntriesForDate(key):[],followUps=key?allFollowUpsForDate(key):[],tasks=key?taskEntriesForDate(key):[],commitments=[];
  appointments.forEach(entry=>commitments.push({at:appointmentTimestamp(entry.appointment,entry.sourceDate),label:`${appointmentTimeLabel(entry.appointment,entry.sourceDate)} · ${appointmentType(entry.appointment)}${entry.appointment.contactName||entry.appointment.name?` · ${entry.appointment.contactName||entry.appointment.name}`:''}`}));
  tasks.forEach(entry=>commitments.push({at:Number(entry.task.scheduledAt)||Number.MAX_SAFE_INTEGER,label:`${entry.task.time?timelineTimeLabel(timelineMinutes(entry.task.time)):'Time not set'} · ${entry.task.title}`}));
  commitments.sort((a,b)=>a.at-b.at);const total=appointments.length+followUps.length+tasks.length,parts=[appointments.length?`${appointments.length} appointment${appointments.length===1?'':'s'}`:'',followUps.length?`${followUps.length} follow-up${followUps.length===1?'':'s'}`:'',tasks.length?`${tasks.length} task${tasks.length===1?'':'s'}`:''].filter(Boolean),freshMarket=marketPulseAutomation.lastImportedDate===todayKey()?Math.max(0,Number(marketPulseAutomation.lastImportedNewCount)||0):0;
  return{key,title:key?fmtDate(key):'Next workday not set',first:commitments[0]?.label||'No timed commitment yet',workload:parts.length?parts.join(' · '):'No appointments, follow-ups or tasks planned yet',readiness:total?`${parseKey(key).toLocaleDateString('en-AU',{weekday:'long'})} is ready`:'Ready to plan',freshMarket}
}
function renderOffDayNextWorkday(){
  const model=offDayNextWorkdayModel();if($('#offDayNextWorkdayTitle'))$('#offDayNextWorkdayTitle').textContent=model.title;if($('#offDayFirstCommitment'))$('#offDayFirstCommitment').textContent=`First commitment · ${model.first}`;if($('#offDayWorkload'))$('#offDayWorkload').textContent=model.workload;if($('#offDayReadiness'))$('#offDayReadiness').textContent=model.readiness;const count=$('#offDayMarketPulseCount');if(count){count.textContent=String(model.freshMarket);count.classList.toggle('hidden',!model.freshMarket);count.setAttribute('aria-hidden',String(!model.freshMarket))}const button=$('#openMarketPulseOffDay');if(button)button.setAttribute('aria-label',model.freshMarket?`Open MarketPulse, ${model.freshMarket} new event${model.freshMarket===1?'':'s'}`:'Open MarketPulse')
}
function offDayConversationCandidates(limit=3){
  const today=todayKey(),now=new Date(),state=offDayConversationState(today),byContact=new Map(),add=candidate=>{if(!candidate?.id)return;const key=candidate.prospectId||candidate.id,current=byContact.get(key);if(!current||candidate.score>current.score)byContact.set(key,candidate)};
  for(const {appointment:a,sourceDate} of appointmentEntriesForDate(today)){
    if(appointmentOutcomeIsClosed(a.outcome))continue;const phone=cleanText(a.contactNumber||a.phone,80),dial=normaliseDialNumber(phone),prospect=prospects.find(item=>appointmentMatchesProspect(a,item));if(!dial||prospect&&state.contacted.has(prospect.id))continue;const at=appointmentTimestamp(a,sourceDate),minutesUntil=Math.round((at-now.getTime())/60000),name=cleanText(a.contactName||a.name,120)||'Appointment contact',type=appointmentType(a),timing=appointmentTimeLabel(a,sourceDate),sellerSignal=Boolean(prospect&&(sellerPriorityTimeframe(prospect)||prospect.sellerProfileActive||['Appraisal Opportunity','Appointment Booked','Pipeline'].includes(prospect.stage))),role=prospect?offDayConversationRole(prospect,sellerSignal):'Appointment';add({id:`appointment-${calendarExportId(a,sourceDate)}`,prospectId:prospect?.id||'',name,role,reason:`${type} appointment at ${timing} · Confirm the conversation and next step.`,lastContact:prospect?offDayConversationLastContact(state,prospect):'No contact recorded',nextContact:`Today · ${timing} ${type} appointment`,score:minutesUntil>=-90?360-Math.max(0,Math.min(180,minutesUntil)):170,action:'view-appointments',eventId:'',phone:dial,appointmentId:calendarExportId(a,sourceDate),sourceDate})
  }
  for(const envelope of buyerMatchContactEnvelopes(today).slice(0,20)){
    const buyer=envelope.buyer;if(state.contacted.has(buyer.id)||state.doNotContact.has(buyer.id)||!envelope.phone||validDateKey(buyer.nextFollowUp)&&buyer.nextFollowUp>today)continue;const alert=envelope.urgency,label=alert?.label||envelope.followUpLabel||'Property match ready',property=[envelope.primary.address,envelope.primary.suburb].filter(Boolean).join(', ');add({id:`buyer-match-${buyer.id}`,prospectId:buyer.id,name:buyer.name,role:prospectIsBuyerSeller(buyer)?'Buyer + Seller':'Buyer',reason:`${label}${property?` · ${property}`:''}.`,lastContact:offDayConversationLastContact(state,buyer),nextContact:`Today · ${label}`,score:250+Number(envelope.priorityScore||0),action:'open-buyer',eventId:buyer.id,phone:envelope.phone,buyerMatchId:envelope.primary.id})
  }
  const candidatePool=prospects.filter(prospect=>!prospect.archived&&(prospectHasContactProfile(prospect)||prospectHasActiveBuyerRole(prospect))&&primaryProspectPhone(prospect));
  for(const prospect of candidatePool){
    if(state.contacted.has(prospect.id)||state.doNotContact.has(prospect.id)||validDateKey(prospect.nextFollowUp)&&prospect.nextFollowUp>today)continue;const followDistance=prospect.nextFollowUp?sellerPriorityDateDistance(prospect.nextFollowUp,today):null,overdueDays=followDistance===null?0:Math.max(0,followDistance),dueToday=prospect.nextFollowUp===today,timeframe=sellerPriorityTimeframe(prospect),sellerSignal=Boolean(timeframe||prospect.sellerProfileActive||['Appraisal Opportunity','Appointment Booked','Pipeline'].includes(prospect.stage)),buyerSignal=prospectHasActiveBuyerRole(prospect),lastAt=Number(state.lastAt.get(prospect.id)||0),daysSince=lastAt?Math.max(0,Math.floor((now.getTime()-lastAt)/86400000)):null,pipelineReady=!prospect.nextFollowUp&&(daysSince===null||daysSince>=21);let score=0,reason='';
    if(overdueDays){score=225+Math.min(30,overdueDays);reason=`Follow-up overdue by ${overdueDays} day${overdueDays===1?'':'s'}.`}
    else if(dueToday){score=220;reason='Follow-up is due today.'}
    else if(sellerSignal){score=145+({Hot:28,Warm:12,Cold:0}[prospect.temperature]||0)+({'Now':34,'1–3 months':24,'6–12 months':10,'12 months+':4}[timeframe]||0);reason=prospect.temperature==='Hot'?`Hot seller${timeframe?` · ${timeframe} timeframe`:''}.`:timeframe?`${timeframe} selling timeframe · no contact today.`:'Seller opportunity ready for a clear next step.'}
    else if(buyerSignal){score=105+({Hot:24,Warm:10,Cold:0}[prospect.temperature]||0);reason=`Active ${String(prospect.buyerStage||'buyer').toLowerCase()} buyer · no contact today.`}
    else if(pipelineReady){score=65+(prospect.temperature==='Hot'?20:prospect.temperature==='Warm'?8:0);reason=daysSince===null?'No conversation has been logged yet.':`Last conversation ${daysSince} days ago.`}
    if(score<60)continue;const role=offDayConversationRole(prospect,sellerSignal);add({id:`contact-${prospect.id}`,prospectId:prospect.id,name:prospect.name,role,reason,lastContact:offDayConversationLastContact(state,prospect),nextContact:`Today · ${reason.replace(/\.$/,'')}`,score,action:buyerSignal&&!sellerSignal?'open-buyer':'open-seller',eventId:prospect.id,phone:primaryProspectPhone(prospect)})
  }
  return[...byContact.values()].sort((a,b)=>b.score-a.score||a.name.localeCompare(b.name,'en-AU',{sensitivity:'base'})).slice(0,Math.max(1,limit))
}
function offDayConversationCallMarkup(item={}){
  if(item.appointmentId)return`<a href="tel:${escapeHtml(item.phone)}" data-appointment-followup-call="${escapeHtml(item.appointmentId)}" data-source-date="${escapeHtml(item.sourceDate||todayKey())}" aria-label="Call ${escapeHtml(item.name)}">Call</a>`;
  if(item.buyerMatchId)return`<button type="button" data-buyer-match-call="${escapeHtml(item.prospectId)}" data-match-id="${escapeHtml(item.buyerMatchId)}" aria-label="Call ${escapeHtml(item.name)}">Call</button>`;
  return`<a href="${escapeHtml(prospectTel(prospectById(item.prospectId)))}" data-prospect-call="${escapeHtml(item.prospectId)}" data-call-from-session="0" aria-label="Call ${escapeHtml(item.name)}">Call</a>`
}
function offDayConversationSmsMarkup(item={}){
  if(item.buyerMatchId)return`<button type="button" data-buyer-match-sms="${escapeHtml(item.prospectId)}" data-match-id="${escapeHtml(item.buyerMatchId)}" aria-label="SMS ${escapeHtml(item.name)}">SMS</button>`;
  return`<a href="sms:${escapeHtml(normaliseDialNumber(item.phone))}" aria-label="SMS ${escapeHtml(item.name)}">SMS</a>`
}
function offDayConversationMoveMarkup(item={}){
  if(item.appointmentId)return`<button type="button" data-off-day-move-appointment="${escapeHtml(item.appointmentId)}" aria-label="Move ${escapeHtml(item.name)}">Move</button>`;
  return`<button type="button" data-off-day-move-contact="${escapeHtml(item.prospectId)}" aria-label="Move ${escapeHtml(item.name)}">Move</button>`
}
function renderOffDayConversations(){
  const host=$('#offDayConversationList');if(!host)return;const items=offDayConversationCandidates(3);if(!items.length){host.innerHTML='<div class="off-day-conversation-empty"><strong>No urgent conversations</strong><small>Your contacted, deferred and completed work is already clear.</small></div>';return}
  host.innerHTML=items.map(item=>`<article class="buyer-card off-day-conversation-card"><button class="buyer-row-profile" type="button" data-plan-action="${escapeHtml(item.action)}" data-event-id="${escapeHtml(item.eventId||'')}"><span class="buyer-row-head"><span class="buyer-row-identity"><strong>${escapeHtml(item.name)}</strong></span><em class="off-day-conversation-role">${escapeHtml(item.role)}</em></span><span class="buyer-row-brief">Last contact · ${escapeHtml(item.lastContact||'No contact recorded')}</span><span class="buyer-row-location">Next contact · ${escapeHtml(item.nextContact||item.reason)}</span></button><div class="buyer-card-actions off-day-conversation-actions">${offDayConversationCallMarkup(item)}${offDayConversationSmsMarkup(item)}${offDayConversationMoveMarkup(item)}</div></article>`).join('')
}
function renderOffDayHome(){renderOffDayNextWorkday();renderOffDayConversations()}
function openOffDayQuickAppointment(){
  editingAppointment=null;appointmentEditReturnState=null;appointmentHistoryMode=null;appointmentQuickReturnHome=false;appointmentDate=todayKey();appointmentLinkedProspectId='';pendingProspectAppointmentFlow=null;$('#appointmentForm')?.reset();$('#appointmentDatePicker').value=appointmentDate;$('#appointmentTime').value='12:00';$('#appointmentAuction').checked=false;updateOfiFormState();switchView('appointmentsView');requestAnimationFrame(()=>$('#appointmentContactName')?.focus({preventScroll:true}))
}
function openWeeklyAppointmentsQuickView(){appointmentQuickReturnHome=true;appointmentHistoryMode='week';switchView('appointmentsView');setAppointmentHistoryScreen('week')}
function openOffDayBroadcast(){homeQuickProspectorReturn=true;selectedBroadcastType='';setProspectorSection('broadcast');switchView('prospectingView')}
function runOffDayQuickAction(action=''){
  if(action==='call'){openManualDialler();return}if(action==='appointment'){openOffDayQuickAppointment();return}if(action==='task'){openTaskComposer();return}if(action==='calendar'){openWeeklyAppointmentsQuickView();return}if(action==='broadcast')openOffDayBroadcast()
}
function openOffDayContactMove(id){
  const prospect=prospectById(id);if(!prospect)return toast('Contact could not be found');const tomorrow=new Date();tomorrow.setDate(tomorrow.getDate()+1);const minimum=dateKey(tomorrow),returnFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;closeBuyerFollowUp({restoreFocus:false});buyerFollowUpReturnFocus=returnFocus;
  const overlay=document.createElement('div');overlay.className='buyer-followup-overlay';overlay.innerHTML=`<form id="offDayMoveForm" class="buyer-followup-sheet" data-prospect-id="${escapeHtml(prospect.id)}" role="dialog" aria-modal="true" aria-labelledby="offDayMoveTitle"><div class="buyer-followup-head"><div><span>MOVE</span><h2 id="offDayMoveTitle">${escapeHtml(prospect.name)}</h2></div><button type="button" data-close-buyer-followup aria-label="Close">×</button></div><label>Next contact date<input name="followUpDate" type="date" value="${validDateKey(prospect.nextFollowUp)&&prospect.nextFollowUp>=minimum?prospect.nextFollowUp:minimum}" min="${minimum}" required></label><div class="buyer-followup-actions"><button class="secondary" type="button" data-close-buyer-followup>Cancel</button><button class="primary" type="submit">Move follow-up</button></div></form>`;document.body.append(overlay);document.body.classList.add('buyer-followup-open');overlay.addEventListener('click',event=>{if(event.target===overlay||event.target.closest('[data-close-buyer-followup]'))closeBuyerFollowUp()});overlay.querySelector('form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,date=String(new FormData(form).get('followUpDate')||''),submit=form.querySelector('button[type="submit"]');if(!validDateKey(date)||date<minimum)return toast('Choose a future date');submit.disabled=true;submit.textContent='Moving…';const at=Date.now();prospectInteractions.push({id:prospectId(),prospectId:prospect.id,date:todayKey(),at,type:'Follow-up',outcome:'Follow-up rescheduled',note:'Moved from the day-off conversation list.',nextFollowUp:date});prospects=prospects.map(item=>item.id===prospect.id?normaliseProspect({...item,nextFollowUp:date,updatedAt:at}):item);closeBuyerFollowUp({restoreFocus:false});renderOffDayHome();renderTimeline();renderNowCard();toast(`Moved to ${fmtDate(date)}`);try{await saveProspecting({render:false,awaitCloud:false})}catch(err){console.error('Follow-up move sync failed',err);toast('Moved locally. Cloud sync will retry.')}});requestAnimationFrame(()=>overlay.querySelector('input')?.focus({preventScroll:true}))
}
...
function dailyCommandPriority(viewDate=selectedDate,items=timelineItemsForDate(viewDate),now=new Date()){
  if(!isWorkDayKey(viewDate)){const conversation=viewDate===todayKey()?offDayConversationCandidates(1)[0]:null;return conversation?{title:`Call ${conversation.name}`,meta:conversation.reason,kicker:'NEXT CONVERSATION',timeLabel:'',focusItemId:'',action:conversation.action,label:'Open profile',eventId:conversation.eventId||''}:{title:'No tracking day scheduled',meta:'No urgent conversations are waiting.',kicker:'TODAY',timeLabel:'',focusItemId:'',action:'',label:'',eventId:''}}
  ...
  if(nowMinutes<18*60+30&&!sellerPriority&&!sellerPriorityCache.ready&&(!current||dailyCommandItemRank(current)>-88))return{title:'Loading today…',meta:'',kicker:'RIGHT NOW',timeLabel:'',focusItemId:'',action:'',label:'Open today timeline',eventId:''};
...
function renderNowCard(){
  const priority=timelinePriority(selectedDate),card=$('#openTodayTimeline');
  if(card){const action=priority.action||'',eventId=priority.eventId||'',label=priority.label||'Open today timeline';if(card.dataset.planAction!==action)card.dataset.planAction=priority.action||'';if(card.dataset.eventId!==eventId)card.dataset.eventId=priority.eventId||'';if(card.getAttribute('aria-label')!==label)card.setAttribute('aria-label',label)}
  for(const [selector,text]of [['#nowCardLabel',priority.kicker||'RIGHT NOW'],['#nowCardTitle',priority.title],['#nowCardMeta',[priority.timeLabel,priority.meta].filter(Boolean).join(' · ')]]){const node=$(selector);if(node&&node.textContent!==text)node.textContent=text}
}
...
function renderTimeline(){
  ...
  const sellerDefer=$('#timelineDeferSeller');if(sellerDefer){sellerDefer.hidden=!priority.sellerPriority;sellerDefer.dataset.prospectId=priority.sellerPriority?priority.eventId:''}
...
function appointmentHistoryEntries(mode){
  ...
  const start=new Date();start.setHours(0,0,0,0);start.setDate(start.getDate()-((start.getDay()+6)%7));const weekEnd=new Date(start);weekEnd.setDate(weekEnd.getDate()+7);
  const entries=[...allAppointmentEntries().map(entry=>({...entry,isTeamAssigned:false})),...shared].filter(({appointment:a,sourceDate})=>{if(mode==='past'&&isOfiAppointment(a))return false;const scheduledAt=appointmentTimestamp(a,sourceDate);if(!scheduledAt)return mode==='past';if(mode==='week')return scheduledAt>now&&scheduledAt<weekEnd.getTime();return mode==='past'?scheduledAt<=now:scheduledAt>now;});
...
function setAppointmentHistoryScreen(mode){
  ...
  if(mode){$('#appointmentHistoryTitle').textContent=mode==='past'?'Past Appointments':mode==='week'?'This Week':'Upcoming Appointments';window.scrollTo({top:0,behavior:'auto'});}
  renderAppointments();
}
...
function renderTaskAssigneePicker(preferredUid=uid){
  const select=$('#taskAssignee');if(!select)return;
  const available=accountMode==='team'&&teamId&&appointmentAssignees.length>1,sorted=available?[...appointmentAssignees].sort((a,b)=>String(a.uid)===uid?-1:String(b.uid)===uid?1:teamAppointmentBookingDisplayName(a).localeCompare(teamAppointmentBookingDisplayName(b))):[{uid,name:displayAgentName()}];
  select.innerHTML=sorted.map(member=>`<option value="${escapeHtml(member.uid)}">${escapeHtml(String(member.uid)===uid?'Me':teamAppointmentBookingDisplayName(member))}</option>`).join('');select.value=sorted.some(member=>String(member.uid)===String(preferredUid))?String(preferredUid):uid;
  $('#taskAssigneeField')?.classList.toggle('hidden',!available);
}
...
function taskEntriesForDate(viewDate){
  const entries=[];Object.entries(days).forEach(([sourceDate,raw])=>(raw?.tasks||[]).forEach(task=>{const normal=normaliseTaskRecord(task,sourceDate);if(normal.scheduledDate===viewDate)entries.push({task:normal,sourceDate,isTeamAssigned:false})}));
  assignedTeamTasks.forEach(task=>{if(task.scheduledDate===viewDate)entries.push({task,sourceDate:task.createdDate||viewDate,isTeamAssigned:true})});
  const unique=new Map();entries.forEach(entry=>unique.set(entry.task.id,entry));return[...unique.values()].sort((a,b)=>a.task.scheduledAt-b.task.scheduledAt||a.task.title.localeCompare(b.task.title));
}
function timelineTaskItemsForDate(viewDate){return taskEntriesForDate(viewDate).map(({task,sourceDate,isTeamAssigned})=>{const assignedElsewhere=task.assignedToUid&&task.assignedToUid!==uid,ownership=assignedElsewhere?`Assigned to ${task.assignedToName||'team member'}`:task.setterUid&&task.setterUid!==uid?`Assigned by ${teamAppointmentSetterFirstName(task.setterName)}`:'My task',meta=[task.note,ownership].filter(Boolean).join(' · ');return{id:`task-${task.id}`,minutes:timelineMinutes(task.time),duration:15,title:task.title,meta,kicker:'TASK',kind:'task',plan:true,clockComplete:Boolean(task.completedAt),completed:Boolean(task.completedAt),actionable:!task.completedAt,action:'',label:'',commandTitle:task.title,commandMeta:meta||'Personal task',task,sourceDate,isTeamAssigned}})}
function localTaskEntry(id){for(const [sourceDate,raw]of Object.entries(days)){const task=(raw?.tasks||[]).find(item=>String(item.id)===String(id));if(task)return{task:normaliseTaskRecord(task,sourceDate),sourceDate}}return null}
...
async function toggleTimelineTask(id){
  ...
  if(team||updated.assignedToUid!==uid){assignedTeamTasks=assignedTeamTasks.map(task=>String(task.id)===String(id)?{...task,completedAt}:task);if(cloud&&db&&teamId)try{await setDoc(doc(db,'teams',teamId,'tasks',String(id)),{completedAt:completedAt?serverTimestamp():null,updatedAt:serverTimestamp()},{merge:true})}catch(err){console.error('Team task completion failed',err);toast('Task updated locally. Team sync needs attention.')}}
  renderTimeline();haptic();toast(completedAt?'Task completed':'Task reopened');
}

function stopTeamAppointmentLayer(){unsubAppointmentAssignees?.();unsubAssignedTeamAppointments?.();unsubAssignedTeamTasks?.();unsubAppointmentAssignees=unsubAssignedTeamAppointments=unsubAssignedTeamTasks=null;subscribedAppointmentTeamId='';appointmentAssignees=[];assignedTeamAppointments=[];assignedTeamTasks=[];pendingTeamAppointmentNotice=null;hideTeamAppointmentNotice({acknowledge:false});renderAppointmentAssigneePicker();renderAppointments();renderTimeline()}
...
function renderLeaderboardPosition(){
  ...
  if(!cloud){position.textContent='—';meta.textContent='— • —';return;}
  const rows=sortedTodayLeaderboard(),index=rows.findIndex(r=>r.uid===uid);
  if(index<0){position.textContent='—';meta.textContent=`— • ${rows.length||'—'}`;return;}
  const me=rows[index];
  position.textContent=`#${index+1}`;
  meta.textContent=`${index+1} • ${rows.length}`;
}
...
function renderScorecardAppointments(entries,base){
  ...
  list.innerHTML=entries.length?entries.map(({appointment:a,sourceDate,scheduled})=>{
    ...
    const tel=phone?`<a class="scorecard-call" href="tel:${escapeHtml(phone.replace(/[^+\d]/g,''))}" data-appointment-followup-call="${escapeHtml(calendarExportId(a,sourceDate))}" data-source-date="${escapeHtml(sourceDate)}">Call ${escapeHtml(contact.split(/\s+/)[0]||'contact')}</a>`:'';
...
function buyerSessionBackupStorageKey(){return`${buyerSessionStorageKey()}-backup`}
function loadBuyerSession(){buyerSession={contacts:[],index:0,active:false,visible:false,fileName:'',importedAt:0};try{const primary=safeJsonParse(localStorage.getItem(buyerSessionStorageKey())||'null',null),backup=safeJsonParse(localStorage.getItem(buyerSessionBackupStorageKey())||'null',null),raw=primary&&Array.isArray(primary.contacts)?primary:backup;if(raw&&Array.isArray(raw.contacts)){buyerSession={contacts:raw.contacts.map((c,i)=>({id:cleanText(c.id,80)||`buyer_${i}`,name:cleanText(c.name,120)||'Unknown buyer',phone:normaliseDialNumber(c.phone),address:cleanText(c.address,240),doNotSms:Boolean(c.doNotSms),status:cleanText(c.status,40)})).filter(c=>c.phone),index:Math.max(0,Number(raw.index)||0),active:Boolean(raw.active),visible:Boolean(raw.visible),fileName:cleanText(raw.fileName,160),importedAt:Number(raw.importedAt)||0};const payload=JSON.stringify(buyerSession);if(raw===backup)localStorage.setItem(buyerSessionStorageKey(),payload);if(!backup)localStorage.setItem(buyerSessionBackupStorageKey(),payload)}}catch(err){console.warn('Buyer session could not be restored',err)}}
function saveBuyerSession(){const payload=JSON.stringify(buyerSession);try{localStorage.setItem(buyerSessionStorageKey(),payload);localStorage.setItem(buyerSessionBackupStorageKey(),payload);if(localStorage.getItem(buyerSessionStorageKey())!==payload)throw new Error('Buyer session verification failed');return true}catch(err){console.warn('Buyer session could not be saved',err);return false}}
function clearBuyerSession(){buyerSession={contacts:[],index:0,active:false,visible:false,fileName:'',importedAt:0};try{localStorage.removeItem(buyerSessionStorageKey());localStorage.removeItem(buyerSessionBackupStorageKey())}catch{}renderBuyerSessionHero()}
...
function launchBuyerSessionCall(){const buyer=buyerSession.contacts[buyerSession.index];if(!buyer)return;const number=normaliseDialNumber(buyer.phone),pending={id:prospectId(),number,launchedAt:Date.now(),outcomeLogged:false,source:'buyer-session',buyerId:buyer.id,buyerName:buyer.name};writePendingManualCall(pending);showManualCallOutcome();window.location.href=`tel:${number}`}
...
function vCardNameParts(name=''){
  const parts=cleanText(name,120).split(/\s+/).filter(Boolean);if(parts.length<2)return{given:parts[0]||'Contact',family:''};return{given:parts.slice(0,-1).join(' '),family:parts.at(-1)}
}
function contactVCardAddress(p={}){
  let street=cleanText(p.address,240),locality=cleanText(p.suburb,100),region='',postcode='';
  const legacyCompany=cleanText(p.company,240);if(!street&&/\d/.test(legacyCompany)&&/\b(?:street|st|road|rd|avenue|ave|drive|dr|place|pl|court|ct|close|crescent|cres|boulevard|blvd|lane|ln|way|parade|pde)\b/i.test(legacyCompany))street=legacyCompany;
  const statePostcode=street.match(/(?:,\s*|\s+)(NSW|VIC|QLD|SA|WA|TAS|NT|ACT)\s+(\d{4})$/i);if(statePostcode){region=statePostcode[1].toUpperCase();postcode=statePostcode[2];street=street.slice(0,statePostcode.index).trim().replace(/,$/,'')}
  if(locality){const escaped=locality.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),tail=new RegExp(`(?:,\\s*|\\s+)${escaped}$`,'i');street=street.replace(tail,'').trim().replace(/,$/,'')}
  else{const segments=street.split(',').map(value=>value.trim()).filter(Boolean);if(segments.length>1&&!/\d/.test(segments.at(-1))){locality=segments.pop();street=segments.join(', ')}}
  return{street,locality,region,postcode};
}
function contactVCardOrganisation(p={}){
  const company=cleanText(p.company,240),address=cleanText(p.address,240);if(!company||company.toLowerCase()===address.toLowerCase())return'';if(/^\d/.test(company)&&/\b(?:street|st|road|rd|avenue|ave|drive|dr|place|pl|court|ct|close|crescent|cres|boulevard|blvd|lane|ln|way|parade|pde)\b/i.test(company))return'';return company
}
function contactVCardNotes(p={}){
  const lines=[],push=(label,value)=>{const clean=cleanText(value,3000);if(clean)lines.push(`${label}: ${clean}`)};
  push('AGNT stage',p.stage);push('Temperature',p.temperature);if(Number(p.motivation))push('Motivation',`${p.motivation}/5`);push('Selling timeframe',pipelineTimeframeForProspect(p));push('Next follow-up',p.nextFollowUp);push('Source',p.source);if(p.tags?.length)push('Tags',p.tags.join(', '));
  if(prospectHasBuyerProfile(p)){push('Buyer stage',p.buyerStage);if(Number(p.buyerBudgetMax))push('Buyer budget',buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax));const config=buyerConfigText(p);if(config&&config!=='Configuration not set')push('Buyer configuration',config);if(p.buyerSuburbs?.length)push('Buyer suburbs',p.buyerSuburbs.join(', '));push('Buyer property type',p.buyerPropertyType);if(p.buyerFeatures?.length)push('Buyer must-haves',p.buyerFeatures.join(', '));const position=buyerPositionTags(p);if(position.length)push('Buyer position',position.join(', '));}
  push('AGNT notes',p.notes);return lines.join('\n')
}
function buildContactVCard(p={}){
  const name=cleanText(p.name,120)||'Contact',parts=vCardNameParts(name),phone=primaryProspectPhone(p),email=cleanText(p.email,180),organisation=contactVCardOrganisation(p),address=contactVCardAddress(p),notes=contactVCardNotes(p),lines=['BEGIN:VCARD','VERSION:3.0',`N:${vCardEscape(parts.family)};${vCardEscape(parts.given)};;;`,`FN:${vCardEscape(name)}`];
...
function primeDirectCallOutcome(link){
  if(!link||link.classList.contains('disabled'))return;
  const appointmentId=link.dataset.appointmentFollowupCall,contactId=link.dataset.prospectCall;
  if(appointmentId){rememberAppointmentFollowUpCallReturn(appointmentId,link.dataset.sourceDate||selectedDate,link.dataset.callReturnMode||'');return}
  if(contactId){rememberProspectCallReturn(contactId,link.dataset.callFromSession==='1',link.dataset.callReturnMode||'');return}
  if(link.matches('[data-timeline-buyer-call]'))return;
  const number=normaliseDialNumber(decodeURIComponent(String(link.getAttribute('href')||'').replace(/^tel:/i,'')));
  if(number.replace(/\D/g,'').length<6)return;
  writePendingManualCall({id:prospectId(),number,launchedAt:Date.now(),outcomeLogged:false,source:'direct-call'});showManualCallOutcome();
}
...
function sellerPropertyConfigured(p={}){return Boolean(p.sellerPropertyType||p.sellerBedrooms||p.sellerBathrooms||p.sellerCars||p.sellerLandSizeBand||p.sellerTitleType)}
function sellerLandSizeBandFromValue(value=0){const size=Number(value)||0;if(!size)return'';if(size<400)return'under-400';if(size<600)return'400-599';if(size<1000)return'600-999';return'1000-plus'}
function sellerEventPropertyConfiguration(event={}){
  const details=cleanText(event.propertyDetails,240),lower=normalisePlace(details),buyerConfig=buyerMarketPropertyConfiguration(event),category=marketPulseEventPropertyCategory(event);let propertyType='';
  ...
  else if(/\bvilla\b/.test(lower))propertyType='Villa';
  else if(/\b(?:unit|apartment|flat)\b/.test(lower)||category==='Strata')propertyType='Unit';
  else if(/\b(?:vacant land|land only)\b/.test(lower))propertyType='Land';
  else if(/\bhouse\b/.test(lower)||category==='House')propertyType='House';
  const landMatch=details.match(/(\d{2,5}(?:\.\d+)?)\s*(?:m2|m²|sqm|square metres?)/i),titleType=/\bcommunity title\b/i.test(details)?'Community title':/\bstrata title\b/i.test(details)?'Strata title':/\btorrens title\b/i.test(details)?'Torrens title':'';
  return{propertyType:propertyType||buyerConfig.propertyType,bedrooms:buyerConfig.bedrooms,bathrooms:buyerConfig.bathrooms,cars:buyerConfig.cars,landSizeBand:sellerLandSizeBandFromValue(landMatch?.[1]),titleType};
}
function sellerConfigurationLabel(p={}){const parts=[];if(p.sellerBedrooms)parts.push(`${p.sellerBedrooms} bed`);if(p.sellerBathrooms)parts.push(`${p.sellerBathrooms} bath`);if(p.sellerCars)parts.push(`${p.sellerCars} car`);if(p.sellerPropertyType)parts.push(p.sellerPropertyType);if(p.sellerLandSizeBand)parts.push(SELLER_LAND_SIZE_LABELS[p.sellerLandSizeBand]);if(p.sellerTitleType)parts.push(p.sellerTitleType);return parts.join(' · ')}
function sellerMarketSimilarity(p={},event={}){
  if(!sellerPropertyConfigured(p))return{score:0,ratio:0,evidence:0,close:false};const config=sellerEventPropertyConfiguration(event);let available=0,score=0,evidence=0;
  const exact=(sellerValue,eventValue,weight)=>{if(!sellerValue||!eventValue)return;available+=weight;evidence++;if(sellerValue===eventValue)score+=weight};
  exact(p.sellerPropertyType,config.propertyType,40);
  const numeric=(sellerValue,eventValue,weight)=>{sellerValue=Number(sellerValue)||0;eventValue=Number(eventValue)||0;if(!sellerValue||!eventValue)return;available+=weight;evidence++;const difference=Math.abs(sellerValue-eventValue);if(!difference)score+=weight;else if(difference===1)score+=Math.round(weight*.45)};
  numeric(p.sellerBedrooms,config.bedrooms,22);numeric(p.sellerBathrooms,config.bathrooms,14);numeric(p.sellerCars,config.cars,12);
  exact(p.sellerLandSizeBand,config.landSizeBand,8);exact(p.sellerTitleType,config.titleType,4);
  const ratio=available?score/available:0,propertyConflict=Boolean(p.sellerPropertyType&&config.propertyType&&p.sellerPropertyType!==config.propertyType);return{score,ratio,evidence,close:!propertyConflict&&evidence>0&&ratio>=.75,config};
}
...
function sellerEstimateComparableEligible(p={},event={},similarity=sellerMarketSimilarity(p,event)){
  const config=similarity?.config||sellerEventPropertyConfiguration(event),propertyType=cleanText(p.sellerPropertyType,40),bedrooms=Number(p.sellerBedrooms)||0;
  if(!propertyType||!bedrooms||config.propertyType!==propertyType||Number(config.bedrooms)!==bedrooms)return false;
  const withinOne=(sellerValue,eventValue)=>{sellerValue=Number(sellerValue)||0;eventValue=Number(eventValue)||0;return!sellerValue||!eventValue||Math.abs(sellerValue-eventValue)<=1};
...
function mergeParsedMarketPulseEvents(parsed=[]){
  ...
  invalidateSellerPriorityCache({delay:80});
...
function sellerChoiceMarkup(target,current,values,labels={}){return`<input type="hidden" name="${target}" value="${escapeHtml(String(current||0))}"><div class="buyer-segment seller-segment">${values.map(value=>`<button type="button" class="${String(current||0)===String(value)?'active':''}" data-seller-choice="${target}" data-seller-choice-value="${value}" aria-pressed="${String(current||0)===String(value)}">${escapeHtml(labels[value]||String(value))}</button>`).join('')}</div>`}
function sellerToggleMarkup(target,current,values,labels={}){return`<input type="hidden" name="${target}" value="${escapeHtml(current||'')}"><div class="buyer-option-tags seller-option-tags">${values.map(value=>`<button type="button" class="${current===value?'active':''}" data-seller-toggle="${target}" data-seller-toggle-value="${escapeHtml(value)}" aria-pressed="${current===value}">${escapeHtml(labels[value]||value)}</button>`).join('')}</div>`}
function sellerPropertyEditorMarkup(current={}){const showLand=current.sellerPropertyType==='House',hideConfig=current.sellerPropertyType==='Land';return`<section class="buyer-editor-section seller-editor-property"><div class="buyer-editor-section-head"><span>PROPERTY DETAILS</span><strong>Optional</strong></div><p class="seller-property-help">Add what you know to sharpen MarketPulse priority and sold estimates.</p><div class="buyer-config-grid seller-config-fields ${hideConfig?'hidden':''}" data-seller-config-fields><div><small>Bedrooms</small>${sellerChoiceMarkup('sellerBedrooms',current.sellerBedrooms||0,[0,1,2,3,4,5],{0:'—',5:'5+'})}</div><div><small>Bathrooms</small>${sellerChoiceMarkup('sellerBathrooms',current.sellerBathrooms||0,[0,1,2,3,4],{0:'—',4:'4+'})}</div><div><small>Cars</small>${sellerChoiceMarkup('sellerCars',current.sellerCars||0,[0,1,2,3],{0:'—',3:'3+'})}</div></div><span class="buyer-field-label seller-field-label">PROPERTY TYPE</span>${sellerToggleMarkup('sellerPropertyType',current.sellerPropertyType||'',SELLER_PROPERTY_TYPES)}<div class="seller-land-fields ${showLand?'':'hidden'}" data-seller-land-fields><span class="buyer-field-label seller-field-label">LAND SIZE</span>${sellerToggleMarkup('sellerLandSizeBand',current.sellerLandSizeBand||'',SELLER_LAND_SIZE_BANDS,SELLER_LAND_SIZE_LABELS)}</div><span class="buyer-field-label seller-field-label">TITLE</span>${sellerToggleMarkup('sellerTitleType',current.sellerTitleType||'',SELLER_TITLE_TYPES)}</section>`}
function syncSellerPropertyEditor(form,resetHidden=false){const type=form?.elements.namedItem('sellerPropertyType')?.value||'',config=form?.querySelector('[data-seller-config-fields]'),land=form?.querySelector('[data-seller-land-fields]');config?.classList.toggle('hidden',type==='Land');land?.classList.toggle('hidden',type!=='House');if(!resetHidden)return;if(type==='Land'){for(const name of ['sellerBedrooms','sellerBathrooms','sellerCars']){const input=form.elements.namedItem(name);if(input)input.value='0';form.querySelectorAll(`[data-seller-choice="${name}"]`).forEach(button=>{const active=button.dataset.sellerChoiceValue==='0';button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))})}}if(type!=='House'){const input=form.elements.namedItem('sellerLandSizeBand');if(input)input.value='';form.querySelectorAll('[data-seller-toggle="sellerLandSizeBand"]').forEach(button=>{button.classList.remove('active');button.setAttribute('aria-pressed','false')})}}
function sellerPropertyEditorForm(p={}){return`<form id="sellerPropertyEditor" class="buyer-editor glass seller-property-editor-form" data-prospect-id="${escapeHtml(p.id)}"><div class="prospect-detail-nav buyer-detail-nav"><button type="button" data-close-seller-property>‹ Back</button><strong>Property Details</strong><span></span></div><div class="buyer-editor-intro"><span>PROPERTY PROFILE</span><h2>${escapeHtml(p.name)}</h2><p>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb)||'Add the property configuration you know.')} These details improve MarketPulse matching across AGNT.</p></div>${sellerPropertyEditorMarkup(p)}${sellerEstimateMarkup(p)}<button class="primary buyer-save-button" type="submit">Save Property Details</button></form>`}
function openSellerPropertyEditor(id=''){const p=prospectById(id);if(!p)return toast('Contact could not be found');activeProspectId=p.id;$('#prospectingDashboard')?.classList.add('hidden');$('#prospectingSession')?.classList.add('hidden');$('#prospectDetail')?.classList.remove('hidden');$('#prospectDetail').innerHTML=sellerPropertyEditorForm(p)}
...
function launchBuyerProfileCall(id,buyerMatchId=''){const p=prospectById(id),number=normaliseDialNumber(primaryProspectPhone(p));if(!prospectHasActiveBuyerRole(p)||number.replace(/\D/g,'').length<6)return toast('Add a valid mobile number first');const pending={id:prospectId(),number,launchedAt:Date.now(),outcomeLogged:false,source:'buyer-profile',buyerProspectId:p.id,buyerName:p.name,buyerMatchId:cleanText(buyerMatchId,180)};writePendingManualCall(pending);showManualCallOutcome();window.location.href=`tel:${number}`}
...
function renderProspecting(){renderBuyerSessionHero();
  ...
  }else if(buyerSession.active&&sessionWasOpen&&session?.dataset.sessionKind==='buyer'){
    showBuyerSession();
  }else if(prospectSessionActive&&session?.dataset.sessionKind!=='buyer'&&prospectSection==='today'&&(sessionWasOpen||session?.dataset.sessionView==='1')){
...
function readProspectingDirtyAt(userId=uid){try{return Math.max(0,Number(localStorage.getItem(prospectingDirtyKey(userId)))||0)}catch{return 0}}
function markProspectingDirty(at=Date.now()){const revision=Math.max(Date.now(),Number(at)||0,readProspectingDirtyAt()+1);try{localStorage.setItem(prospectingDirtyKey(),String(revision))}catch(err){console.warn('Prospecting retry marker could not be saved',err)}return revision}
function clearProspectingDirty(revision=0){try{if(readProspectingDirtyAt()<=Number(revision))localStorage.removeItem(prospectingDirtyKey())}catch(err){console.warn('Prospecting retry marker could not be cleared',err)}}
function prospectingLatestLocalAt(prospectList=prospects,interactionList=prospectInteractions,marketEvents=marketPulseEvents){return Math.max(0,...prospectList.map(item=>Number(item.updatedAt)||0),...interactionList.map(item=>Number(item.at)||0),...marketEvents.map(item=>Math.max(Number(item.createdAt)||0,Number(item.sessionStartedAt)||0,Number(item.sessionCompletedAt)||0)))}
function mergeProspectingRecovery(cloudProspects=[],cloudInteractions=[]){
  const records=new Map(cloudProspects.map(item=>[item.id,item]));for(const local of prospects){const remote=records.get(local.id);if(!remote||Number(local.updatedAt)>=Number(remote.updatedAt))records.set(local.id,local)}
  const interactions=new Map(cloudInteractions.map(item=>[item.id,item]));for(const local of prospectInteractions){const remote=interactions.get(local.id);if(!remote||Number(local.at)>=Number(remote.at))interactions.set(local.id,local)}
  return{prospects:normaliseProspects([...records.values()]),interactions:normaliseProspectInteractions([...interactions.values()].sort((a,b)=>(Number(a.at)||0)-(Number(b.at)||0)))}
}
function compactProspectingCloudValue(value){
  if(Array.isArray(value))return value.map(compactProspectingCloudValue);
  if(!value||typeof value!=='object')return value;
  const compact={};for(const [key,item] of Object.entries(value)){const next=compactProspectingCloudValue(item),empty=next===undefined||next===null||next===''||next===false||next===0||Array.isArray(next)&&!next.length;if(!empty)compact[key]=next}return compact
}
function preserveNewerLocalSellerDetails(cloudProspects=[]){
  const localById=new Map(prospects.map(record=>[record.id,record])),sellerFields=['sellerBedrooms','sellerBathrooms','sellerCars','sellerPropertyType','sellerLandSizeBand','sellerTitleType'];let retained=false;
...
async function flushProspectingSave(){
  ...
  const payload=pendingProspectingPayload,signature=pendingProspectingSignature,revision=pendingProspectingRevision,waiters=prospectingSaveWaiters.splice(0);
  pendingProspectingPayload=null;pendingProspectingSignature='';pendingProspectingRevision=0;
  if(signature===lastProspectingSignature){clearProspectingDirty(revision);waiters.forEach(({resolve})=>resolve());return}
  const writeUid=uid,writeUser=currentUser,sameSession=()=>cloud&&uid===writeUid&&currentUser===writeUser;
  prospectingWriteInFlight=true;beginSyncOperation();
  try{await setDoc(doc(db,'users',writeUid,'prospecting','state'),{...payload,clientUpdatedAt:revision||Date.now(),updatedAt:serverTimestamp()},{merge:false});if(!sameSession()){waiters.forEach(({resolve})=>resolve());return}lastProspectingSignature=signature;clearProspectingDirty(revision);clearTimeout(prospectingRetryTimer);prospectingRetryTimer=null;prospectingRetryDelay=2500;clearSyncError();endSyncOperation();waiters.forEach(({resolve})=>resolve())}
  catch(err){if(!sameSession()){waiters.forEach(({resolve})=>resolve());return}console.error(err);endSyncOperation({error:true});waiters.forEach(({reject})=>reject(err));toast('Prospecting changes saved on this device. Cloud sync will retry.');scheduleProspectingRetry()}
  finally{if(sameSession()){prospectingWriteInFlight=false;if(pendingProspectingPayload){clearTimeout(prospectingSaveTimer);prospectingSaveTimer=setTimeout(flushProspectingSave,80)}}}
}
function queueProspectingSave(){
  ...
  const normalised={prospects:normaliseProspects(prospects),interactions:normaliseProspectInteractions(prospectInteractions),marketPulseEvents:normaliseMarketPulseEvents(marketPulseEvents),marketPulseHistory},signature=prospectingSignature(normalised.prospects,normalised.interactions,normalised.marketPulseEvents,normalised.marketPulseHistory),snapshot=compactProspectingCloudValue(normalised),revision=readProspectingDirtyAt()||markProspectingDirty(prospectingLatestLocalAt());
  if(signature===lastProspectingSignature&&!pendingProspectingPayload&&!prospectingWriteInFlight){clearProspectingDirty(revision);return Promise.resolve()}
  pendingProspectingPayload=snapshot;pendingProspectingSignature=signature;pendingProspectingRevision=Math.max(pendingProspectingRevision,revision);
...
function marketFollowUpSessionPromptMarkup(event,prospectId){const prompt=marketFollowUpPrompt(event,prospectId),person=prospectById(prospectId),similarity=person?sellerMarketSimilarity(person,event):null,close=similarity?.close?`<aside class="seller-close-match"><span>✓</span><div><strong>Close match</strong><small>${escapeHtml(sellerConfigurationLabel(person))}</small></div></aside>`:'';return`${close}${prompt?`<aside class="market-followup-prompt"><span>HOT SPOTTING FOLLOW-UP</span><strong>${escapeHtml(prompt.reason)}</strong><p>${escapeHtml(prompt.context)}</p></aside>`:''}`}
...
function sendHotSpotSmsAfterOutcome(prospectIdValue){
  ...
  const message=hotSpotSmsMessage(p),interactionId=prospectId(),at=Date.now(),eventId=cleanText(prospectSessionContext?.eventId,160);
  prospectInteractions.push({id:interactionId,prospectId:p.id,date:todayKey(),at,type:'SMS',outcome:'Sent SMS',note:cleanText(message,2000),nextFollowUp:'',marketEventId:eventId});
  prospects=prospects.map(x=>x.id===p.id?normaliseProspect({...x,lastContact:todayKey(),updatedAt:at}):x);
  prospectSessionStats.sms=(Number(prospectSessionStats.sms)||0)+1;
  closeHotSpotSmsDecision();advanceHotSpotSmsOutcome();
  saveProspecting({render:false,awaitCloud:false}).catch(err=>{console.error('Hot Spotting SMS save failed',err);toast('SMS was saved locally. Please check sync.')});
  toast('SMS ready · next contact loaded');window.location.href=smsHref(phone,message)
}
...
function knockingCaptureMarkup(type,entry=null){
  ...
  return `<section class="knocking-capture-card glass" role="dialog" aria-modal="true" aria-label="${title}"><div class="knocking-capture-head"><div><span>DOORKNOCKING</span><h2>${title}</h2></div><button type="button" data-close-knock-capture aria-label="Close">×</button></div><form id="knockingCaptureForm">${isAppointment&&!entry?knockingContactPickerMarkup():''}<label>Client name<input name="name" type="text" autocomplete="name" autocapitalize="words" value="${escapeHtml(entry?.name||'')}" required></label><label>Phone number<input name="phone" type="tel" autocomplete="tel" inputmode="tel" value="${escapeHtml(entry?.phone||'')}" required></label><label>Property address<input name="address" type="text" autocomplete="street-address" value="${escapeHtml(entry?.address||'')}" required></label>${isAppointment?`<div class="knocking-capture-schedule"><label>Booking date<input name="date" type="date" value="${escapeHtml(entry?.date||todayKey())}" required></label><label>Booking time<input name="time" type="time" value="${escapeHtml(entry?.time||'12:00')}" required></label></div>`:''}<div class="form-error hidden" data-knock-capture-error role="alert"></div><button class="primary" type="submit">${entry?'Save Changes':isAppointment?`Book ${type}`:'Add to Contacts'}</button></form></section>`;
}
...
function knockingContactPickerMarkup(){return `<section class="cleanup-contact-picker"><label>Find an existing contact<input type="search" data-knock-contact-search placeholder="Search name, address or phone" autocomplete="off"></label><div data-knock-contact-results aria-live="polite"></div></section>`}
document.addEventListener('input',e=>{
  if(!e.target.matches('[data-knock-contact-search]'))return;
  const list=e.target.closest('form').querySelector('[data-knock-contact-results]'),query=e.target.value.trim();
  if(!query){list.innerHTML='';return}
  const matches=appointmentContactMatches(query);
  list.innerHTML=matches.length?matches.map(p=>`<button type="button" data-knock-select-contact="${escapeHtml(p.id)}"><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb)||'No address')}${primaryProspectPhone(p)?` · ${escapeHtml(primaryProspectPhone(p))}`:''}</small></button>`).join(''):'<p>No matching contacts. Enter their details below.</p>';
});
document.addEventListener('click',e=>{
  const button=e.target.closest('[data-knock-select-contact]');if(!button)return;
  const p=prospectById(button.dataset.knockSelectContact),form=button.closest('form');if(!p||!form)return;
  form.elements.name.value=p.name||'';form.elements.phone.value=primaryProspectPhone(p)||'';form.elements.address.value=formatProspectAddress(p.address||p.company,p.suburb)||'';
...
function consumerSyncError(error,fallback='Sync is reconnecting. Your changes remain saved on this device.'){
  const message=String(error?.message||error||''),code=String(error?.code||'').replace(/^firestore\//,'');
  if(/INTERNAL ASSERTION FAILED|quota has been exceeded|setItem@|firebase-firestore\.js/i.test(message))return'Sync is reconnecting. Your changes remain saved on this device.';
  if(['aborted','cancelled','deadline-exceeded','network-request-failed','resource-exhausted','unavailable','unknown'].includes(code))return'Sync is reconnecting. Your changes remain saved on this device.';
  return fallback
}
...
function subscribeTeamMembersForOwner(){
  ...
  },err=>{if(teamId!==listeningTeamId)return;console.error('Team member list failed',err);teamMembersStatus='error';teamMembersError=consumerSyncError(err,'Member list is temporarily unavailable.');renderTeamManager();renderTeamSettings()});
}
...
async function initialiseTeamLayer(profile={}, {promptNew=false}={}){
  ...
  try{
    ...
    clearVerifiedTeamState({forgetCached:!isTransientTeamError(err)});setTeamLayerStatus('error',consumerSyncError(err,'Team setup is temporarily unavailable. Core sync is unaffected.'));
  }
}
...
$('#openTodayTimeline').onclick=e=>{const action=e.currentTarget.dataset.planAction,eventId=e.currentTarget.dataset.eventId;if(action)navigateDailyPlanAction(action,eventId);else switchView('scheduleView')};
$('#openTaskComposer')?.addEventListener('click',openTaskComposer);
$('#closeTaskComposer')?.addEventListener('click',closeTaskComposer);
...
$('#openMarketPulseOffDay')&&($('#openMarketPulseOffDay').onclick=()=>openMarketPulseDataArea('home'));
$('#todayView')?.addEventListener('click',event=>{
  const quickAction=event.target.closest('[data-off-day-quick]');if(!quickAction)return;event.preventDefault();event.stopPropagation();runOffDayQuickAction(quickAction.dataset.offDayQuick)
});
$('#offDayConversations')?.addEventListener('click',event=>{
  const moveAppointment=event.target.closest('[data-off-day-move-appointment]');if(moveAppointment){event.preventDefault();openAppointmentEditorFromToday(moveAppointment.dataset.offDayMoveAppointment);return}
  const moveContact=event.target.closest('[data-off-day-move-contact]');if(moveContact){event.preventDefault();openOffDayContactMove(moveContact.dataset.offDayMoveContact);return}
  const buyerMatchCall=event.target.closest('[data-buyer-match-call]');if(buyerMatchCall){event.preventDefault();launchBuyerMatchCall(buyerMatchCall.dataset.buyerMatchCall,buyerMatchCall.dataset.matchId);return}
  const buyerMatchSms=event.target.closest('[data-buyer-match-sms]');if(buyerMatchSms){event.preventDefault();launchBuyerMatchSms(buyerMatchSms.dataset.buyerMatchSms,buyerMatchSms.dataset.matchId);return}
...
$('#cancelSellerPriorityDeferral')?.addEventListener('click',closeSellerPriorityDeferral);
$('#sellerPriorityDeferralModal')?.addEventListener('click',e=>{if(e.target.id==='sellerPriorityDeferralModal')closeSellerPriorityDeferral();const resolution=e.target.closest('[data-seller-resolution]');if(resolution){resolveSellerPriority(resolution.dataset.sellerResolution);return}const choice=e.target.closest('[data-seller-deferral-hours]');if(choice)deferSellerPriority(choice.dataset.sellerDeferralHours,choice.dataset.sellerDeferralReason)});
...
$('#clearMarketPulseEvents')&&($('#clearMarketPulseEvents').onclick=async()=>{if(!marketPulseEvents.length&&!marketPulseHistory.length)return;marketPulseEvents=[];marketPulseHistory=[];invalidateSellerPriorityCache({delay:80});saveLocal();renderMarketPulse();try{await queueProspectingSave()}catch(err){console.error('Hot Spotting clear sync failed',err)}toast('Hot Spotting cleared')});
...
$('#prospectingView').onclick=async e=>{
  ...
  if(e.target.closest('[data-buyer-session-back]')){buyerSession.active=true;buyerSession.visible=false;saveBuyerSession();$('#prospectingSession').classList.add('hidden');delete $('#prospectingSession').dataset.sessionView;delete $('#prospectingSession').dataset.sessionKind;$('#prospectingDashboard').classList.remove('hidden');renderBuyerSessionHero();return}
  ...
  const openSellerProperty=e.target.closest('[data-open-seller-property]');if(openSellerProperty){openSellerPropertyEditor(openSellerProperty.dataset.openSellerProperty);return}
  if(e.target.closest('[data-close-seller-property]')){renderProspectDetail(activeProspectId);return}
  ...
  const sellerChoice=e.target.closest('[data-seller-choice]');if(sellerChoice){const form=sellerChoice.closest('#sellerPropertyEditor,#prospectEditor'),target=sellerChoice.dataset.sellerChoice,value=sellerChoice.dataset.sellerChoiceValue,input=form?.elements.namedItem(target);if(input)input.value=value;form?.querySelectorAll(`[data-seller-choice="${target}"]`).forEach(button=>{const active=button===sellerChoice;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))});if(form?.dataset.contactDraft==='1')saveContactDraftFromForm(form);return}
  const sellerToggle=e.target.closest('[data-seller-toggle]');if(sellerToggle){const form=sellerToggle.closest('#sellerPropertyEditor,#prospectEditor'),target=sellerToggle.dataset.sellerToggle,value=sellerToggle.dataset.sellerToggleValue,input=form?.elements.namedItem(target),wasActive=sellerToggle.classList.contains('active');form?.querySelectorAll(`[data-seller-toggle="${target}"]`).forEach(button=>{button.classList.remove('active');button.setAttribute('aria-pressed','false')});if(input)input.value=wasActive?'':value;if(!wasActive){sellerToggle.classList.add('active');sellerToggle.setAttribute('aria-pressed','true')}if(target==='sellerPropertyType')syncSellerPropertyEditor(form,true);if(form?.dataset.contactDraft==='1')saveContactDraftFromForm(form);return}
  ...
  const open=e.target.closest('[data-open-prospect]');if(open){sellerPriorityReturnView=homeQuickProspectorReturn?'todayView':'';$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');renderProspectDetail(open.dataset.openProspect);return}
  if(e.target.closest('[data-close-prospect]')){if($('#prospectEditor')?.dataset.contactDraft==='1')cancelContactEditor();else if($('#prospectEditor')&&activeProspectId)renderProspectDetail(activeProspectId);else if(buyerSession.active&&buyerSession.visible&&$('#prospectingSession')?.dataset.sessionKind==='buyer'){activeProspectId=null;setProspectorSection('today',{resetSubview:false});showBuyerSession()}else closeProspectDetail();return}
  ...
  if(e.target.closest('[data-cancel-sms]')){if(!finishHotSpotSmsAfterOutcome())showProspectingSession();return}
  const openMessages=e.target.closest('[data-open-hotspot-messages]');if(openMessages){const p=prospectById(openMessages.dataset.openHotspotMessages),body=cleanText($('#prospectingSession [data-hotspot-sms-body]')?.value,2000);if(!p||!body)return toast('Add a message first');const pending={prospectId:p.id,eventId:cleanText(prospectSessionContext?.eventId,160),message:body,afterOutcome:hotSpotSmsAfterOutcome,openedAt:Date.now()};saveHotSpotSmsPending(pending);showHotSpotSmsConfirmation(pending);window.location.href=smsHref(primaryProspectPhone(p),body);return}
  if(e.target.closest('[data-sms-sent]')){confirmHotSpotSmsSent();return}
  if(e.target.closest('[data-sms-not-sent]')){saveHotSpotSmsPending(null);if(!finishHotSpotSmsAfterOutcome())showProspectingSession();return}
...
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='sellerPropertyEditor'){
    e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),id=cleanText(form.dataset.prospectId,80),p=prospectById(id),f=new FormData(form);if(!p)return toast('Contact could not be found');if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}
    const updated=normaliseProspect({...p,sellerBedrooms:f.get('sellerBedrooms'),sellerBathrooms:f.get('sellerBathrooms'),sellerCars:f.get('sellerCars'),sellerPropertyType:f.get('sellerPropertyType'),sellerLandSizeBand:f.get('sellerLandSizeBand'),sellerTitleType:f.get('sellerTitleType'),updatedAt:Date.now()});prospects=prospects.map(item=>item.id===updated.id?updated:item);
    saveProspecting({render:false,awaitCloud:false}).catch(err=>console.error('Deferred property details sync failed',err));renderProspectDetail(updated.id);toast(sellerPropertyConfigured(updated)?'Property details saved':'Property details cleared');return
  }
...
$('#importData').onchange=async e=>{const file=e.target.files?.[0];e.target.value='';if(!file)return;try{const raw=JSON.parse(await file.text());if(!raw||typeof raw!=='object'||Array.isArray(raw))throw new Error('Invalid backup');if(!confirm('Import this AGNT backup?\n\nExisting records with the same date or ID may be updated.'))return;const importedDays=normaliseDaysMap(raw.days||{}),dayKeys=Object.keys(importedDays),prospectingIncluded=Array.isArray(raw.prospects)||Array.isArray(raw.prospectInteractions)||Array.isArray(raw.marketPulseEvents)||Array.isArray(raw.marketPulseHistory);if(raw.targets&&typeof raw.targets==='object')targets={...DEFAULTS,...raw.targets};if(Array.isArray(raw.workDays)&&raw.workDays.length)workDays=normaliseWorkDays(raw.workDays);if(typeof raw.agentName==='string'&&raw.agentName.trim())agentName=cleanText(raw.agentName,120);if(raw.calendarPreference==='apple'||raw.calendarPreference==='outlook')calendarPreference=raw.calendarPreference;if(['system','light','dark'].includes(raw.appearancePreference))applyAppearance(raw.appearancePreference);days={...days,...importedDays};if(Array.isArray(raw.prospects))prospects=normaliseProspects(mergeBackupRecords(prospects,raw.prospects));if(Array.isArray(raw.prospectInteractions))prospectInteractions=normaliseProspectInteractions(mergeBackupRecords(prospectInteractions,raw.prospectInteractions));if(Array.isArray(raw.marketPulseEvents))marketPulseEvents=normaliseMarketPulseEvents(mergeBackupRecords(marketPulseEvents,raw.marketPulseEvents));if(Array.isArray(raw.marketPulseHistory))marketPulseHistory=normaliseMarketPulseHistory(mergeBackupRecords(marketPulseHistory,raw.marketPulseHistory));if(Array.isArray(raw.campaignHistory))campaignHistory=mergeBackupRecords(campaignHistory,raw.campaignHistory).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0)).slice(0,20);if(Array.isArray(raw.bulkSmsTestLaunches))bulkSmsTestLaunches=mergeBackupRecords(bulkSmsTestLaunches,raw.bulkSmsTestLaunches).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0)).slice(0,10);saveLocal();restoreBuyerSessionBackup(raw.buyerSession);renderAll();syncImportedBackup(dayKeys,prospectingIncluded);toast('Backup restored · cloud sync running')}catch(err){console.error('Backup import failed',err);toast('Backup could not be read')}};
