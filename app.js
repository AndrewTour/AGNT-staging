...
let prospectMapReturnSection='contacts',prospectMapFilter='contacts',prospectMap=null,prospectMapTheme='',prospectMapReady=false,prospectMapLocations=new Map(),prospectMapLocalLoaded=false,prospectMapCloudLoaded=false,prospectMapAssetsPromise=null,prospectMapFitPending=true,prospectMapIsGeocoding=false,prospectMapManualProspectId='',prospectMapManualMarker=null,prospectMapManualCoordinate=null;
const PROSPECT_MAP_SOURCE='agnt-prospector-map-source',PROSPECT_MAP_TOKEN_KEY='agnt-prospector-geoapify-key-v1',PROSPECT_MAP_BATCH_LIMIT=100,PROSPECT_MAP_GEOCODE_DELAY_MS=225;
...
function returningSnapshotOpenKey(){return `${storagePrefix(uid)}returning-snapshot-open-toggle`}
function shouldShowReturningSnapshotThisOpen(){try{const previous=Number(localStorage.getItem(returningSnapshotOpenKey())||0),next=(previous+1)%2;localStorage.setItem(returningSnapshotOpenKey(),String(next));return next===0}catch{return true}}
function showLaunchExperience(){if(showOffDayReview())return;if(returningSnapshotHasHistory()){if(shouldShowReturningSnapshotThisOpen())showReturningSnapshot();return}showDailyWelcome()}
...
function pageHeaderState(id=activeViewId()){
  ...
  if(id==='prospectingView'){
    ...
    if(prospectSection==='map'){const mapped=prospectMapAllRecords().filter(p=>prospectMapCurrentLocation(p)).length;return{title:'Prospector Map',subtitle:mapped?`${mapped} client address${mapped===1?'':'es'} ready to explore.`:'Map your database by address.'}};
...
function prospectFollowUpsForDate(viewDate){
  return [...activeProspects(),...activeBuyerProspects()].filter(p=>{
...
function timelineItemsForDate(viewDate){
  ...
  if(!isSaturday)prospectFollowUpsForDate(viewDate).forEach((p,index)=>{
    const rawPhone=String(primaryProspectPhone(p)||'').trim(),dial=rawPhone.replace(/[^+\d]/g,''),isBuyer=p.recordType==='buyer';
...
async function completePendingProspectAppointmentFlow(){
  ...
  prospectInteractions.push({id:interactionId,prospectId:p.id,date:todayKey(),at:Date.now(),type:'Call',outcome,note:flow.note||'',nextFollowUp:validDateKey(next)?next:'',marketEventId:cleanText(flow.marketEventId,160),metricsApplied:false});
...
function normaliseProspectInteractions(list){return(Array.isArray(list)?list:[]).filter(x=>x&&typeof x==='object').map(x=>({id:cleanText(x.id,80)||prospectId(),prospectId:cleanText(x.prospectId,80),date:validDateKey(x.date)?x.date:todayKey(),at:Number(x.at)||Date.now(),type:cleanText(x.type,40)||'Note',outcome:cleanText(x.outcome,80),note:cleanText(x.note,2000),nextFollowUp:validDateKey(x.nextFollowUp)?x.nextFollowUp:'',marketEventId:cleanText(x.marketEventId,160)})).filter(x=>x.prospectId).slice(-20000)}
function prospectById(id){return prospects.find(p=>p.id===id)}
function activeProspects(){return prospects.filter(p=>!p.archived&&p.recordType!=='buyer')}
function archivedProspects(){return prospects.filter(p=>p.archived&&p.recordType!=='buyer')}
function activeBuyerProspects(){return prospects.filter(p=>!p.archived&&p.recordType==='buyer')}
function archivedBuyerProspects(){return prospects.filter(p=>p.archived&&p.recordType==='buyer')}
...
function normaliseMarketPulseEvents(list){return(Array.isArray(list)?list:[]).filter(x=>x&&typeof x==='object').map(x=>{const address=cleanText(x.address,240),suburb=cleanText(x.suburb,100),junk=/^(?:property image|logo)$/i;let agency=cleanText(x.agency,160),agents=[...new Set((Array.isArray(x.agents)?x.agents:[]).map(value=>cleanText(value,120)).filter(value=>value&&!junk.test(value)))].slice(0,4);if(junk.test(agency)){agency=agents.at(-1)||'';agents=agency?agents.slice(0,-1):agents}return{id:cleanText(x.id,160),eventType:cleanText(x.eventType,60),address,suburb,streetKey:marketStreetKey(address,suburb)||cleanText(x.streetKey,180),receivedDate:validDateKey(x.receivedDate)?x.receivedDate:todayKey(),createdAt:Number(x.createdAt)||Date.now(),daysOnMarket:cleanText(x.daysOnMarket,80),price:cleanText(x.price,120),guide:cleanText(x.guide,120),propertyDetails:cleanText(x.propertyDetails,240),agency,agents,priceMovementAmount:cleanText(x.priceMovementAmount,80),priceMovementPercent:cleanText(x.priceMovementPercent,40),priceMovementDirection:['above','below'].includes(x.priceMovementDirection)?x.priceMovementDirection:'',priorPrice:cleanText(x.priorPrice,120),sessionStartedAt:Number(x.sessionStartedAt)||0,sessionCompletedAt:Number(x.sessionCompletedAt)||0,skippedProspectIds:[...new Set((Array.isArray(x.skippedProspectIds)?x.skippedProspectIds:[]).map(id=>cleanText(id,160)).filter(Boolean))]}}).filter(x=>x.id&&x.address&&x.streetKey).filter((x,i,a)=>a.findIndex(y=>y.id===x.id)===i).slice(-300)}
...
function parseMarketPulse(text=''){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    const priceIndex=afterAddress.findIndex(line=>/\$\s*[\d,.]+\s*[kKmM]?/i.test(line));const priceLine=priceIndex>=0?afterAddress[priceIndex]:'';const priceData=parseMarketPriceLine(priceLine,eventType);
    const activeMatch=lines[i].match(/(\d+)\s+days?\s+active/i)||afterAddress.join(' ').match(/(\d+)\s+days?\s+active/i);const daysOnMarket=activeMatch?`${activeMatch[1]} days active`:'';
    const peopleAndAgency=(priceIndex>=0?afterAddress.slice(priceIndex+1):[]).filter(line=>!/^(?:property image|logo)$/i.test(line));const agency=peopleAndAgency.length?peopleAndAgency.at(-1):'';const agents=peopleAndAgency.slice(0,-1);
    events.push({id:marketEventId(eventType,address,suburb),eventType,address,suburb,streetKey,receivedDate:todayKey(),createdAt:Date.now(),daysOnMarket,propertyDetails,agency,agents,...priceData});
  }
...
function streetConversationCount(events=[]){const ids=new Set(events.flatMap(event=>marketMatches(event).map(p=>p.id)));return prospectInteractions.filter(x=>ids.has(x.prospectId)&&x.type==='Call'&&!events.some(event=>x.marketEventId===event.id)).length}
...
function marketSessionProgress(event,matches=marketMatches(event)){
  ...
  return{total:matches.length,called:Math.min(calledIds.size,matches.length),sms:Math.min(smsIds.size,matches.length),connects:connectedIds.size,followUps:followUpIds.size,active,complete,workedIds};
}
...
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{const matches=marketMatches(event),progress=marketSessionProgress(event,matches),remaining=Math.max(0,progress.total-progress.workedIds.size),recency=relativeEventRecency(event),priority=hotSpottingPriority(event,matches.length),buttonLabel=progress.complete?'Session Complete':progress.active?'Active Session':'Start Session',buttonClass=progress.complete?'primary market-session-complete-btn':progress.active?'primary market-session-active-btn':'primary market-session-start-btn';return`<article class="market-pulse-card${progress.complete?' market-session-complete-card':''}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p>${marketEventDetailHtml(event)}<div class="hotspot-opportunity-summary"><span><strong>${remaining}</strong> remaining</span><span><strong>${formatEstimatedTime(estimatedMinutes(remaining))}</strong> session</span><span class="priority-${priority.label.toLowerCase()}"><strong>${priority.label}</strong> priority</span><span><strong>${recency.label}</strong> event</span></div><button class="${buttonClass}" type="button" data-start-market-session="${escapeHtml(event.id)}" ${matches.length&&!progress.complete?'':'disabled'}>${matches.length?buttonLabel:'No Matching Contacts'}</button></article>`}).join(''):'<div class="prospect-empty"><strong>No hot spotting opportunities yet</strong><small>Paste today’s email below, then tap Find Opportunities.</small></div>';
}
async function importMarketPulse(){const input=$('#marketPulseInput'),parsed=parseMarketPulse(input?.value||'');if(!parsed.length){$('#marketPulseImportStatus').textContent='No supported property events were found. Paste the full email text, including event labels and addresses.';return toast('No market events found')}const existingById=new Map(marketPulseEvents.map(event=>[event.id,event])),fresh=parsed.filter(event=>!existingById.has(event.id));parsed.forEach(event=>{const existing=existingById.get(event.id);existingById.set(event.id,existing?{...existing,...event,sessionStartedAt:existing.sessionStartedAt,sessionCompletedAt:existing.sessionCompletedAt,skippedProspectIds:existing.skippedProspectIds}:event)});marketPulseEvents=normaliseMarketPulseEvents([...existingById.values()]);saveLocal();renderMarketPulse();const updated=parsed.length-fresh.length;$('#marketPulseImportStatus').textContent=`${fresh.length} new event${fresh.length===1?'':'s'} imported${updated?` · ${updated} existing event${updated===1?'':'s'} refreshed`:''}.`;try{await queueProspectingSave()}catch(err){console.error('Hot Spotting sync failed',err)}toast(fresh.length?'Hot Spotting imported':'Hot Spotting refreshed')}
function startMarketPulseSession(eventId){const event=marketPulseEvents.find(x=>x.id===eventId);if(!event)return;if(event.sessionCompletedAt)return;if(prospectSessionActive&&prospectSessionContext?.eventId===eventId){setProspectorSection('today');showProspectingSession();return}const matches=marketMatches(event),progress=marketSessionProgress(event,matches),remainingIds=matches.map(p=>p.id).filter(id=>!progress.workedIds.has(id));if(!matches.length)return toast('No available contacts for this session');prospectSessionIds=remainingIds.length?remainingIds:matches.map(p=>p.id);prospectSessionIndex=remainingIds.length?0:prospectSessionIds.length;prospectSessionActive=true;prospectSessionStats={calls:0,connects:0,temperate:0,appointments:0,sms:0};prospectSessionContext={eventId:event.id,eventType:event.eventType,address:event.address,suburb:event.suburb,daysOnMarket:event.daysOnMarket||'',guide:event.guide||'',price:event.price||'',propertyDetails:event.propertyDetails||'',agency:event.agency||'',agents:event.agents||[],priceMovementAmount:event.priceMovementAmount||'',priceMovementPercent:event.priceMovementPercent||'',priceMovementDirection:event.priceMovementDirection||'',priorPrice:event.priorPrice||''};if(!event.sessionStartedAt){event.sessionStartedAt=Date.now();marketPulseEvents=normaliseMarketPulseEvents(marketPulseEvents);saveLocal();renderMarketPulse();queueProspectingSave().catch(err=>console.error('Hot Spotting session sync failed',err))}saveProspectingSessionState();setProspectorSection('today');showProspectingSession();requestAnimationFrame(()=>requestAnimationFrame(()=>{const view=$('#prospectingView');if(view)view.scrollTop=0;window.scrollTo({top:0,behavior:'instant'})}));toast(`${event.eventType} session ready`)}
...
function buyerEditorForm(p={},prefill={}){
  const current={...p,...prefill},max=buyerMaximumBudget(current),stage=BUYER_STAGES.includes(current.buyerStage)&&current.buyerStage!=='Purchased'?current.buyerStage:'Looking',temperature=['Cold','Warm','Hot'].includes(current.temperature)?current.temperature:'Warm',currentPositionTags=buyerPositionTags(current);
  return`<form id="buyerEditor" class="buyer-editor glass"><div class="prospect-detail-nav buyer-detail-nav"><button type="button" data-close-buyer>‹ Back</button><strong>${p.id?'Edit buyer':'New buyer'}</strong><span></span></div><div class="buyer-editor-intro"><span>BUYER BRIEF</span><h2>${p.id?'Keep the brief current':'Capture the brief quickly'}</h2><p>Keep the essentials simple. Add more detail only where it helps the search.</p></div><div class="prospect-form-grid"><label>Name<input name="name" value="${escapeHtml(current.name||'')}" autocomplete="name" required></label><label>Mobile<input name="phone" type="tel" inputmode="tel" value="${escapeHtml(current.phone||'')}" autocomplete="tel"></label></div><label>Current address <small>Optional</small><input name="address" value="${escapeHtml(current.address||'')}" autocomplete="street-address"></label><section class="buyer-editor-section buyer-budget-section"><div class="buyer-editor-section-head"><span>MAXIMUM BUDGET</span><strong id="buyerBudgetMaxLabel">${buyerBudgetEditorLabel(max)}</strong></div><input id="buyerBudgetMax" name="buyerBudgetMax" class="buyer-budget-slider" type="range" min="0" max="5000000" step="50000" value="${max}" aria-label="Maximum buyer budget"></section><section class="buyer-editor-section"><span class="buyer-field-label">SUBURBS</span><div class="buyer-suburb-picker"><div class="buyer-suburb-entry"><input type="search" data-buyer-suburb-input placeholder="Add suburb" autocomplete="off"><button type="button" data-add-buyer-suburb>Add</button></div><div class="buyer-suburb-suggestions hidden" data-buyer-suburb-suggestions></div></div><div class="buyer-suburb-chips" data-buyer-suburb-chips></div></section><section class="buyer-editor-section"><span class="buyer-field-label">CONFIGURATION</span><div class="buyer-config-grid"><div><small>Bedrooms</small>${buyerChoiceMarkup('buyerBedrooms',current.buyerBedrooms||0,[0,1,2,3,4,5],{0:'Any'})}</div><div><small>Bathrooms</small>${buyerChoiceMarkup('buyerBathrooms',current.buyerBathrooms||0,[0,1,2,3,4],{0:'Any'})}</div><div><small>Cars</small>${buyerChoiceMarkup('buyerCars',current.buyerCars||0,[0,1,2,3],{0:'Any'})}</div></div></section><section class="buyer-editor-section buyer-position-section"><span class="buyer-field-label">POSITION</span><div class="buyer-option-tags buyer-position-tags">${buyerPositionEditorMarkup(currentPositionTags)}</div><small class="buyer-position-help">Select any that describe this buyer.</small></section><section class="buyer-editor-section"><span class="buyer-field-label">PROPERTY TYPE</span><div class="buyer-option-tags buyer-property-types">${buyerPropertyTypeMarkup(current.buyerPropertyType||'')}</div></section><section class="buyer-editor-section"><span class="buyer-field-label">MUST-HAVES</span><div class="buyer-option-tags buyer-feature-tags">${buyerFeatureMarkup(current.buyerFeatures||[])}</div></section><section class="buyer-editor-section"><span class="buyer-field-label">JOURNEY</span><div class="prospect-form-grid"><label>Stage<select name="buyerStage">${['Looking','Inspecting','Negotiating'].map(value=>`<option ${stage===value?'selected':''}>${value}</option>`).join('')}</select></label><label>Temperature<select name="temperature">${['Cold','Warm','Hot'].map(value=>`<option ${temperature===value?'selected':''}>${value}</option>`).join('')}</select></label></div></section><label>Buyer notes<textarea name="notes" rows="4" placeholder="Anything else that matters to the search">${escapeHtml(current.notes||'')}</textarea></label><button class="primary buyer-save-button" type="submit">${p.id?'Save buyer':'Add buyer'}</button></form>`
}
function openBuyerEditor(id='',prefill={},context=null){const p=id?prospectById(id):{};activeProspectId=id||null;pendingBuyerEditorContext=context;setProspectorSection('buyers');$('#prospectingDashboard').classList.add('hidden');$('#prospectingSession').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');$('#prospectDetail').innerHTML=buyerEditorForm(p,prefill);ensureBuyerSuburbDatalist();const form=$('#buyerEditor');form._buyerSuburbs=[...(p?.buyerSuburbs||prefill.buyerSuburbs||[])];renderBuyerSuburbChips(form);if(!id&&!prefill.name&&!prefill.phone)requestAnimationFrame(()=>form.querySelector('input[name="name"]')?.focus({preventScroll:true}))}
function findBuyerByPhone(phone='',excludeId=''){const digits=normalisedPhoneDigits(phone);if(!digits)return null;return prospects.find(p=>p.recordType==='buyer'&&p.id!==excludeId&&normalisedPhoneDigits(primaryProspectPhone(p))===digits)||null}
async function upsertBuyer(data,id='',context=null){
  const duplicate=!id?findBuyerByPhone(data.phone):null;
  if(duplicate){const error=new Error('A buyer with this mobile already exists');error.code='buyer-duplicate';error.existingId=duplicate.id;throw error}
...
function renderBuyerDetail(id){
  const p=prospectById(id);if(!p||p.recordType!=='buyer')return closeProspectDetail();activeProspectId=id;
  const history=interactionsFor(id),phone=primaryProspectPhone(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#',archived=Boolean(p.archived),initials=p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase();
  ...
  $('#prospectDetail').innerHTML=`<div class="prospect-detail-nav buyer-detail-nav"><button type="button" data-close-buyer>‹ Back</button>${archived?'<span></span>':`<button type="button" data-edit-buyer="${p.id}">Edit</button>`}</div><section class="buyer-detail-card glass"><div class="buyer-detail-head"><span class="prospect-avatar large">${escapeHtml(initials)}</span><div><span>${archived?'Archived':escapeHtml(p.buyerStage||'Looking')} · ${escapeHtml(p.temperature)}</span><h2>${escapeHtml(p.name)}</h2>${buyerDetailContactMarkup(p,phone)}</div></div>${actions}${buyerRequirementRows(p)}${p.notes?`<p class="prospect-background">${escapeHtml(p.notes)}</p>`:''}${lifecycle}</section><section class="prospecting-section glass buyer-activity-section"><div class="prospecting-section-head"><div><span>ACTIVITY</span><h3>Every conversation</h3></div></div><div class="prospect-history">${buyerHistoryMarkup(history)}</div></section>${archived?`<button class="prospect-delete" type="button" data-delete-buyer="${p.id}">Delete buyer permanently</button>`:''}`
}

function prospectMapToken(){try{return cleanText(localStorage.getItem(PROSPECT_MAP_TOKEN_KEY)||'',700)}catch{return''}}
function prospectMapStorageKey(){return`${storagePrefix(uid)}prospector-map-locations-v1`}
function prospectMapAddress(p={}){const address=formatProspectAddress(p.address||p.company,p.suburb);if(!address)return'';return /\baustralia\b/i.test(address)?address:`${address}, NSW, Australia`}
function prospectMapAddressKey(p={}){return prospectMapAddress(p).toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()}
function prospectMapDocId(id=''){return encodeURIComponent(cleanText(id,1000)).slice(0,1400)||`map_${Date.now()}`}
function normaliseProspectMapLocation(raw={}){const lat=Number(raw.lat),lng=Number(raw.lng),source=raw.source==='manual'?'manual':raw.source==='mapbox'?'mapbox':'geoapify';return{prospectId:cleanText(raw.prospectId,80),addressKey:cleanText(raw.addressKey,360),addressLabel:cleanText(raw.addressLabel,360),lat:Number.isFinite(lat)&&Math.abs(lat)<=90?lat:null,lng:Number.isFinite(lng)&&Math.abs(lng)<=180?lng:null,accuracy:cleanText(raw.accuracy,40)||'unknown',status:['mapped','review','confirmed','failed'].includes(raw.status)?raw.status:'review',source,providerFeatureId:cleanText(raw.providerFeatureId,300),updatedAt:Number(raw.updatedAt)||Date.now()}}
function loadProspectMapLocal(){if(prospectMapLocalLoaded)return;prospectMapLocalLoaded=true;try{const raw=safeJsonParse(localStorage.getItem(prospectMapStorageKey())||'{}',{});Object.values(raw&&typeof raw==='object'?raw:{}).forEach(value=>{const location=normaliseProspectMapLocation(value);if(location.prospectId)prospectMapLocations.set(location.prospectId,location)})}catch(err){console.warn('Prospector map cache could not be read',err)}}
function saveProspectMapLocal(){try{localStorage.setItem(prospectMapStorageKey(),JSON.stringify(Object.fromEntries(prospectMapLocations)))}catch(err){console.warn('Prospector map cache could not be saved',err)}}
async function loadProspectMapCloud(){if(prospectMapCloudLoaded||!cloud||!db||!uid)return;prospectMapCloudLoaded=true;try{const snapshot=await getDocs(collection(db,'users',uid,'prospectingMap'));snapshot.forEach(item=>{const location=normaliseProspectMapLocation(item.data());if(location.prospectId){const current=prospectMapLocations.get(location.prospectId);if(!current||location.updatedAt>=current.updatedAt)prospectMapLocations.set(location.prospectId,location)}});saveProspectMapLocal();updateProspectorMap()}catch(err){prospectMapCloudLoaded=false;console.warn('Prospector map cloud cache could not be loaded',err)}}
function persistProspectMapLocation(raw={}){const location=normaliseProspectMapLocation(raw);if(!location.prospectId)return;prospectMapLocations.set(location.prospectId,location);saveProspectMapLocal();if(cloud&&db&&uid)setDoc(doc(db,'users',uid,'prospectingMap',prospectMapDocId(location.prospectId)),{...location,clientUpdatedAt:Date.now(),updatedAt:location.updatedAt},{merge:false}).catch(err=>console.warn('Prospector map location could not sync',err))}
function prospectMapAllRecords(){return[...activeProspects(),...activeBuyerProspects()].filter(p=>prospectMapAddress(p))}
function prospectMapVisibleRecords(){const queryText=cleanText($('#prospectSearch')?.value||'',120).toLowerCase();return prospectMapAllRecords().filter(p=>{if(prospectMapFilter==='contacts'&&p.recordType==='buyer')return false;if(prospectMapFilter==='buyers'&&p.recordType!=='buyer')return false;if(prospectMapFilter==='hot'&&p.temperature!=='Hot')return false;if(prospectMapFilter==='followup'&&!p.nextFollowUp)return false;if(!queryText)return true;return[p.name,p.phone,p.email,p.address,p.suburb,p.stage,p.buyerStage,...(p.tags||[]),...(p.buyerPositionTags||[])].join(' ').toLowerCase().includes(queryText)})}
function prospectMapCurrentLocation(p={}){const location=prospectMapLocations.get(p.id);return location&&location.addressKey===prospectMapAddressKey(p)&&Number.isFinite(location.lat)&&Number.isFinite(location.lng)?location:null}
function prospectMapAddressAttempt(p={}){const location=prospectMapLocations.get(p.id);return location&&location.addressKey===prospectMapAddressKey(p)?location:null}
function prospectMapPendingRecords(){return prospectMapAllRecords().filter(p=>!prospectMapAddressAttempt(p))}
function prospectMapUrgency(p={}){if(p.nextFollowUp&&p.nextFollowUp<todayKey())return'overdue';if(p.nextFollowUp===todayKey())return'today';if(p.nextFollowUp)return'scheduled';return'none'}
function prospectMapFeatureCollection(){const groups=new Map();prospectMapVisibleRecords().forEach(p=>{const location=prospectMapCurrentLocation(p);if(!location)return;const key=`${location.lng.toFixed(6)},${location.lat.toFixed(6)}`,group=groups.get(key)||{location,records:[]};group.records.push(p);groups.set(key,group)});const heat={Hot:3,Warm:2,Cold:1};return{type:'FeatureCollection',features:[...groups.values()].map(({location,records})=>{const hottest=[...records].sort((a,b)=>(heat[b.temperature]||0)-(heat[a.temperature]||0))[0]?.temperature||'Cold',urgencies=records.map(prospectMapUrgency),urgency=urgencies.includes('overdue')?'overdue':urgencies.includes('today')?'today':urgencies.includes('scheduled')?'scheduled':'none';return{type:'Feature',geometry:{type:'Point',coordinates:[location.lng,location.lat]},properties:{prospectIds:JSON.stringify(records.map(p=>p.id)),temperature:hottest,urgency,review:records.some(p=>prospectMapCurrentLocation(p)?.status==='review')?1:0,groupCount:records.length}}})}}
function loadProspectorMapAssets(){if(window.maplibregl)return Promise.resolve(window.maplibregl);if(prospectMapAssetsPromise)return prospectMapAssetsPromise;prospectMapAssetsPromise=new Promise((resolve,reject)=>{if(!document.querySelector('#agntMapLibreCss')){const link=document.createElement('link');link.id='agntMapLibreCss';link.rel='stylesheet';link.href='https://unpkg.com/maplibre-gl@5/dist/maplibre-gl.css';document.head.append(link)}const existing=document.querySelector('#agntMapLibreJs');if(existing){existing.addEventListener('load',()=>resolve(window.maplibregl),{once:true});existing.addEventListener('error',()=>reject(new Error('MapLibre could not load')),{once:true});return}const script=document.createElement('script');script.id='agntMapLibreJs';script.src='https://unpkg.com/maplibre-gl@5/dist/maplibre-gl.js';script.onload=()=>resolve(window.maplibregl);script.onerror=()=>reject(new Error('MapLibre could not load'));document.head.append(script)});return prospectMapAssetsPromise}
function showProspectorMapMessage(title='',detail=''){const panel=$('#prospectorMapMessage');if(!panel)return;if(!title){panel.classList.add('hidden');panel.innerHTML='';return}panel.innerHTML=`<strong>${escapeHtml(title)}</strong><small>${escapeHtml(detail)}</small>`;panel.classList.remove('hidden')}
function setProspectorMapSetup(open=true){$('#prospectorMapSetup')?.classList.toggle('hidden',!open);$('#prospectorMapCanvas')?.classList.toggle('setup-active',open);$('#mapProspectorAddresses')?.toggleAttribute('disabled',open||prospectMapIsGeocoding)}
function resetProspectorMapInstance(){if(prospectMapManualMarker){prospectMapManualMarker.remove();prospectMapManualMarker=null}if(prospectMap){prospectMap.remove();prospectMap=null}prospectMapTheme='';prospectMapReady=false;prospectMapFitPending=true}
function addProspectorMapLayers(){if(!prospectMap||prospectMap.getSource(PROSPECT_MAP_SOURCE))return;prospectMap.addSource(PROSPECT_MAP_SOURCE,{type:'geojson',data:prospectMapFeatureCollection(),cluster:true,clusterMaxZoom:14,clusterRadius:48});prospectMap.addLayer({id:'agnt-map-clusters',type:'circle',source:PROSPECT_MAP_SOURCE,filter:['has','point_count'],paint:{'circle-color':'#111318','circle-radius':['step',['get','point_count'],20,10,24,50,30],'circle-stroke-color':'#ffffff','circle-stroke-width':2}});prospectMap.addLayer({id:'agnt-map-cluster-count',type:'symbol',source:PROSPECT_MAP_SOURCE,filter:['has','point_count'],layout:{'text-field':['get','point_count_abbreviated'],'text-size':12},paint:{'text-color':'#ffffff'}});prospectMap.addLayer({id:'agnt-map-points',type:'circle',source:PROSPECT_MAP_SOURCE,filter:['!',['has','point_count']],paint:{'circle-color':['match',['get','temperature'],'Hot','#df4c43','Warm','#e2a634','#6c7b8d'],'circle-radius':['case',['>', ['get','groupCount'],1],11,9],'circle-stroke-color':['case',['==',['get','urgency'],'overdue'],'#cf2f25',['==',['get','review'],1],'#f2a900','#ffffff'],'circle-stroke-width':['case',['any',['==',['get','urgency'],'overdue'],['==',['get','review'],1]],4,2]}});prospectMap.addLayer({id:'agnt-map-group-count',type:'symbol',source:PROSPECT_MAP_SOURCE,filter:['all',['!',['has','point_count']],['>', ['get','groupCount'],1]],layout:{'text-field':['to-string',['get','groupCount']],'text-size':10},paint:{'text-color':'#ffffff'}});prospectMap.on('click','agnt-map-clusters',event=>{const feature=prospectMap.queryRenderedFeatures(event.point,{layers:['agnt-map-clusters']})[0],clusterId=feature?.properties?.cluster_id;if(clusterId==null)return;prospectMap.getSource(PROSPECT_MAP_SOURCE).getClusterExpansionZoom(clusterId).then(zoom=>prospectMap.easeTo({center:feature.geometry.coordinates,zoom})).catch(err=>console.warn('Map cluster could not expand',err))});prospectMap.on('click','agnt-map-points',event=>{if(prospectMapManualProspectId)return;let ids=[];try{ids=JSON.parse(event.features?.[0]?.properties?.prospectIds||'[]')}catch{}openProspectorMapSheet(ids)});['agnt-map-clusters','agnt-map-points'].forEach(layer=>{prospectMap.on('mouseenter',layer,()=>{prospectMap.getCanvas().style.cursor='pointer'});prospectMap.on('mouseleave',layer,()=>{prospectMap.getCanvas().style.cursor=''})});prospectMap.on('click',event=>{if(prospectMapManualProspectId){placeProspectorMapCorrection(event.lngLat);return}const features=prospectMap.queryRenderedFeatures(event.point,{layers:['agnt-map-clusters','agnt-map-points']});if(!features.length)closeProspectorMapSheet()});prospectMapReady=true;updateProspectorMap()}
async function ensureProspectorMap(){const apiKey=prospectMapToken();setProspectorMapSetup(!apiKey);try{await loadProspectorMapAssets();const theme=document.documentElement.dataset.theme==='dark'?'dark':'light';if(prospectMap&&prospectMapTheme!==theme)resetProspectorMapInstance();if(!prospectMap){prospectMapTheme=theme;prospectMap=new window.maplibregl.Map({container:'prospectorMapCanvas',style:theme==='dark'?'https://tiles.openfreemap.org/styles/dark':'https://tiles.openfreemap.org/styles/positron',center:[150.99,-33.81],zoom:9.5,attributionControl:false});prospectMap.addControl(new window.maplibregl.AttributionControl({compact:true,customAttribution:'Address matching © Geoapify'}),'bottom-right');prospectMap.on('load',addProspectorMapLayers);prospectMap.on('error',event=>{const message=cleanText(event?.error?.message||'',240);if(/network|failed|fetch|tile|style/i.test(message))console.warn('Free map resource could not load',message)})}else{prospectMap.resize();if(prospectMap.loaded()&&!prospectMapReady)addProspectorMapLayers()}}catch(err){console.error('Prospector map failed to load',err);showProspectorMapMessage('Map unavailable','Connect to the internet and try again.')}}
function prospectMapReviewRecords(){return prospectMapAllRecords().filter(p=>{const location=prospectMapLocations.get(p.id);return location&&location.addressKey===prospectMapAddressKey(p)&&(location.status==='review'||location.status==='failed')})}
function updateProspectorMapStatus(){const records=prospectMapAllRecords(),mapped=records.filter(p=>prospectMapCurrentLocation(p)),review=prospectMapReviewRecords().length,pending=prospectMapPendingRecords().length,withoutAddress=[...activeProspects(),...activeBuyerProspects()].filter(p=>!prospectMapAddress(p)).length,visible=prospectMapVisibleRecords().filter(p=>prospectMapCurrentLocation(p)).length;$('#prospectorMapMappedCount').textContent=`${mapped.length} mapped · ${visible} shown`;$('#prospectorMapReviewCount').textContent=`${review} need review${withoutAddress?` · ${withoutAddress} without address`:''}`;$('#prospectorMapSummary').textContent=records.length?`${records.length} address${records.length===1?'':'es'} available · ${pending} waiting to map`:'Add a full address to a contact or buyer to place them on the map.';const button=$('#mapProspectorAddresses'),reviewButton=$('#reviewProspectorMap');if(reviewButton)reviewButton.disabled=!review;if(button&&!prospectMapIsGeocoding){button.disabled=!prospectMapToken()||!pending;button.textContent=pending>PROSPECT_MAP_BATCH_LIMIT?`Map next ${PROSPECT_MAP_BATCH_LIMIT}`:pending?`Map ${pending} address${pending===1?'':'es'}`:'Addresses mapped'}}
function updateProspectorMap(){loadProspectMapLocal();updateProspectorMapStatus();$$('[data-prospector-map-filter]').forEach(button=>button.classList.toggle('active',button.dataset.prospectorMapFilter===prospectMapFilter));const source=prospectMap?.getSource(PROSPECT_MAP_SOURCE);if(!source)return;const data=prospectMapFeatureCollection();source.setData(data);if(prospectMapFitPending&&data.features.length){prospectMapFitPending=false;if(data.features.length===1)prospectMap.easeTo({center:data.features[0].geometry.coordinates,zoom:15,duration:500});else{const bounds=new window.maplibregl.LngLatBounds();data.features.forEach(feature=>bounds.extend(feature.geometry.coordinates));prospectMap.fitBounds(bounds,{padding:{top:54,right:34,bottom:150,left:34},maxZoom:15,duration:500})}}if(!prospectMapIsGeocoding)showProspectorMapMessage(data.features.length?'':prospectMapAllRecords().length?'No mapped clients in this view':'No addresses ready to map',data.features.length?'':prospectMapAllRecords().length?'Map the waiting addresses or adjust the search and filters.':'Add a complete street address to a contact or buyer.')}
function renderProspectorMap(){if(prospectSection!=='map')return;loadProspectMapLocal();const backLabel=prospectMapReturnSection==='buyers'?'Buyers':'Contacts';$('#closeProspectorMap').textContent=`‹ ${backLabel}`;$$('[data-prospector-map-filter]').forEach(button=>button.classList.toggle('active',button.dataset.prospectorMapFilter===prospectMapFilter));updateProspectorMapStatus();ensureProspectorMap();loadProspectMapCloud()}
function openProspectorMap(from='contacts'){prospectMapReturnSection=from==='buyers'?'buyers':'contacts';prospectMapFilter=prospectMapReturnSection;prospectMapFitPending=true;closeProspectorMapSheet();setProspectorSection('map');renderProspecting();requestAnimationFrame(()=>prospectMap?.resize())}
function closeProspectorMap(){cancelProspectorMapCorrection();closeProspectorMapSheet();setProspectorSection(prospectMapReturnSection);renderProspecting()}
function prospectorMapHistory(p={}){return interactionsFor(p.id).slice(0,2)}
function prospectorMapRecordMarkup(p={}){const buyer=p.recordType==='buyer',phone=primaryProspectPhone(p),address=formatProspectAddress(p.address||p.company,p.suburb)||'No address',history=prospectorMapHistory(p),next=buyer?buyerNextAction(p):{label:dueText(p),className:prospectMapUrgency(p)},tags=buyer?[p.buyerStage,p.temperature,...buyerPositionTags(p)]:[p.stage,p.temperature,...(p.tags||[])];return`<div class="prospector-map-sheet-handle" aria-hidden="true"></div><div class="prospector-map-sheet-head"><div><span>${buyer?'BUYER':'CONTACT'}</span><h3>${escapeHtml(p.name)}</h3><small>${escapeHtml(address)}</small></div><button type="button" data-close-prospector-map-sheet aria-label="Close">×</button></div><div class="prospector-map-sheet-tags">${tags.filter(Boolean).slice(0,5).map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')}</div><div class="prospector-map-next map-next-${escapeHtml(next.className||'none')}"><span>NEXT ACTION</span><strong>${escapeHtml(next.label||'No follow-up set')}</strong></div><div class="prospector-map-history"><span>LATEST CONVERSATIONS</span>${history.length?history.map(item=>`<article><strong>${escapeHtml(item.outcome||item.type)}</strong><small>${fmtDate(item.date)}${item.note?` · ${escapeHtml(item.note)}`:''}</small></article>`).join(''):'<small>No conversation history yet.</small>'}</div><div class="prospector-map-actions">${buyer?`<button type="button" data-call-buyer="${escapeHtml(p.id)}" ${phone?'':'disabled'}>Call</button><a href="${phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#'}" class="${phone?'':'disabled'}">SMS</a><button type="button" data-buyer-followup="${escapeHtml(p.id)}">Follow up</button><button type="button" data-open-buyer="${escapeHtml(p.id)}">Open profile</button>`:`<a href="${phone?prospectTel(p):'#'}" class="${phone?'':'disabled'}" data-prospect-call="${escapeHtml(p.id)}" data-call-from-session="0">Call</a><a href="${phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#'}" class="${phone?'':'disabled'}">Message</a><button type="button" data-log-prospect="${escapeHtml(p.id)}">Log contact</button><button type="button" data-open-prospect="${escapeHtml(p.id)}">Open profile</button>`}</div><button class="prospector-map-correct" type="button" data-correct-prospector-map="${escapeHtml(p.id)}">Correct pin location</button>`}
function openProspectorMapSheet(ids=[]){const records=ids.map(prospectById).filter(Boolean),sheet=$('#prospectorMapSheet');if(!sheet||!records.length)return closeProspectorMapSheet();if(records.length===1)sheet.innerHTML=prospectorMapRecordMarkup(records[0]);else sheet.innerHTML=`<div class="prospector-map-sheet-handle" aria-hidden="true"></div><div class="prospector-map-sheet-head"><div><span>SHARED ADDRESS</span><h3>${records.length} clients here</h3><small>${escapeHtml(formatProspectAddress(records[0].address||records[0].company,records[0].suburb))}</small></div><button type="button" data-close-prospector-map-sheet aria-label="Close">×</button></div><div class="prospector-map-shared-list">${records.map(p=>`<button type="button" data-prospector-map-record="${escapeHtml(p.id)}"><span class="prospect-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><span><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.recordType==='buyer'?'Buyer':p.stage)}</small></span><em class="prospect-temp temp-${p.temperature.toLowerCase()}">${escapeHtml(p.temperature)}</em><b>›</b></button>`).join('')}</div>`;sheet.classList.remove('hidden')}
function openProspectorMapReview(){const records=prospectMapReviewRecords(),sheet=$('#prospectorMapSheet');if(!sheet||!records.length)return toast('No mapped addresses need review');sheet.innerHTML=`<div class="prospector-map-sheet-handle" aria-hidden="true"></div><div class="prospector-map-sheet-head"><div><span>ADDRESS REVIEW</span><h3>${records.length} pin${records.length===1?'':'s'} need checking</h3><small>Confirm approximate or unresolved addresses.</small></div><button type="button" data-close-prospector-map-sheet aria-label="Close">×</button></div><div class="prospector-map-shared-list">${records.map(p=>{const location=prospectMapLocations.get(p.id);return`<button type="button" data-prospector-map-record="${escapeHtml(p.id)}"><span class="prospect-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><span><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb))}</small></span><em class="prospector-map-review-pill">${location?.status==='failed'?'Not found':'Review'}</em><b>›</b></button>`}).join('')}</div>`;sheet.classList.remove('hidden')}
function closeProspectorMapSheet(){$('#prospectorMapSheet')?.classList.add('hidden')}
function startProspectorMapCorrection(id){const p=prospectById(id);if(!p||!prospectMap)return;closeProspectorMapSheet();prospectMapManualProspectId=p.id;const current=prospectMapCurrentLocation(p),centre=current?[current.lng,current.lat]:prospectMap.getCenter();prospectMapManualCoordinate={lng:Number(centre.lng??centre[0]),lat:Number(centre.lat??centre[1])};if(prospectMapManualMarker)prospectMapManualMarker.remove();prospectMapManualMarker=new window.maplibregl.Marker({color:'#111318',draggable:true}).setLngLat(prospectMapManualCoordinate).addTo(prospectMap);prospectMapManualMarker.on('dragend',()=>{const point=prospectMapManualMarker.getLngLat();prospectMapManualCoordinate={lng:point.lng,lat:point.lat}});$('#prospectorMapCorrection')?.classList.remove('hidden');prospectMap.easeTo({center:prospectMapManualCoordinate,zoom:Math.max(16,prospectMap.getZoom()),duration:400})}
function placeProspectorMapCorrection(point){if(!prospectMapManualProspectId||!prospectMapManualMarker)return;prospectMapManualCoordinate={lng:Number(point.lng),lat:Number(point.lat)};prospectMapManualMarker.setLngLat(prospectMapManualCoordinate)}
function cancelProspectorMapCorrection(){prospectMapManualProspectId='';prospectMapManualCoordinate=null;if(prospectMapManualMarker){prospectMapManualMarker.remove();prospectMapManualMarker=null}$('#prospectorMapCorrection')?.classList.add('hidden')}
function saveProspectorMapCorrection(){const p=prospectById(prospectMapManualProspectId),point=prospectMapManualCoordinate;if(!p||!point)return;persistProspectMapLocation({prospectId:p.id,addressKey:prospectMapAddressKey(p),addressLabel:prospectMapAddress(p),lat:point.lat,lng:point.lng,accuracy:'manual',status:'confirmed',source:'manual',providerFeatureId:'',updatedAt:Date.now()});cancelProspectorMapCorrection();prospectMapFitPending=false;updateProspectorMap();openProspectorMapSheet([p.id]);toast('Pin location confirmed')}
function locateProspectorMap(){if(!navigator.geolocation)return toast('Location is not available on this device');navigator.geolocation.getCurrentPosition(position=>prospectMap?.easeTo({center:[position.coords.longitude,position.coords.latitude],zoom:15,duration:500}),()=>toast('Allow location access to centre the map'),{enableHighAccuracy:true,timeout:8000,maximumAge:60000})}
async function geocodeProspectMapRecord(p,apiKey){const address=prospectMapAddress(p),addressKey=prospectMapAddressKey(p),url=`https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(address)}&filter=countrycode:au&limit=1&format=geojson&apiKey=${encodeURIComponent(apiKey)}`,response=await fetch(url);if(response.status===401||response.status===403){const err=new Error('Geoapify key rejected');err.code='geocoder-key';throw err}if(response.status===429){const err=new Error('Geoapify free allowance reached');err.code='geocoder-limit';throw err}if(!response.ok)throw new Error(`Address matching failed (${response.status})`);const data=await response.json(),feature=data?.features?.[0],properties=feature?.properties||{},coordinates=feature?.geometry?.coordinates;if(!feature||!Array.isArray(coordinates)||coordinates.length<2){persistProspectMapLocation({prospectId:p.id,addressKey,addressLabel:address,lat:null,lng:null,accuracy:'not-found',status:'failed',source:'geoapify',updatedAt:Date.now()});return false}const confidence=Number(properties?.rank?.confidence)||0,matchType=cleanText(properties?.rank?.match_type||properties?.result_type||'unknown',40),strongMatch=/^(full_match|inner_part|match_by_building|building)$/i.test(matchType),status=strongMatch&&confidence>=.75?'mapped':'review';persistProspectMapLocation({prospectId:p.id,addressKey,addressLabel:address,lng:Number(coordinates[0]),lat:Number(coordinates[1]),accuracy:matchType,status,source:'geoapify',providerFeatureId:properties.place_id||feature.id||'',updatedAt:Date.now()});return true}
async function mapProspectorAddresses(){const apiKey=prospectMapToken();if(!apiKey){setProspectorMapSetup(true);return}if(prospectMapIsGeocoding)return;const pending=prospectMapPendingRecords().slice(0,PROSPECT_MAP_BATCH_LIMIT);if(!pending.length)return toast('All available addresses are mapped or ready for review');prospectMapIsGeocoding=true;const button=$('#mapProspectorAddresses');if(button)button.disabled=true;let mapped=0,failed=0;showProspectorMapMessage('Mapping addresses',`0 of ${pending.length} complete`);try{for(let index=0;index<pending.length;index++){try{if(await geocodeProspectMapRecord(pending[index],apiKey))mapped++;else failed++}catch(err){if(['geocoder-key','geocoder-limit'].includes(err?.code))throw err;failed++;console.warn('Address could not be mapped',pending[index]?.id,err)}if(button)button.textContent=`Mapping ${index+1} of ${pending.length}`;showProspectorMapMessage('Mapping addresses',`${index+1} of ${pending.length} complete`);updateProspectorMap();if(index<pending.length-1)await new Promise(resolve=>setTimeout(resolve,PROSPECT_MAP_GEOCODE_DELAY_MS))}prospectMapFitPending=true;updateProspectorMap();showProspectorMapMessage('', '');toast(`${mapped} address${mapped===1?'':'es'} mapped${failed?` · ${failed} need review`:''}`)}catch(err){console.error('Prospector mapping stopped',err);const limited=err?.code==='geocoder-limit';showProspectorMapMessage(limited?'Free daily allowance reached':'Address matching needs attention',limited?'Try mapping the remaining addresses tomorrow.':'Check the free Geoapify API key in Map settings.');if(err?.code==='geocoder-key')setProspectorMapSetup(true)}finally{prospectMapIsGeocoding=false;updateProspectorMapStatus()}}
async function archiveBuyer(id){const p=prospectById(id);if(!p||p.recordType!=='buyer'||p.archived)return;const now=Date.now(),cleared=p.nextFollowUp?` Follow-up for ${fmtDate(p.nextFollowUp)} was cleared.`:'';prospects=prospects.map(item=>item.id===id?normaliseProspect({...item,archived:true,archivedAt:now,nextFollowUp:'',updatedAt:now}):item);prospectInteractions.push({id:prospectId(),prospectId:id,date:todayKey(),at:now,type:'Archive',outcome:'Buyer archived',note:`Moved from active buyers to Archived.${cleared}`,nextFollowUp:''});await saveProspecting({render:false,awaitCloud:false});buyerBrowseMode='active';closeProspectDetail();renderTimeline();renderNowCard();toast('Buyer archived')}
async function restoreBuyer(id){const p=prospectById(id);if(!p||p.recordType!=='buyer'||!p.archived)return;const now=Date.now();prospects=prospects.map(item=>item.id===id?normaliseProspect({...item,archived:false,archivedAt:0,updatedAt:now}):item);prospectInteractions.push({id:prospectId(),prospectId:id,date:todayKey(),at:now,type:'Archive',outcome:'Buyer restored',note:'Returned to active buyers.',nextFollowUp:''});await saveProspecting({render:false,awaitCloud:false});buyerBrowseMode='active';buyerFilterState=defaultBuyerFilters();buyerQuickFilter='All';closeProspectDetail();toast('Buyer restored')}
...
function openBuyerFollowUp(id){
  const p=prospectById(id);if(!p||p.recordType!=='buyer'||p.archived)return;
...
async function scheduleBuyerFollowUp(id,date,note=''){const p=prospectById(id),followUpDate=validDateKey(date)?date:'';if(!p||p.recordType!=='buyer'||!followUpDate)return false;const at=Date.now();prospectInteractions.push({id:prospectId(),prospectId:p.id,date:todayKey(),at,type:'Follow-up',outcome:'Follow-up scheduled',note:cleanText(note,2000),nextFollowUp:followUpDate});prospects=prospects.map(item=>item.id===p.id?normaliseProspect({...item,nextFollowUp:followUpDate,updatedAt:at}):item);await saveProspecting({render:false,awaitCloud:false});closeBuyerFollowUp();renderBuyerProfiles();renderTimeline();renderNowCard();if(activeProspectId===p.id&&!$('#prospectDetail')?.classList.contains('hidden'))renderBuyerDetail(p.id);updateTopbar();toast('Buyer follow-up added');return true}
function openBuyerPurchaseForm(id){const p=prospectById(id);if(!p||p.recordType!=='buyer')return;activeProspectId=id;$('#prospectDetail').innerHTML=`<form id="buyerPurchaseForm" class="buyer-editor glass"><div class="prospect-detail-nav buyer-detail-nav"><button type="button" data-cancel-buyer-purchase>‹ Back</button><strong>Buyer → owner</strong><span></span></div><div class="buyer-editor-intro"><span>PURCHASED</span><h2>Move ${escapeHtml(p.name.split(' ')[0])} into Contacts</h2><p>Add the property they purchased. Their buyer history stays attached to the same record.</p></div><label>Purchased property address<input name="buyerPurchaseAddress" autocomplete="street-address" required placeholder="24 Smith Street, Toongabbie"></label><div class="prospect-form-grid"><label>Purchase price <small>Optional</small><input name="buyerPurchasePrice" type="number" inputmode="numeric" min="0" step="1000" placeholder="950000"></label><label>Purchase date<input name="buyerPurchaseDate" type="date" value="${todayKey()}" required></label></div><button class="primary" type="submit">Convert to owner</button></form>`}
async function convertBuyerToOwner(id,{address,price,date}){const p=prospectById(id);if(!p||p.recordType!=='buyer')return;const now=Date.now(),purchaseAddress=cleanText(address,240),purchasePrice=Math.max(0,Number(price)||0),purchaseDate=validDateKey(date)?date:todayKey(),tags=Array.from(new Set([...(p.tags||[]),'Owner'])).slice(0,12),updated=normaliseProspect({...p,recordType:'contact',buyerStage:'Purchased',buyerPurchaseAddress:purchaseAddress,buyerPurchasePrice:purchasePrice,buyerPurchaseDate:purchaseDate,buyerConvertedAt:now,address:purchaseAddress,tags,stage:'Nurture',updatedAt:now});prospects=prospects.map(item=>item.id===id?updated:item);prospectInteractions.push({id:prospectId(),prospectId:id,date:todayKey(),at:now,type:'Buyer',outcome:'Purchased · Converted to owner',note:`Purchased ${purchaseAddress}${purchasePrice?` for ${formatBuyerMoney(purchasePrice)}`:''}.`,nextFollowUp:''});await saveProspecting({render:false});activeProspectId=null;prospectContactsMode='active';setProspectorSection('contacts');renderProspecting();toast('Buyer moved to Contacts as an owner')}
function launchBuyerProfileCall(id){const p=prospectById(id),number=normaliseDialNumber(primaryProspectPhone(p));if(!p||p.recordType!=='buyer'||number.replace(/\D/g,'').length<6)return toast('Add a valid mobile number first');const pending={id:prospectId(),number,launchedAt:Date.now(),outcomeLogged:false,source:'buyer-profile',buyerProspectId:p.id,buyerName:p.name};writePendingManualCall(pending);manualCallLaunchGuardUntil=Date.now()+1600;window.location.href=`tel:${number}`;setTimeout(maybeShowManualCallOutcome,2600)}
async function logManualCallToBuyer(outcome,pending){if(pending?.source!=='buyer-profile'||!pending.buyerProspectId||outcome==='Cancelled')return;const p=prospectById(pending.buyerProspectId);if(!p||p.recordType!=='buyer'||prospectInteractions.some(item=>item.id===pending.id))return;prospectInteractions.push({id:pending.id,prospectId:p.id,date:todayKey(),at:Date.now(),type:'Call',outcome:cleanText(outcome,80),note:'',nextFollowUp:''});prospects=prospects.map(item=>item.id===p.id?normaliseProspect({...item,lastContact:todayKey(),updatedAt:Date.now()}):item);try{await saveProspecting({render:false,awaitCloud:false})}catch(err){console.error('Buyer call history save failed',err)}}
function saveManualCallAsBuyer(){const pending=readPendingManualCall();if(!pending?.number)return;const sessionBuyer=pending.source==='buyer-session'?buyerSession.contacts.find(item=>item.id===pending.buyerId):null,existing=(pending.buyerProspectId?prospectById(pending.buyerProspectId):null)||findBuyerByPhone(pending.number),prefill={name:existing?.name||sessionBuyer?.name||pending.buyerName||'',phone:existing?.phone||pending.number,address:existing?.address||sessionBuyer?.address||''},context={interactionId:pending.id,outcome:pending.outcome||manualCallOutcome,at:pending.loggedAt||Date.now(),returnToBuyerSession:pending.source==='buyer-session'};closeManualCallOutcome();switchView('prospectingView');setProspectorSection('buyers');openBuyerEditor(existing?.id||'',prefill,context);if(existing)toast(existing.archived?'Existing archived buyer found. Update the brief, then restore them.':'Existing buyer found. Update their brief.')}
function findProspectForAppointment({contactName='',contactNumber='',address=''}){
  const digits=normalisedPhoneDigits(contactNumber);
  if(digits){const byPhone=activeProspects().find(p=>normalisedPhoneDigits(primaryProspectPhone(p))===digits);if(byPhone)return byPhone}
  const name=cleanText(contactName,120).toLowerCase(),property=cleanText(address,240).toLowerCase();
  return activeProspects().find(p=>cleanText(p.name,120).toLowerCase()===name&&cleanText(p.address||p.company,240).toLowerCase()===property)||null;
}
...
function setProspectorSection(section='today'){
  prospectSection=['today','contacts','buyers','map','pipeline','market','broadcast','insights'].includes(section)?section:'today';
  $$('[data-prospector-section]').forEach(button=>{const active=button.dataset.prospectorSection===(prospectSection==='map'?prospectMapReturnSection:prospectSection);button.classList.toggle('active',active);if(active)button.setAttribute('aria-current','page');else button.removeAttribute('aria-current')});
  ...
  $('#prospectorMapPanel')?.classList.toggle('hidden',prospectSection!=='map');
  ...
  const toolbar=$('.prospecting-toolbar');toolbar?.classList.toggle('hidden',prospectSection==='insights'||prospectSection==='market'||prospectSection==='broadcast');toolbar?.classList.toggle('prospector-map-toolbar',prospectSection==='map');
  const input=$('#prospectSearch');if(input)input.placeholder=prospectSection==='contacts'?'Search name, street, suburb, phone etc.':prospectSection==='buyers'?'Search buyers':prospectSection==='map'?'Search mapped clients':prospectSection==='pipeline'?'Search seller pipeline':'Search name, address or phone';const addButton=$('#addProspectButton'),dialler=$('#openManualDialler');if(addButton){addButton.setAttribute('aria-label',prospectSection==='buyers'?'Add buyer':'Add prospect');addButton.classList.toggle('hidden',prospectSection==='map')}dialler?.classList.toggle('hidden',prospectSection==='map');
  if(prospectSection==='market')renderMarketPulse();if(prospectSection==='broadcast')renderBroadcastScreen();if(prospectSection==='map')renderProspectorMap();if(prospectSessionActive&&prospectSection==='today')return;
...
function openBroadcastBuilder(typeKey){
  ...
  selectedBroadcastType=typeKey;selectedBroadcastSuburb=normalisePlace('Toongabbie');selectedBroadcastStreet='';selectedBroadcastRecipientIds=new Set();broadcastStep=1;broadcastReviewMode='live';broadcastLastLaunch=null;
...
function closeBroadcastBuilder(){selectedBroadcastType='';selectedBroadcastSuburb='';selectedBroadcastStreet='';selectedBroadcastRecipientIds=new Set();broadcastStep=1;broadcastReviewMode='live';broadcastLastLaunch=null;renderBroadcastScreen();window.scrollTo({top:0,behavior:'smooth'})}
...
function renderProspecting(){renderBuyerSessionHero();
  ...
  }else if(prospectSessionActive&&prospectSection==='today'&&$('#prospectingSession')?.dataset.sessionView==='1'){
...
function prospectingSignature(prospectList=prospects,interactionList=prospectInteractions,marketEvents=marketPulseEvents){return JSON.stringify({prospects:prospectList,interactions:interactionList,marketPulseEvents:normaliseMarketPulseEvents(marketEvents)})}
...
function queueProspectingSave(){
  ...
  const snapshot={prospects:normaliseProspects(prospects),interactions:normaliseProspectInteractions(prospectInteractions),marketPulseEvents:normaliseMarketPulseEvents(marketPulseEvents)},signature=prospectingSignature(snapshot.prospects,snapshot.interactions,snapshot.marketPulseEvents);
...
function knockingHotSpottingRecommendations(){
  ...
  for(const event of normaliseMarketPulseEvents(marketPulseEvents)){
    ...
    const [street,suburb]=key.split('|'),matches=marketMatches(event),priority=hotSpottingPriority(event,matches.length),recency=relativeEventRecency(event);
...
async function endKnockingSession(){
  ...
  try{
    ...
    knockingSessionActive=false;knockingSessionVisible=false;knockingSessionStats={knocks:0,clients:0,data:0,MAP:0,LAP:0};knockingSessionLog=[];knockingSessionStartSeconds=0;clearKnockingSessionState();closeKnockingCapture();renderKnockingSession();renderToday();closeKnockingHistory();switchView('todayView');
...
$('#clearMarketPulseEvents')&&($('#clearMarketPulseEvents').onclick=async()=>{if(!marketPulseEvents.length)return;marketPulseEvents=[];saveLocal();renderMarketPulse();try{await queueProspectingSave()}catch(err){console.error('Hot Spotting clear sync failed',err)}toast('Hot Spotting cleared')});
...
$('#prospectingView').onclick=async e=>{
  const openMap=e.target.closest('[data-open-prospector-map]');if(openMap){openProspectorMap(openMap.dataset.openProspectorMap);return}
  if(e.target.closest('#closeProspectorMap')){closeProspectorMap();return}
  const mapFilter=e.target.closest('[data-prospector-map-filter]');if(mapFilter){prospectMapFilter=mapFilter.dataset.prospectorMapFilter||'all';prospectMapFitPending=true;updateProspectorMap();updateTopbar();return}
  if(e.target.closest('#mapProspectorAddresses')){await mapProspectorAddresses();return}
  if(e.target.closest('#reviewProspectorMap')){openProspectorMapReview();return}
  if(e.target.closest('#changeProspectorMapToken')){const input=$('#prospectorMapToken');if(input)input.value=prospectMapToken();setProspectorMapSetup(true);input?.focus({preventScroll:true});return}
  if(e.target.closest('#locateProspectorMap')){locateProspectorMap();return}
  if(e.target.closest('[data-close-prospector-map-sheet]')){closeProspectorMapSheet();return}
  const mapRecord=e.target.closest('[data-prospector-map-record]');if(mapRecord){openProspectorMapSheet([mapRecord.dataset.prospectorMapRecord]);return}
  const correctMap=e.target.closest('[data-correct-prospector-map]');if(correctMap){startProspectorMapCorrection(correctMap.dataset.correctProspectorMap);return}
  if(e.target.closest('#cancelProspectorMapCorrection')){cancelProspectorMapCorrection();return}
  if(e.target.closest('#saveProspectorMapCorrection')){saveProspectorMapCorrection();return}
  ...
  const openBuyer=e.target.closest('[data-open-buyer]');if(openBuyer){closeProspectorMapSheet();$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');renderBuyerDetail(openBuyer.dataset.openBuyer);return}
  ...
  const archiveBuyerButton=e.target.closest('[data-archive-buyer]');if(archiveBuyerButton&&confirm('Archive this buyer and clear their outstanding follow-up?')){await archiveBuyer(archiveBuyerButton.dataset.archiveBuyer);return}
  ...
  const deleteBuyer=e.target.closest('[data-delete-buyer]'),buyerToDelete=deleteBuyer?prospectById(deleteBuyer.dataset.deleteBuyer):null;if(deleteBuyer&&buyerToDelete?.recordType==='buyer'&&buyerToDelete.archived&&confirm('Permanently delete this archived buyer and their interaction history?')){prospects=prospects.filter(p=>p.id!==buyerToDelete.id);prospectInteractions=prospectInteractions.filter(x=>x.prospectId!==buyerToDelete.id);await saveProspecting({render:false,awaitCloud:false});closeProspectDetail();toast('Buyer permanently deleted');return}
  ...
  const buyerPositionTag=e.target.closest('[data-buyer-position-tag]');if(buyerPositionTag){buyerPositionTag.classList.toggle('active');buyerPositionTag.setAttribute('aria-pressed',String(buyerPositionTag.classList.contains('active')));return}
  ...
  if(e.target.closest('#broadcastBack')){if(selectedBroadcastType){if(broadcastStep>1)setBroadcastStep(broadcastStep-1);else closeBroadcastBuilder()}else{setProspectorSection('today');renderProspecting()}return}
  ...
  if(e.target.closest('#broadcastReturnProspector')){closeBroadcastBuilder();setProspectorSection('today');renderProspecting();return}
  ...
  const open=e.target.closest('[data-open-prospect]');if(open){closeProspectorMapSheet();$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.remove('hidden');renderProspectDetail(open.dataset.openProspect);return}
  ...
  const log=e.target.closest('[data-log-prospect]');if(log){closeProspectorMapSheet();openProspectLog(log.dataset.logProspect);return}
...
$('#prospectingView').addEventListener('keydown',e=>{if(e.key==='Enter'&&e.target.matches('[data-buyer-suburb-input]')){e.preventDefault();const form=e.target.closest('#buyerEditor'),first=form?.querySelector('[data-select-buyer-suburb]');addBuyerSuburb(form,first?.dataset.selectBuyerSuburb||'')}});
$('#prospectingView').onsubmit=async e=>{
  if(e.target.id==='prospectorMapTokenForm'){e.preventDefault();const token=cleanText(new FormData(e.target).get('token')||'',700);if(token.length<20||/\s/.test(token))return toast('Add a valid Geoapify API key');try{localStorage.setItem(PROSPECT_MAP_TOKEN_KEY,token);localStorage.removeItem('agnt-prospector-mapbox-token-v1')}catch{return toast('Address key could not be saved on this device')}setProspectorMapSetup(false);showProspectorMapMessage('', '');await ensureProspectorMap();updateProspectorMapStatus();toast('Free address matching connected');return}
  if(e.target.id==='buyerEditor'){
    e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),editingBuyerId=activeProspectId||'',wasExisting=Boolean(editingBuyerId);if(submit?.disabled)return;if(submit){submit.disabled=true;submit.textContent='Saving…'}
    ...
    try{await upsertBuyer({name:f.get('name'),phone:f.get('phone'),address:f.get('address'),buyerBudgetMin:0,buyerBudgetMax:maxValue,buyerBedrooms:Number(f.get('buyerBedrooms'))||0,buyerBathrooms:Number(f.get('buyerBathrooms'))||0,buyerCars:Number(f.get('buyerCars'))||0,buyerSuburbs:form._buyerSuburbs||[],buyerPropertyType:propertyType,buyerFeatures:features,buyerPositionTags:positionTags,buyerSeller:positionTags.includes('Buyer Seller'),buyerStage:f.get('buyerStage'),temperature:f.get('temperature'),notes:f.get('notes'),source:wasExisting?prospectById(editingBuyerId)?.source||'Buyer':'Buyer'},editingBuyerId,pendingBuyerEditorContext);toast(wasExisting?'Buyer saved':'Buyer added')}
    catch(err){if(err?.code==='buyer-duplicate'&&err.existingId){const context=pendingBuyerEditorContext,duplicate=prospectById(err.existingId);openBuyerEditor(err.existingId,{},context);toast(duplicate?.archived?'This mobile belongs to an archived buyer. Update the brief, then restore them.':'Existing buyer found. Update their brief.');return}console.error('Buyer save failed',err);toast('Buyer saved locally. Please check sync.');if(submit){submit.disabled=false;submit.textContent=wasExisting?'Save buyer':'Add buyer'}}
    return
  }
