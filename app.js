...
function returningSnapshotNextBlock(now,data){
  const primary=dailyRunSheetState(todayKey(),now).items[0];
  return primary?{title:primary.title,meta:primary.meta}:{title:'Strengthen tomorrow',meta:'Use the clear space for follow-ups, appointments and pipeline'};
}
...
function ensureTick(){
  ...
  const knockingActive=Boolean(dayData(selectedDate).timerStartedAt);
  timerTick=setInterval(()=>{
    if(dayData(selectedDate).timerStartedAt){
      renderKnockTimerOnly();renderKnockingSession();
      const d=dayData(selectedDate),target=rollingKnockTarget(selectedDate)*60;
      if(liveKnockSeconds(d)>=target&&!d.alarmPlayed){d.alarmPlayed=true;days[selectedDate]=d;saveDay(selectedDate,{quiet:true});alarm()}
      return;
    }
    renderNowCard();
    if(activeViewId()==='scheduleView')renderTimeline();
    refreshReturningSnapshotIfVisible();
  },knockingActive?1000:60000);
}
...
function runSheetRoundedMinutes(minutes){
  const value=Math.max(0,Number(minutes)||0);
  if(value<=5)return Math.ceil(value)||1;
  return Math.ceil(value/5)*5;
}
function runSheetEstimate(count,secondsEach){return formatEstimatedTime(runSheetRoundedMinutes(estimatedMinutes(count,secondsEach)))}
function dailyRunSheetHotSpotting(){
  const opportunities=normaliseMarketPulseEvents(marketPulseEvents).map(event=>{
    const matches=marketMatches(event),progress=marketSessionProgress(event,matches),remaining=Math.max(0,progress.total-progress.workedIds.size),priority=hotSpottingPriority(event,matches.length);
    return{event,matches,progress,remaining,priority};
  }).filter(item=>item.remaining>0).sort((a,b)=>Number(b.progress.active)-Number(a.progress.active)||b.priority.score-a.priority.score||Number(b.event.createdAt)-Number(a.event.createdAt));
  return{opportunities,total:opportunities.reduce((sum,item)=>sum+item.remaining,0),best:opportunities[0]||null};
}
function dailyRunSheetState(viewDate=selectedDate,now=new Date()){
  const priority=timelinePriority(viewDate);
  if(viewDate<todayKey())return{label:'REVIEW',items:[{id:'review-day',title:'Review this day',meta:`${completion(viewDate)}% complete · See the activity and outcomes recorded`,action:'timeline',actionLabel:'Review',date:viewDate}]};
  if(viewDate>todayKey())return{label:'PLAN',items:[{id:'plan-day',title:'Plan this day',meta:priority.meta||'Review appointments, follow-ups and the first prospecting block',action:'timeline',actionLabel:'Plan',date:viewDate}]};

  const candidates=[],nowMinutes=now.getHours()*60+now.getMinutes(),workday=isWorkDayKey(viewDate),metricState=coachingMetricState(viewDate),allCoreComplete=metricState.incomplete.length===0,knockRemaining=metricState.knockRemaining;
  const appointments=appointmentEntriesForDate(viewDate).map(entry=>({entry,start:timelineMinutes(entry.appointment.time),duration:appointmentDurationMinutes(entry.appointment)})).sort((a,b)=>a.start-b.start);
  const currentAppointment=appointments.find(item=>nowMinutes>=item.start&&nowMinutes<item.start+item.duration),nextAppointment=appointments.find(item=>item.start>nowMinutes);
  const appointmentItem=currentAppointment||nextAppointment;
  if(appointmentItem){
    const a=appointmentItem.entry.appointment,isCurrent=appointmentItem===currentAppointment,minutesAway=appointmentItem.start-nowMinutes,contact=a.contactName||a.name||appointmentType(a),address=a.address||'Address not recorded',time=appointmentTimeLabel(a,appointmentItem.entry.sourceDate);
    candidates.push({
      id:`appointment-${calendarExportId(a,appointmentItem.entry.sourceDate)}`,
      rank:isCurrent?0:minutesAway<=30?2:minutesAway<=90?18:48,
      when:isCurrent?'NOW':time,
      title:isCurrent?`${appointmentType(a)} appointment now`:`Prepare for ${appointmentType(a)} appointment`,
      meta:`${time} · ${appointmentItem.duration} min · ${contact} · ${address}`,
      action:'appointment',actionLabel:'Open',date:viewDate
    });
  }

  const contactFollowUps=prospectFollowUpsForDate(viewDate),overdueContacts=contactFollowUps.filter(p=>p.nextFollowUp<viewDate).length;
  if(contactFollowUps.length)candidates.push({
    id:'contact-followups',rank:overdueContacts?4:nowMinutes<12*60?8:24,when:overdueContacts?'OVERDUE':'NEXT',
    title:overdueContacts?`Clear ${overdueContacts} overdue follow-up${overdueContacts===1?'':'s'}`:`Complete ${contactFollowUps.length} contact follow-up${contactFollowUps.length===1?'':'s'}`,
    meta:`Protect the commitments already made · ${runSheetEstimate(contactFollowUps.length,240)}`,
    action:'contact-followups',actionLabel:'Start',date:viewDate
  });

  const appointmentFollowUps=dueFollowUps(),overdueAppointments=appointmentFollowUps.filter(({appointment:a})=>!a.followUpDate||a.followUpDate<viewDate).length;
  if(appointmentFollowUps.length)candidates.push({
    id:'appointment-followups',rank:overdueAppointments?5:10,when:overdueAppointments?'OVERDUE':'NEXT',
    title:`Close ${appointmentFollowUps.length} appointment outcome${appointmentFollowUps.length===1?'':'s'}`,
    meta:`Keep every appointment moving to a clear next step · ${runSheetEstimate(appointmentFollowUps.length,360)}`,
    action:'appointment-followups',actionLabel:'Open',date:viewDate
  });

  const sessionRemaining=Math.max(0,prospectSessionIds.length-prospectSessionIndex),activeEventId=cleanText(prospectSessionContext?.eventId,160);
  if(prospectSessionActive){
    const activeEvent=activeEventId?normaliseMarketPulseEvents(marketPulseEvents).find(event=>event.id===activeEventId):null;
    candidates.push({
      id:activeEventId?'active-hotspot':'active-pipeline',rank:6,when:'ACTIVE',
      title:activeEventId?`Resume ${activeEvent?.suburb||'hot spotting'}`:'Resume today’s pipeline',
      meta:`${sessionRemaining} contact${sessionRemaining===1?'':'s'} remaining · ${runSheetEstimate(sessionRemaining,activeEventId?150:180)}`,
      action:activeEventId?'hotspot':'pipeline-session',actionLabel:'Resume',eventId:activeEventId,date:viewDate
    });
  }

  if(workday&&knockRemaining>0){
    if(nowMinutes>=14*60&&nowMinutes<17*60){
      const finish=new Date(now.getTime()+knockRemaining*60000),finishLabel=finish.toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'});
      candidates.push({id:'knocking-now',rank:11,when:'NOW',title:knockingSessionActive?'Resume door knocking':'Start door knocking',meta:`${knockRemaining} min remaining · Finish around ${finishLabel}`,action:'knocking',actionLabel:knockingSessionActive?'Resume':'Start',date:viewDate});
    }else if(nowMinutes>=13*60+30&&nowMinutes<14*60){
      candidates.push({id:'knocking-ready',rank:16,when:'2:00PM',title:'Prepare the knocking block',meta:`Starts in ${14*60-nowMinutes} min · ${knockRemaining} min planned`,action:'timeline',actionLabel:'View',date:viewDate});
    }else if(nowMinutes<13*60+30){
      candidates.push({id:'knocking-later',rank:52,when:'2:00PM',title:'Door knocking at 2:00pm',meta:`${knockRemaining} min planned · Protect the field block`,action:'timeline',actionLabel:'View',date:viewDate});
    }else{
      candidates.push({id:'knocking-unfinished',rank:68,when:'REVIEW',title:'Review unfinished knocking',meta:`${knockRemaining} min will roll into the next scheduled workday`,action:'timeline',actionLabel:'Review',date:viewDate});
    }
  }

  if(workday&&!prospectSessionActive){
    const hotSpot=dailyRunSheetHotSpotting().best;
    if(hotSpot)candidates.push({
      id:`hotspot-${hotSpot.event.id}`,rank:hotSpot.progress.active?12:27,when:hotSpot.progress.active?'ACTIVE':'NEXT',
      title:`${hotSpot.progress.active?'Resume':'Hot spot'} ${hotSpot.event.suburb||hotSpot.event.address}`,
      meta:`${hotSpot.remaining} nearby contact${hotSpot.remaining===1?'':'s'} · ${runSheetEstimate(hotSpot.remaining,150)} · ${hotSpot.event.eventType}`,
      action:'hotspot',actionLabel:hotSpot.progress.active?'Resume':'Start',eventId:hotSpot.event.id,date:viewDate
    });

    const pipelineCount=returningSnapshotPipelineCount();
    if(pipelineCount)candidates.push({
      id:'daily-pipeline',rank:allCoreComplete?62:32,when:'NEXT',title:'Work today’s pipeline',
      meta:`${pipelineCount} contact${pipelineCount===1?'':'s'} · ${runSheetEstimate(pipelineCount,180)} · Builds calls, connects and data`,
      action:'pipeline-session',actionLabel:'Start',date:viewDate
    });
    else if(!allCoreComplete&&nowMinutes<18*60+30)candidates.push({
      id:'core-prospecting',rank:34,when:'NEXT',title:priority.title,
      meta:priority.meta,action:'prospector',actionLabel:'Open',date:viewDate
    });
  }

  if(nowMinutes>=18*60+30)candidates.push({id:'plan-tomorrow',rank:15,when:'NOW',title:'Plan tomorrow',meta:'Close today’s loop and leave the next workday ready · ~10 min',action:'timeline',actionLabel:'Plan',date:viewDate});
  else if(nowMinutes>=17*60&&allCoreComplete&&knockRemaining===0)candidates.push({id:'calendar-management',rank:15,when:'NOW',title:'Calendar management',meta:'Finalise follow-ups and protect tomorrow’s first block · ~10 min',action:'appointments',actionLabel:'Open',date:viewDate});

  if(!candidates.length){
    candidates.push(workday
      ?{id:'day-complete',rank:100,when:'NOW',title:'Day complete',meta:'All daily targets are banked · Review the day or strengthen tomorrow',action:'timeline',actionLabel:'Review',date:viewDate}
      :{id:'recovery-day',rank:100,when:'NOW',title:'Recovery day',meta:'No accountability targets are scheduled · Appointments and commitments will still surface here',action:'timeline',actionLabel:'Review',date:viewDate});
  }
  const seen=new Set(),items=candidates.sort((a,b)=>a.rank-b.rank).filter(item=>{if(seen.has(item.id))return false;seen.add(item.id);return true}).slice(0,3);
  return{label:'NOW',items};
}
function openRunSheetTimeline(date=todayKey()){
  selectedDate=date;appointmentDate=date;if($('#appointmentDatePicker'))$('#appointmentDatePicker').value=date;
  switchView('scheduleView');setTodayPage('overview');
}
function executeDailyRunSheetAction(index=0){
  const item=dailyRunSheetState(selectedDate).items[index];if(!item)return;
  if(item.action==='timeline'){openRunSheetTimeline(item.date);return}
  if(item.action==='appointment'){appointmentDate=item.date||todayKey();if($('#appointmentDatePicker'))$('#appointmentDatePicker').value=appointmentDate;switchView('appointmentsView');return}
  if(item.action==='appointment-followups'){appointmentDate=todayKey();if($('#appointmentDatePicker'))$('#appointmentDatePicker').value=appointmentDate;switchView('appointmentsView');setAppointmentHistoryScreen('past');return}
  if(item.action==='contact-followups'){openRunSheetTimeline(item.date);requestAnimationFrame(()=>document.querySelector('.timeline-item.current.followup,.timeline-item.followup')?.scrollIntoView({block:'center',behavior:'smooth'}));return}
  if(item.action==='pipeline-session'){selectedDate=todayKey();switchView('prospectingView');setProspectorSection('today');renderProspecting();startProspectingSession();return}
  if(item.action==='hotspot'){switchView('prospectingView');if(item.eventId){startMarketPulseSession(item.eventId);return}setProspectorSection('market');renderProspecting();return}
  if(item.action==='knocking'){selectedDate=todayKey();switchView('todayView');startKnockingSession();return}
  if(item.action==='prospector'){switchView('prospectingView');setProspectorSection('today');renderProspecting();return}
  openRunSheetTimeline(item.date);
}
function renderNowCard(){
  const state=dailyRunSheetState(selectedDate),primary=state.items[0];if(!primary)return;
  if($('#nowCardLabel'))$('#nowCardLabel').textContent=state.label;
  if($('#nowCardTitle'))$('#nowCardTitle').textContent=primary.title;
  if($('#nowCardMeta'))$('#nowCardMeta').textContent=primary.meta;
  if($('#nowCardAction'))$('#nowCardAction').textContent=primary.actionLabel||'Open';
  if($('#openTodayTimeline'))$('#openTodayTimeline').setAttribute('aria-label',`${primary.actionLabel||'Open'} ${primary.title}`);
  const queue=$('#runSheetQueue');if(!queue)return;
  queue.innerHTML=state.items.slice(1).map((item,index)=>`<button class="run-sheet-queue-item" type="button" data-run-sheet-action="${index+1}" aria-label="${escapeHtml(`${item.actionLabel||'Open'} ${item.title}`)}"><span>${escapeHtml(item.when||(index?'LATER':'NEXT'))}</span><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.meta)}</small></span><b>${escapeHtml(item.actionLabel||'Open')} <i aria-hidden="true">›</i></b></button>`).join('');
  queue.classList.toggle('hidden',state.items.length<=1);
}
