...
function buyerCard(p){const phone=primaryProspectPhone(p),sms=phone?`sms:${phone.replace(/[^+\d]/g,'')}`:'#',tags=buyerTagsForCard(p),last=p.lastContact?`Last contact ${fmtDate(p.lastContact)}`:'No contact logged';return`<article class="buyer-card"><div class="buyer-card-head"><button type="button" data-open-buyer="${p.id}" class="buyer-card-name"><span class="prospect-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'B')}</span><span><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax))}</small></span></button><div class="buyer-card-actions"><button type="button" data-call-buyer="${p.id}" ${phone?'':'disabled'}>Call</button><a href="${sms}" class="${phone?'':'disabled'}" aria-label="SMS ${escapeHtml(p.name)}">SMS</a></div></div><button type="button" data-open-buyer="${p.id}" class="buyer-card-brief"><strong>${escapeHtml(buyerConfigText(p))}</strong><small>${escapeHtml(buyerSuburbText(p))}</small><span class="buyer-card-tags">${tags.length?tags.slice(0,3).map(tag=>`<span>${escapeHtml(tag)}</span>`).join('')+(tags.length>3?`<span>+${tags.length-3}</span>`:''):'<span>Requirements not set</span>'}</span><span class="buyer-card-footer"><span class="buyer-stage-pill">${escapeHtml(p.buyerStage||'Looking')}</span><span class="prospect-temp temp-${p.temperature.toLowerCase()}">${escapeHtml(p.temperature)}</span><em>${escapeHtml(last)}</em></span></button></article>`}
...
function setProspectorSection(section='today'){
  ...
  const input=$('#prospectSearch');if(input)input.placeholder=prospectSection==='contacts'?'Search name, street, suburb, phone etc.':prospectSection==='buyers'?'Search buyers, suburbs or requirements':prospectSection==='pipeline'?'Search seller pipeline':'Search name, address or phone';const addButton=$('#addProspectButton');if(addButton)addButton.setAttribute('aria-label',prospectSection==='buyers'?'Add buyer':'Add prospect');
