...
function leaderboardAppointmentDetailsBetween(startKey,endKey){
  ...
  Object.entries(days).forEach(([sourceDate,raw])=>(raw?.appointments||[]).forEach(appointment=>{
    ...
    details.push({type,bookedDate,scheduledDate:appointmentScheduledDate(appointment,sourceDate),time:appointment.time||'',contactName:appointment.contactName||appointment.name||'',address:appointment.address||''});
  }));
...
function showLeaderboardAgentSummary(agentUid){
  ...
  const cards=details?.length?details.map(item=>`<article class="leaderboard-agent-appointment"><span>${escapeHtml(item.type)}</span><div><strong>${escapeHtml(item.contactName||'Client not recorded')}</strong><small>${escapeHtml(item.address||'Address not recorded')}</small>${item.type==='Data'?'':`<time>${escapeHtml(leaderboardAppointmentDateTime(item))}</time>`}</div></article>`).join(''):details===null&&knownTotal?`<div class="leaderboard-agent-summary-empty"><strong>${knownTotal} appointment${knownTotal===1?'':'s'} booked</strong><p>Details will appear after this agent receives the latest app update.</p></div>`:`<div class="leaderboard-agent-summary-empty"><strong>No appointments or data recorded</strong><p>There are no MAP, LAP, BAP or Data entries recorded for this period.</p></div>`;
...
function nextProspectPipelineBatch(){
  const excluded=new Set(prospectSessionIds);
  return sortedEligibleProspectPipeline().filter(p=>!excluded.has(p.id)&&!prospectContactedToday(p.id)).slice(0,50).map(p=>p.id);
}
...
function openPipelineRefreshConfirm(){
  ...
  overlay.innerHTML=`<section class="pipeline-refresh-card glass" role="dialog" aria-modal="true" aria-labelledby="pipelineRefreshTitle"><span class="eyebrow">REFRESH SESSION</span><h2 id="pipelineRefreshTitle">Load a new client queue?</h2><p>This will replace the unworked contacts in this session with ${next.length} new eligible client${next.length===1?'':'s'} using the same pipeline criteria. Activity you’ve already logged today stays unchanged.</p><div class="pipeline-refresh-actions"><button class="secondary" type="button" data-cancel-pipeline-refresh>Keep Current</button><button class="primary" type="button" data-confirm-pipeline-refresh>Refresh ${next.length}</button></div></section>`;
