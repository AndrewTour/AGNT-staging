...
let appointmentAssignees=[],assignedTeamAppointments=[],pendingTeamAppointmentNotice=null,teamAppointmentNoticeOpen=false,teamAppointmentNoticeReturnState=null,subscribedAppointmentTeamId='',pendingAppointmentAssignment=null;
...
function clearActiveSession(){teamInitialisationToken++;unsubDays?.();unsubProfile?.();unsubLeaderboard?.();unsubProspecting?.();unsubTeamMembership?.();unsubTeamMembers?.();unsubAppointmentAssignees?.();unsubAssignedTeamAppointments?.();unsubDays=unsubProfile=unsubLeaderboard=unsubProspecting=unsubTeamMembership=unsubTeamMembers=unsubAppointmentAssignees=unsubAssignedTeamAppointments=null;hideTeamAppointmentNotice({acknowledge:false,restore:false});hideTeamManager({restoreFocus:false});closeTeamMemberRemoval({force:true});hideTeamLeaveConfirmation({force:true,restoreFocus:false});hideTeamCodeRefreshConfirmation({force:true,restoreFocus:false});clearInterval(timerTick);clearTimeout(syncTimer);clearTimeout(leaderboardPublishTimer);clearTimeout(prospectingSaveTimer);clearTimeout(returningSnapshotTimer);returningSnapshotTimer=null;prospectingSaveTimer=null;pendingProspectingPayload=null;pendingProspectingSignature='';prospectingWriteInFlight=false;prospectingSaveWaiters.splice(0).forEach(({resolve})=>resolve());currentUser=null;uid='local';cloud=false;pendingSyncOperations=0;syncHasError=false;lastLeaderboardSignature='';lastTeamLeaderboardSignature='';lastProspectingSignature='';dirtyDayKeys=new Set();resetState()}
...
function renderAppointmentAssignmentPopup(preferredUid=uid){
  ...
  const sorted=[...appointmentAssignees].sort((a,b)=>String(a.uid)===uid?-1:String(b.uid)===uid?1:teamAppointmentMemberName(a).localeCompare(teamAppointmentMemberName(b)));
  const members=sorted.length?sorted:[{uid,name:displayAgentName()}];
  select.innerHTML=members.map(member=>`<option value="${escapeHtml(member.uid)}">${escapeHtml(String(member.uid)===uid?'Me':teamAppointmentMemberName(member))}</option>`).join('');
...
function stopTeamAppointmentLayer(){unsubAppointmentAssignees?.();unsubAssignedTeamAppointments?.();unsubAppointmentAssignees=unsubAssignedTeamAppointments=null;subscribedAppointmentTeamId='';appointmentAssignees=[];assignedTeamAppointments=[];pendingTeamAppointmentNotice=null;hideTeamAppointmentNotice({acknowledge:false,restore:false});renderAppointmentAssigneePicker();renderAppointments()}
...
function captureTeamAppointmentNoticeReturnState(){return{view:activeViewId(),todayPage,selectedDate,appointmentDate,appointmentHistoryMode,prospectSection,activeProspectId,scrollY:window.scrollY}}
function restoreTeamAppointmentNoticeReturnState(state){
  if(!state)return;
  selectedDate=state.selectedDate||selectedDate;appointmentDate=state.appointmentDate||appointmentDate;todayPage=state.todayPage||todayPage;prospectSection=state.prospectSection||prospectSection;activeProspectId=state.activeProspectId||activeProspectId;
  if(state.view==='appointmentsView'){appointmentHistoryMode=state.appointmentHistoryMode||null;$('#appointmentMainContent')?.classList.toggle('hidden',Boolean(appointmentHistoryMode));$('#appointmentHistoryScreen')?.classList.toggle('hidden',!appointmentHistoryMode)}
  switchView(state.view||'todayView');
  if(state.view==='scheduleView'||state.view==='todayView')setTodayPage(state.todayPage||'overview');
  if(state.view==='prospectingView'){renderProspecting();if(prospectSessionActive&&prospectSection==='today')showProspectingSession();else if(activeProspectId)renderProspectDetail(activeProspectId)}
  requestAnimationFrame(()=>window.scrollTo({top:Number(state.scrollY)||0,behavior:'instant'}));
}
function maybeShowTeamAppointmentNotice(){if(teamAppointmentNoticeOpen||!cloud||accountMode!=='team'||!teamId||$('#app')?.classList.contains('hidden'))return;const next=assignedTeamAppointments.filter(a=>!a.acknowledgedAt&&a.setterUid!==uid).sort((a,b)=>(Number(a.at)||0)-(Number(b.at)||0))[0];if(!next)return;pendingTeamAppointmentNotice=next;teamAppointmentNoticeReturnState=captureTeamAppointmentNoticeReturnState();teamAppointmentNoticeOpen=true;const modal=$('#teamAppointmentNotice');if(!modal)return;const setter=teamAppointmentSetterFirstName(next.setterName),label=teamAppointmentConfirmationLabel(appointmentType(next));$('#teamAppointmentNoticeTitle').textContent=`${setter} booked this ${label} for you.`;$('#teamAppointmentNoticeType').textContent=appointmentType(next);$('#teamAppointmentNoticeContact').textContent=next.contactName||'Contact not recorded';$('#teamAppointmentNoticeAddress').textContent=next.address||'Address not recorded';$('#teamAppointmentNoticeWhen').textContent=`${fmtDate(appointmentScheduledDate(next,next.createdDate||todayKey()))} · ${appointmentTimeLabel(next,next.createdDate||todayKey())}`;modal.classList.remove('hidden');modal.setAttribute('aria-hidden','false');document.body.classList.add('team-appointment-notice-open');requestAnimationFrame(()=>$('#teamAppointmentAddCalendar')?.focus({preventScroll:true}))}
...
function hideTeamAppointmentNotice({acknowledge=true,calendar=false,restore=true}={}){const modal=$('#teamAppointmentNotice');if(!modal)return;const current=pendingTeamAppointmentNotice,returnState=teamAppointmentNoticeReturnState;modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');document.body.classList.remove('team-appointment-notice-open');teamAppointmentNoticeOpen=false;pendingTeamAppointmentNotice=null;teamAppointmentNoticeReturnState=null;if(acknowledge&&current){current.acknowledgedAt=current.acknowledgedAt||Date.now();if(calendar)current.calendarAddedAt=current.calendarAddedAt||Date.now();acknowledgeTeamAppointment(current,{calendar})}if(restore)restoreTeamAppointmentNoticeReturnState(returnState)}
...
function activeProspects(){return prospects.filter(p=>!p.archived)}
function archivedProspects(){return prospects.filter(p=>p.archived)}
...
$('#teamAppointmentAddCalendar')?.addEventListener('click',()=>{const a=pendingTeamAppointmentNotice;if(!a)return;hideTeamAppointmentNotice({acknowledge:true,calendar:true,restore:true});exportAppointmentToCalendar(a,a.createdDate||todayKey())});
$('#teamAppointmentGotIt')?.addEventListener('click',()=>hideTeamAppointmentNotice({acknowledge:true,restore:true}));
$('#teamAppointmentNotice')?.addEventListener('click',e=>{if(e.target.id==='teamAppointmentNotice')hideTeamAppointmentNotice({acknowledge:true,restore:true})});
...
$('#manualCallDone').onclick=()=>{const pending=readPendingManualCall();closeManualCallOutcome();if(pending?.source==='buyer-session')showBuyerSession();else openManualDialler()};
