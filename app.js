...
function myMarketPropertyMarkup(row,recorded){
  ...
  return`<details class="my-market-property" data-property-key="${escapeHtml(row.key)}"><summary><span><strong>${escapeHtml(e.address)}</strong><small>${escapeHtml(e.suburb)} · ${escapeHtml(e.propertyDetails||marketPulseEventPropertyCategory(e)||'Details not recorded')}</small><span class="my-market-result"><b>${escapeHtml(price)}</b><small>${escapeHtml(label)} · ${escapeHtml(fmtDate(e.receivedDate))}</small></span></span></summary><div class="my-market-property-body"><p class="my-market-muted">${escapeHtml(agency)}${e.agents?.length?` · ${escapeHtml(e.agents.join(', '))}`:''}</p>${myMarketFactsMarkup(facts)}<details class="my-market-outreach" data-market-detail="history"><summary>Campaign history · ${stats.events.length} report${stats.events.length===1?'':'s'}</summary><p class="my-market-muted">Dates are report dates. Earlier or unreported steps may be missing.</p><ol class="my-market-history">${history}</ol></details><details class="my-market-outreach" data-market-detail="outreach"><summary>Recorded outreach · ${recorded.length} contact${recorded.length===1?'':'s'}</summary><p class="my-market-muted">Includes attempts and requested follow-ups; these do not confirm successful contact.</p>${recorded.length?recorded.map(myMarketContactMarkup).join(''):'<p class="my-market-muted">No property-linked outreach recorded.</p>'}</details></div></details>`;
}
function renderMyMarketHub(){
  ...
  const contacts=myMarketContactIndex(rows),list=$('#myMarketProperties'),open=new Map([...list.querySelectorAll(':scope > details[open]')].map(el=>[el.dataset.propertyKey,new Set([...el.querySelectorAll('details[open][data-market-detail]')].map(child=>child.dataset.marketDetail))]));
  ...
  if(list.dataset.markup!==markup){list.innerHTML=markup;list.dataset.markup=markup;for(const detail of list.querySelectorAll(':scope > details')){const nested=open.get(detail.dataset.propertyKey);detail.open=Boolean(nested);if(nested)for(const child of detail.querySelectorAll('details[data-market-detail]'))child.open=nested.has(child.dataset.marketDetail)}}
